...
if( nVal=="" ){
S(_RowEdit).css("height", _RowEdit.getAttribute("eHeight"));
}
if( !elExist ){
setTimeout(function(){
elNextField(oTD);
...
var eventType, eventButton, eventCtrl;
if( window.event!=null ){
eventType = window.event.type;
eventButton = window.event.button;
eventCtrl = window.event.ctrlKey;
if( eventCtrl && eventButton==2 ) return true;
}
if( _RowExit ) return;
var oTD = (xObj==undefined || xObj.tagName==undefined) ? S.event(window) : xObj;
if( oTD.tagName=='IMG' || oTD.tagName=='I' ) oTD = oTD.parentNode;
if( oTD.tagName=='TH' && eventType=='click' ){
try{
if( eventType=='click' ) SeleccionaLinea();
}catch(e){}
return true;
}
if( oTD.children.length && oTD.children[0].tagName!='I' ){
oTD.children[0].focus();
return eClearEvent();
}
if( oTD.tagName!='TD' || oTD.disabled || oTD.parentNode.className=='PieLista' || oTD.parentNode.getAttribute("libre")==1 ){
if( oTD.parentNode==oTD["eParent"] ){
oTD.focus();
}
return eClearEvent();
}
...
var  font = (/^(\+|\+,|\-|\-,|F4|P4|CP|T|CDI|H8|H5|H2|CLR|clr|0|DC)$/.test(attr["td"])) ? top._FontNumber+";" : ""
,a = "100%"
,txt = "<input id='_tmpInput' size=50 style='display:inline-block;"+font+("*,+,-,".indexOf(attr["td"])>-1 ? "text-align:right;":"")+"width:"+a+"; background:transparent; border-style:hidden; border-radius:0px; padding:0px !important; margin:0px;";
txt += "'";
...
o.obj["eParent"] = oTD;
if( (w.x+w.w)<(c.x+c.w+30) ){
S.scrollSet(window, {left: (c.x+c.w+30)-w.w});
}
o.obj.focus();
S.session.lastKey = 0;
...
document.body.focus();
setTimeout('_RecalcSlideTH()', 750);
S.call("edes.php?E:$mod_td.gs"+_DBExt, datos, {function:function(v){
S.info();
var ok = true;
if( (v*1)==v ){
S.info(237);
