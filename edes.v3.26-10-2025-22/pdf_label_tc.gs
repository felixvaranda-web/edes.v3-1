...
if( $i==5 ){
if( ($_PDFLABEL[$n][$i][0]=='"' && mb_substr($_PDFLABEL[$n][$i],-1)=='"') || ($_PDFLABEL[$n][$i][0]=="'" && mb_substr($_PDFLABEL[$n][$i],-1)=="'") ){
$_PDFLABEL[$n][$i] = mb_substr($_PDFLABEL[$n][$i],1,-1);
}
}
}
...
$tmp = trim($tmp[1]);
$tmp = str_replace('\\','/',$tmp);
$LabelRellena = (mb_strtolower($tmp)=='label');
if( $tmp=='' ){
...
if( eSubstrCount($NomPDF, '.pdf')==0 ) $NomPDF .= '.pdf';
$NomFile = $NomPDF;
...
clearstatcache();
list($OffsetX, $OffsetY) = explode(',', trim(str_replace(' ','',mb_strtoupper($_PDFLABEL[0][1]))));
$OffsetX = mb_strtoupper($OffsetX);
$OffsetY = mb_strtoupper($OffsetY);
list($tmp1, $tmp2) = explode('/', trim(str_replace(' ','',mb_strtoupper($_PDFLABEL[0][2]))));
list($Ancho, $Alto) = explode(',', $tmp1);
list($AnchoPT, $AltoPT) = explode(',', $tmp2);
$tmp = trim($_PDFLABEL[0][3]);
$tmp = str_replace(' ','',mb_strtoupper($tmp));
...
if( eSubstrCount('CHT',$tmp[0])==0 ) eMessage("ERROR: Tipo letra desconocido '{$tmp}'",'HES');
$tmp = str_replace('I','O',$tmp);
$tmp = str_replace('N','B',$tmp);
if( mb_strlen($tmp)>=2 && eSubstrCount('OB',mb_substr($tmp,1,1))==0 ) eMessage("ERROR: Tipo letra desconocido '{$tmp}'",'HES');
if( mb_strlen($tmp)==3 && eSubstrCount('OB',mb_substr($tmp,2,1))==0 ) eMessage("ERROR: Tipo letra desconocido '{$tmp}'",'HES');
if( mb_strlen($tmp)==3 && mb_substr($tmp,-1)=='B' ) $tmp = str_replace('OB','BO',$tmp);
...
$p = new TCPDF( (($PDF_Horientacion=='V')?'P':'L'), 'pt', 'A4', false, 'UTF-8', false );
...
if( $_PDFLABEL[$n][3]!='' ){
$txt = mb_substr($txt, 0, $_PDFLABEL[$n][3]);
}
...
ePDF_ShowXY($p, mb_substr($txt,0,$Ancho), $x, $y);
}
...
$y = $ay + ($sNEtiPorColumna * $Alto);
$y = $HojaAlto - ($y * $PDF_FontSize * $SaltoLinea) - ($sNEtiPorColumna * $MargenBottom) + $EntreLineas - $OffsetY - ($PDF_FontSize * $SaltoLinea);
ePDF_ShowXY($p, mb_substr(str_repeat('123456789-',20),0,$Ancho), $x, $y);
}
}
$_NumReg++;
$sNEtiPorLinea++;
...
if( mb_strlen($Codigo)> 13 ) die( "Codigo superior a 13 caracteres" );
if( mb_strlen($Codigo)< 12 ) $Codigo = str_pad($Codigo, 12, '0', STR_PAD_LEFT);
if( mb_strlen($Codigo)==12 ) $Codigo = _ean13CheckDigit($Codigo) . $Codigo;
if( mb_strlen($Codigo)<=12 ) $Codigo = str_pad($Codigo, 13, '0', STR_PAD_LEFT);
$nChecksum = 0;
for( $i=0; $i<mb_strlen($Codigo); $i++ ){
if( $i % 2 == 0 ){
...
$prodStr= '';
$encbit = $Codigo[0];
for( $i=1; $i<mb_strlen($Codigo); $i++ ){
$num = (int)$Codigo{$i};
if( $i < 7 ){
$even=(mb_substr($encTable[$encbit],$i-1,1)==1);
if( !$even ){
$mfcStr.=$leftOdd[$num];
}else{
$mfcStr.=$leftEven[$num];
...
for( $i=0; $i<mb_strlen($bars); $i++ ){
$h = $Alto;
$val = mb_strtoupper($bars[$i]);
if( preg_match("/[a-z]/iu",$val) ){
$val = mb_ord($val)-65;
$h = $Alto2;
}
if( $val==1 ){
...
$str = mb_substr($Codigo,0,1);
$Dim[] = array( $space['left'], $Alto, $str );
$str = mb_substr($Codigo,1,6);
$x = $space['left']+$Escala*mb_strlen($Codigo)+$Escala*6;
$Dim[] = array( $x, $Alto2, $str );
$str = mb_substr($Codigo,7,6);
$x = $space['left']+$Escala*mb_strlen($bars)/1.65+$Escala*6;
$Dim[] = array( $x, $Alto2, $str );
...
$csumTotal = 0;
if(mb_strlen($barnumber) <= 12 ) $barnumber = str_pad($barnumber, 13, "0", STR_PAD_LEFT);
for($i=0;$i<mb_strlen($barnumber);$i++) {
if($i % 2 == 0 ){
...
if( $cnf["textAlign"]!="" ) $cnf["textAlign"] = trim(mb_strtoupper($cnf["textAlign"]))[0];
else $cnf["textAlign"] = "";
