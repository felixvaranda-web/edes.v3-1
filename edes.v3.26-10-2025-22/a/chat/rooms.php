...
<?=(($_SESSION["_Development"])? 'S("#ICON_CHATSUBMIT").css("padding-right:30px;");' : "")?>;
...
S.notification(S.clearTags(data.msg));
}
if( data.tools.indexOf("A")>-1 ){
S.sound("notification");
}
...
<?PHP
SS::query("select max(last_msg) from {$_ENV['SYSDB']}gs_chat_room where cdi_close is null");
list($LastMsg) = SS::get("num");
if( $LastMsg=="" ) $LastMsg = "2000-01-01 00:00:00";
SS::query("select cdi_insert, cd_gs_chat_room from {$_ENV['SYSDB']}gs_chat_msg where cd_gs_chat_msg in (select max(cd_gs_chat_msg) from {$_ENV['SYSDB']}gs_chat_msg where cdi_insert>'{$LastMsg}' group by cd_gs_chat_room) order by cdi_insert desc");
while($r=SS::get("num")){
SS::query("update {$_ENV['SYSDB']}gs_chat_room set last_msg='{$r[0]}' where cd_gs_chat_room={$r[1]}");
}
$totalMsgSinLeer = 0;
$_DimRoom = array();
$_DimRoomON = array();
...
(select count(*) from {$_ENV['SYSDB']}gs_chat_msg_user where cd_gs_user=".S::$_User." and cd_gs_chat_room=gs_chat_room.cd_gs_chat_room and cdi_read is null) msg_sin_leer,
(select max(cdi_read) from {$_ENV['SYSDB']}gs_chat_msg_user where cd_gs_user=".S::$_User." and cd_gs_chat_room=gs_chat_room.cd_gs_chat_room) cdi_read
from {$_ENV['SYSDB']}gs_chat_room
where cdi_close is null and (type_room='S' or EXISTS (select cd_gs_user from {$_ENV['SYSDB']}gs_chat_user where cd_gs_user=".S::$_User." and cd_gs_chat_room=gs_chat_room.cd_gs_chat_room and cdi_ini<now() and cdi_end is null))
order by last_msg desc, nm_gs_chat_room"
);
while( $r = SS::get() ){
if( $r["type_room"]=="P" ){
SS::query("select email from {$_ENV['SYSDB']}gs_user where cd_gs_user in (select cd_gs_user from {$_ENV['SYSDB']}gs_chat_user where cd_gs_chat_room=".$r["cd_gs_chat_room"]." and cd_gs_user<>".S::$_User.")", [], 1);
$r["nm_gs_chat_room"] = SS::get("num", 1)[0];
}
...
from {$_ENV['SYSDB']}gs_chat_msg_user D, {$_ENV['SYSDB']}gs_chat_msg M, {$_ENV['SYSDB']}gs_user U, {$_ENV['SYSDB']}gs_chat_room R
where M.cd_gs_chat_msg=D.cd_gs_chat_msg and	U.cd_gs_user=M.cd_gs_user and R.cd_gs_chat_room=M.cd_gs_chat_room and
D.cd_gs_user=".S::$_User."
...
if( $lastDay!=mb_substr($r["cdi_insert"],0,10) ){
$dia = eYmd2Dmy(mb_substr($r["cdi_insert"],0,10));
if( $semanaPasada<$r["cdi_insert"] ){
$d = explode("-", mb_substr($r["cdi_insert"],0,10));
$dia = $diasSemana[date("w", mktime(0,0,0, $d[1],$d[2],$d[0]))];
if( $hoy==$dia ) $dia = "Hoy";
}
echo '<div class="ROOM_DATE" cdi="'.mb_substr($r["cdi_insert"],0,10).'">'.$dia.'</div>';
$userMsg = null;
}
...
$hora = mb_substr($r["cdi_insert"],11,5);
if( $r["msg_type"]=="A" ){
...
$star = (preg_match("/S/u", $r["tools"]))? " MSG_STAR":"";
echo str_replace(
...
$userMsg = $r["user_you"];
$lastDay = mb_substr($r["cdi_insert"],0,10);
}
...
<input name="CHAT_NOTI" id="checkbox" type="checkbox" diferente="" checkbox="1" conimagen="1" size="1" value="" onchange="this.value=(this.checked?S.setup.checkOn:S.setup.checkOff)" class="EDITABLE" ewe="1" onfocus="S.key(&quot;N&quot;,1,0)">
<tr><td><i class="ICONWINDOW" style="cursor:pointer;">&#214;</i><td>Alerta<td>
<input name="CHAT_ALERT" id="checkbox" type="checkbox" diferente="" checkbox="1" conimagen="1" size="1" value="" onchange="this.value=(this.checked?S.setup.checkOn:S.setup.checkOff)" class="EDITABLE" ewe="1" onfocus="S.key(&quot;N&quot;,1,0)">
<tr><td><i class="ICONWINDOW" style="cursor:pointer;">Z</i><td>Mensaje&nbsp;destacado<td>
<input name="CHAT_STAR" id="checkbox" type="checkbox" diferente="" checkbox="1" conimagen="1" size="1" value="" onchange="this.value=(this.checked?S.setup.checkOn:S.setup.checkOff)" class="EDITABLE" ewe="1" onfocus="S.key(&quot;N&quot;,1,0)">
...
<input name="CHAT_DATE" value="" class="EDITABLE FondoSelect" ewe="1" leng="10" maxlength="10" size="10" onfocus="S.key('F4',10,0)" tc="F4" style="width:71px;"><i class="ICONINPUT" onclick="S(&quot;:CHAT_DATE&quot;).calendar()" oncontextmenu="S(&quot;:CHAT_DATE&quot;).calendar(S.lng(365)[0])" id="TOOLSDate">,</i>
