...
SS::query("select * from {$_ENV['SYSDB']}gs_chat_file where cd_gs_chat_file=".($pk*1));
$r = SS::get();
if( $r["md5"]==$md5 ){
...
SS::query("insert into {$_ENV['SYSDB']}gs_chat_file
(  cd_gs_chat_room ,       cd_gs_user     ,         cdi_create      ,   file  ,   md5  ) values
(".$_POST["room"].",".S::$_User.",'".date("Y-m-d H:i:s")."','{$file}','{$md5}')
");
$pk = SS::id();
