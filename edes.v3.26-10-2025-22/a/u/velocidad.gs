...
$Dim[$n] = trim($Dim[$n]);
if( mb_substr($Dim[$n],0,5) == '$_Sql' ){
$tmp = explode( '=', $Dim[$n] );
$tmp[0] = trim($tmp[0]);
$Dim[$n] = $tmp[0];
$tmp[0] = mb_substr($tmp[0],1);
$Dim[$n] .= "='".${$tmp[0]}."';";
}
if( $Dim[$n]!='' ) $txt .= $Dim[$n]."\n";
}
...
$tmp[1] = trim($tmp[1]);
while( eSubstrCount($tmp[1],'  ') > 1 ) $tmp[1] = str_replace('  ',' ',$tmp[1]);
switch( $tmp[0] ){
case 'Total':
if( '1 1 1' == $tmp[1] ){
