<?PHP
if( S::$_User==-1 || $_SESSION["_D_"]=="" || $_gsID!=getmypid() ) exit;
$_DirG = 'g/e'; include_once(DIREDES.'t/lp.gs' );
...
"\t".'"Tools"=>'.($tmp[6]*1)."\n".
');'."\n".
'$_gsNomUser = $_SESSION["_gsACCESO"]["EMAIL"];'."\n".
"?".">";
SS::query("select cd_gs_user from {$_ENV['SYSDB']}gs_user where email='{$tmp[5]}'");
list($pk) = SS::get("num");
if( $pk>0 ){
$DimUser[] = $pk;
$file = "../_d_/".$pk.'.dvl';
...
if( $file!='.' && $file!='..' ){
if( !is_dir("{$DirBase}/{$file}") && (mb_substr($file,-4)==".dvl" || mb_substr($file,-8)==".dvl.bak") ){
...
if( eSubstrCount($txt, mb_chr(9).$_SESSION["_UserEMail"].mb_chr(9))==0 ) ePrintR("");
return $txt;
}
...
'<FORM accept-charset="utf-8" METHOD=POST ACTION="edes.php?E:$a/u/_usuarios.gs<?=eSessionAddUrl()?>">';
...
echo '</TBODY></TABLE><BR>';
?>
<table onclick=Alta()	class="AddButton" border=0px cellspacing=0px cellpadding=0px style="display:inline-table;"><tr><td><i class="ICONINPUT ICONINSERT">I</i><td>&nbsp;Alta</table>&nbsp;
<table onclick=Baja()	class="AddButton" border=0px cellspacing=0px cellpadding=0px style="display:inline-table;"><tr><td><i class="ICONINPUT ICONDELETE">D</i><td>&nbsp;Borrar</table>&nbsp;&nbsp;&nbsp;
<table onclick=Grabar()	class="AddButton" border=0px cellspacing=0px cellpadding=0px style="display:inline-table;"><tr><td><i class="ICONINPUT ICONUPDATE">U</i><td>&nbsp;Grabar</table>
<?PHP
if( $_SESSION["_D_"]=="~" ){
?>
<table onclick=GenerarPermisos() class="AddButton AddButton2" border=0px cellspacing=0px cellpadding=0px style="display:inline-table;"><tr><td><i class="ICONINPUT ICONSEEK">&#241;</i><td>&nbsp;Generar Permisos</table>
<?PHP
}
echo '</CENTER>';
?>
...
$NewUSU = str_replace("&#13;&#10;", "\n", $NewUSU);
copy('../_d_/cfg/e'.'d.l'.'p', '../_d_/cfg/e'.'d.l'.'p.bak');
$DimFila  = explode("\n", trim(Leer_LP()));
