<?PHP
eLngLoad(DIREDES.'lng/_http.gs.lng');
...
fclose( $df );
if( mb_substr($cTxt,0,9)=='[Titulo]'."\t" ) return($cTxt);
...
$Basura += $LongDeLong + 2;
$txt = substr($cTxt, $Basura, (int)$LenCadena);
...
'<FORM accept-charset="utf-8" METHOD=POST ACTION="edes.php?E:$a/u/_http.gs">';
...
TR.style.height = S(".BROWSE TH").height()+"px";
...
<?PHP
echo '<br><bt><center>';
echo eLng('Código de actualización').': <b onclick="uCapturar()" id=IDSRV style="cursor:var(--cPointer)">'._IDSRV().'</b>';
echo '</center>';
echo '</BODY></HTML>';
eEnd();
}
