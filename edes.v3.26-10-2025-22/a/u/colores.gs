...
for( $j=0; $j<mb_strlen($linea); $j++ ){
$ValCar = mb_substr( $linea, $j, 1 );
if( $ValCar == ":" ){
...
if( mb_substr($linea,$j,1) == "#" ){
$buffer = mb_strtoupper(mb_substr($linea,$j,7));
$lotenemos = 0;
for( $k=0; $k<count($func); $k++ ){
if( $func[$k][0] == $buffer ){
$func[$k][1].= "\n".mb_substr($NomFile,4,mb_strlen($NomFile)-8)."\t\t".$title;
$lotenemos = 1;
}
}
if( $lotenemos == 0 ){
...
$func[$reg][1] = mb_substr($NomFile,4,mb_strlen($NomFile)-8)."\t\t".$title;
}
}
}
$buffer = "";
...
if( $ValCar == '{' )
$title = trim(mb_substr($buffer,0,mb_strlen($buffer)-1));
}
}
}
fclose($File);
...
$NewLinea = str_replace( mb_strtolower($c2), $c1, $NewLinea );
