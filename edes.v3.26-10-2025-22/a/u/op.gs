...
file_put_contents($File, S::$_User.','.(((int)eGetMicroTime())+120).','.$_SESSION["_UserName"].','.$_SESSION["_UserEMail"]);
}
if( SS::count("{$_ENV['SYSDB']}gs_op", '')==0 ){
...
if( trim($Dim[$n])!='' ){
$tmp = explode('~',mb_substr($Dim[$n],4));
SS::query("update {$_ENV['SYSDB']}gs_op set show_type='{$tmp[2]}' where cd_gs_op='{$tmp[1]}'");
}
}
@unlink($File);
}
...
$Dim = array();
$nMax = 0;
SS::query("select cd_gs_user,email,count(*) from {$_ENV['SYSDB']}gs_activity where script='{$Buscar}' group by 1,2 order by 3 desc");
while( $r=SS::get("num") ){
...
sort($Dim);
$txt = '<pre style="font-family:monospace;">';
...
?>
<script type="text/javascript">
top.eInfo(window, '<?= 'SCRIPT: '.$Buscar.$txt ?>', 99);
</script>
<?PHP
eEnd();
}
...
$Test = false;
$_POST['grabar'] = str_replace(
array( '&#13;',  '&#10;'),
array(CHR13 , CHR10 ),
$_POST['grabar']
);
$MaxId = 1;
$nReg = SS::count("{$_ENV['SYSDB']}gs_op");
if( $nReg>0 ){
SS::query("select max(cd_gs_op) from {$_ENV['SYSDB']}gs_op order by 1 desc");
list($MaxId) = SS::get("num");
...
$txt .= ",'{$Hoy}'";
$txt .= ",'','{$Hoy}'";
$sql = "insert into {$_ENV['SYSDB']}gs_op (seq,cd_gs_op,show_type,mode,icon,indent,type,caption,script_url,tip,icons,alias, dt_add, status,dt_status) values ({$txt})";
...
$NumDelete++;
$xID = $xID * -1;
$sql = "delete from {$_ENV['SYSDB']}gs_op where cd_gs_op={$xID}";
SS::query("delete from {$_ENV['SYSDB']}gs_tree_op where cd_gs_op={$xID}");
}else{
$sql = "update {$_ENV['SYSDB']}gs_op set seq='{$Seq}', show_type='{$tmp[1]}', mode='{$tmp[2]}', icon='{$tmp[3]}', indent='{$tmp[4]}', type='{$tmp[5]}',
caption='{$tmp[6]}',
script_url='{$tmp[7]}',
tip='{$tmp[8]}',
icons='{$tmp[9]}',
...
SS::query("update {$_ENV['SYSDB']}gs_op set show_type='' where type<>'O' and show_type<>''");
if( $Test ){
echo '</table>';
echo $_POST['grabar'];
}else{
...
if( ($uFile=='.') || ($uFile=='..') || (mb_substr($uFile,0,6)!='gs_op.') || (is_dir( $dir.'/'.$uFile )) ) continue;
$file = gmdate("YmdHis", filemtime($dir .'/'. $uFile));
$DimFile[] = $file.'|'.$uFile;
}
closedir($uDir);
$ArbolTxt = '';
SS::query("select * from {$_ENV['SYSDB']}gs_op order by seq");
while( $r=SS::get("num") ){
...
SS::query("insert into {$_ENV['SYSDB']}gs_activity (cd_gs_user, cdi, script, edes, email) values ({$_User}, '". date('Y-m-d H:i:s')."', '/tree/gs_op.unl', 'C', '".$_SESSION["_UserEMail"]."')");
}
?>
<script type="text/javascript">
...
SS::query("update {$_ENV['SYSDB']}gs_tree set permission='".(($_GET['TreeOnOff']==$_ENV['ON']) ? $_ENV['ON'] : $_ENV['OFF'])."' where cd_gs_tree=".$_GET['TreeCd']);
?>
<script type="text/javascript">
...
$_POST['NmTree'] = str_replace( "'", '&#39;', $_POST['NmTree'] );
SS::query("insert into {$_ENV['SYSDB']}gs_tree (nm_gs_tree, filename) values ('".$_POST['NmTree']."', '')");
$id = SS::id();
?>
<script type="text/javascript">
...
$txt = '';
SS::query("select O.seq, T.cd_gs_op from {$_ENV['SYSDB']}gs_tree_op T left join {$_ENV['SYSDB']}gs_op O on T.cd_gs_op=O.cd_gs_op where T.cd_gs_tree=".$_POST['LoadTree'].' order by O.seq desc');
while( $r=SS::get("num") ){
...
<?PHP
if( $_POST['IgualQue']>0 ){
SS::query("insert into {$_ENV['SYSDB']}gs_tree_op (select {$id}, cd_gs_op from {$_ENV['SYSDB']}gs_tree_op where cd_gs_tree={$_POST['IgualQue']})");
}
eEnd();
}
...
$uDir = opendir( $dir );
while( $uFile = readdir($uDir) ){
if( ($uFile=='.') || ($uFile=='..') || (mb_substr($uFile,0,11)!='gs_tree_op.') || (is_dir($dir.'/'.$uFile)) ) continue;
$file = gmdate("YmdHis", filemtime($dir .'/'. $uFile));
$DimFile[] = $file.'|'.$uFile;
}
closedir($uDir);
...
if( $Test ){
eTrace('Tree: '.$Dim[0]);
eTrace("update {$_ENV['SYSDB']}gs_tree set nm_gs_tree='".$NomTree."' where cd_gs_tree=".$Tree);
echo '<table border=1>';
}else{
_Log('Comprueba que el nombre no se duplique');
if( SS::count("{$_ENV['SYSDB']}gs_tree", "nm_gs_tree='".$NomTree."' and cd_gs_tree<>".$Tree)>0 ){
SS::query("select nm_gs_tree from {$_ENV['SYSDB']}gs_tree where cd_gs_tree=".$Tree);
list($NomTree) = SS::get("num");
?>
<script type="text/javascript">
...
SS::query("update {$_ENV['SYSDB']}gs_tree set nm_gs_tree='".$NomTree."', cdi='".date('Y-m-d H:i:s')."' where cd_gs_tree=".$Tree);
...
SS::query("delete from {$_ENV['SYSDB']}gs_tree_op where cd_gs_tree={$Tree}");
}
...
if( trim($Dim[$n])!='' ) SS::query("insert into {$_ENV['SYSDB']}gs_tree_op values ({$Tree}, {$Dim[$n]})");
}
$TReg--;
M($TReg);
$Contenido = ob_get_contents();
...
$DimIdiomas = array();
SS::query("select cd_gs_language from {$_ENV['SYSDB']}gs_language");
while( $r = SS::get("num") ){
$DimIdiomas[] = trim($r[0]);
}
...
SS::query("select filename from {$_ENV['SYSDB']}gs_tree where cd_gs_tree='{$CdTree}'", [], 1);
list($FileName) = SS::get("num", 1);
...
$DimLabel = array();
SS::query("select cd_gs_op,caption,caption_tip from {$_ENV['SYSDB']}gs_op_lng where cd_gs_language='{$Lenguaje}' order by cd_gs_op");
while( $r=SS::get("num") ) $DimLabel[trim($r[2]).$r[0]] = Decode(trim($r[1]));
$txt = '';
SS::query("select * from {$_ENV['SYSDB']}gs_op where cd_gs_op in (select cd_gs_op from {$_ENV['SYSDB']}gs_tree_op where cd_gs_tree={$CdTree}) order by seq");
while( $r = SS::get() ){
...
closedir($di);
rsort($t);
echo '<table class="col_0ff" id=RestoreTable onclick="RestoreDatos()"><col style="cursor:var(--cPointer)"><tr><th style="cursor:var(--cPointer);font-size:14px">FECHA Backup';
for($n=0; $n<count($t); $n++) echo '<tr><td n='.mb_substr($t[$n],-2).'>'.mb_substr($t[$n],0,19);
echo '</table>';
?>
<script type="text/javascript">
top.S.init(window);
...
$Test = false;
$DimAdd = array();
SS::query("select cd_gs_op, dt_add from {$_ENV['SYSDB']}gs_op");
while( $r=SS::get("num") ) $DimAdd[$r[0]] = $r[1];
...
SS::query("delete from {$_ENV['SYSDB']}gs_op");
}
...
while( $file=readdir($di) ){
if( $file!='.' && $file!='..' && !is_dir($file) ){
if( mb_substr($file,0,mb_strlen($NomFile))==$NomFile ){
$t[] =date('Y-m-d H:i:s', filemtime($Dir.$file)) .'|'. $sDir.$file;
}
}
}
closedir($di);
rsort($t);
echo '<table id=RestoreTable onclick="RestoreDatosTree()"><col style="cursor:var(--cPointer)"><tr><th style="cursor:var(--cPointer);font-size:14px">FECHA Backup';
for( $n=0; $n<count($t); $n++ ) echo '<tr><td n='.mb_substr($t[$n],-2).'>'.mb_substr($t[$n],0,19);
echo '</table>';
?>
<script type="text/javascript">
...
eTrace("delete from {$_ENV['SYSDB']}gs_tree_op where cd_gs_tree={$NArbol}");
echo '<table border=1>';
}else{
SS::query("delete from {$_ENV['SYSDB']}gs_tree_op where cd_gs_tree={$NArbol}");
}
for( $n=2; $n<count($Dim); $n++ ){
if( $Test ){
echo '<tr><td>'.$Dim[0].'<td>'.$Dim[$n];
echo '<td>'."insert into {$_ENV['SYSDB']}gs_tree_op values({$NArbol}, {$Dim[$n]})";
}else{
SS::query("insert into {$_ENV['SYSDB']}gs_tree_op values({$NArbol}, {$Dim[$n]})");
}
}
if( $Test ){
echo '</table>';
...
SS::query("select distinct(cd_gs_tree) from {$_ENV['SYSDB']}gs_tree where cd_gs_tree in (select cd_gs_tree from {$_ENV['SYSDB']}gs_tree_op where cd_gs_op={$Tree})");
$xTree = '';
while( $r=SS::get("num") ) $xTree .= ','.$r[0];
$xTree .= ',';
?>
<script type="text/javascript">
...
$Opciones = mb_substr($Opciones,1);
if( $Arboles!='' ) $Arboles = trim(mb_substr($Arboles,1));
$Opciones = explode(',', $Opciones);
$UnaOpcion  = $Opciones[0];
$DimArbols = array();
SS::query("select cd_gs_tree from {$_ENV['SYSDB']}gs_tree_op where cd_gs_op in ({$UnaOpcion})");
while( $r=SS::get("num") ) $DimArbols[$r[0]*1] = 1;
SS::query("delete from {$_ENV['SYSDB']}gs_tree_op where cd_gs_op in ({$UnaOpcion})");
if( $Arboles!='' ){
$DimIdiomas = array();
SS::query("select cd_gs_language from {$_ENV['SYSDB']}gs_language");
while( $r=SS::get("num") ) $DimIdiomas[] = trim($r[0]);
if( count($DimIdiomas)==0 ) $DimIdiomas[] = 'es';
...
$Opcion = $Opciones[$o];
if( SS::count("{$_ENV['SYSDB']}gs_tree_op", "cd_gs_tree={$Arbol} and cd_gs_op={$Opcion}")==0 ){
SS::query("insert into {$_ENV['SYSDB']}gs_tree_op (cd_gs_tree,cd_gs_op) values ({$Arbol},{$Opcion})");
}
}
$DimArbols[$Arbol*1] = 1;
}
...
SS::query("select cd_gs_op from {$_ENV['SYSDB']}gs_op where cd_gs_op not in (select cd_gs_op from {$_ENV['SYSDB']}gs_tree_op)");
$Op = '';
while( $r=SS::get("num") ) $Op .= ','.$r[0];
if( $Op!='' ) $Op .= ',';
?>
<script type="text/javascript">
...
eHTML();
eLngLoad(DIREDES.'lng/usertree.zdf');
?>
...
$_DimUser = array();
if( $xDim!='' ) $_DimUser = unserialize(gzuncompress($xDim));
$Dim = array();
...
var _TxtModeView = '<?PHP  foreach($_ModeLabel as $k=>$v) if( eSubstrCount("IDVU",$k)>0 ) echo $v[0]; ?>';
...
foreach( $_ShowLabel as $k=>$v ){
if( $n>0 ) echo ',';
if( eSubstrCount($v,'_')>0 ) $v = mb_substr($v,mb_strpos($v,'_')+1,1);
echo "'{$k}':'".$v[0]."'";
$n++;
}
?>};
...
var _QuienLoDesarrollo = false;
var _XY = new Array();
var _Icono;
var _SoloConsulta = false;
<?PHP
if( !$_SESSION["_Development"] || $_GET['_GEST']!="ALL" ){
echo '_EditandoArbol = true;';
}
...
fullscreen:true,
iconAdd:
"svg<i  id='__svg'  class='ICONWINDOW' eON=1 style='cursor:var(--cPointer);margin-right:50px;'>&#321;</i>"+
"img<i  id='__img'  class='ICONWINDOW' eON=1 style='cursor:var(--cPointer);margin-right:10px;'>&#321;</i>"+
"jpeg<i id='__jpeg' class='ICONWINDOW' eON=1 style='cursor:var(--cPointer);margin-right:10px;'>&#321;</i>"+
"jpg<i  id='__jpg'  class='ICONWINDOW' eON=1 style='cursor:var(--cPointer);margin-right:10px;'>&#321;</i>"+
"png<i  id='__png'  class='ICONWINDOW' eON=1 style='cursor:var(--cPointer);margin-right:10px;'>&#321;</i>"+
"gif<i  id='__gif'  class='ICONWINDOW' eON=1 style='cursor:var(--cPointer);margin-right:50px;'>&#321;</i>"
});
<?PHP } ?>
return eClearEvent();
}
_OpEnTreeON = false;
if( el.cellIndex==_LABEL && _QuienLoDesarrollo ){
...
function addHelpMode(o){
var oTD = S.event(top), value, cellIndex;
if( oTD.tagName!="TD" ) return;
cellIndex = oTD.cellIndex;
value = S.trim(oTD.innerText.split("/")[0])
o = S.toClass(S("#"+o.id, top), "CAJA");
if( cellIndex>1 ) value+=":";
S("INPUT", o).dim[5].value += value;
}
function helpModes(){
return `<table style="zoom:0.9" id="alertHelp" class="BROWSE col_0c col_1c col_2c col_3c col_4c" onclick="S.session.opener.addHelpMode(this)">
<tr><th>Directiva<th>Objeto<th colspan=2>Modo<th>SubModo</tr>
<tr><td>?<td>F / #<td>a<td><td>gl</tr>
<tr><td>?2<td>G / @<td>b<td>bR<td>bl</tr>
<tr><td>w<td>L / =<td>c<td>cR<td>cl</tr>
<tr><td>W<td>E / ><td>m<td>mR<td>ml</tr>
<tr><td><td>R<td>l<td><td></tr>
<tr><td><td>D<td>s<td><td></tr>
<tr><td><td>^<td>q<td><td></tr>
<tr><td><td>H<td><td><td></tr>
<tr><td><td>V<td><td><td></tr>
</table>`;
}
...
function _opSave(){
var txt='', oID=aID=nErrores=TReg=anterior=0, v, Borradas=new Array(), oCaption='', LineaErronea=-1, xError="";
for(v=1; v<_FILAS.length-1; v++){
if( _FILAS[v].getAttribute("dlt")!=null && _FILAS[v].getAttribute("dlt")==-1 ){
continue;
}else{
if( _FILAS[v].getAttribute("dlt")!=null ){
if( _FILAS[v].getAttribute("dlt")==1 && _FILAS[v].getAttribute("n")>0 ){
_FILAS[v].setAttribute("n", parseInt(_FILAS[v].getAttribute("n"))*-1);
}
}else{
aID = parseInt(_FILAS[v].cells[_LABEL].id.substr(1));
if( v>1 && anterior>0 ){
if( _FILAS[v].cells[_LABEL].getAttribute("tp")!='F' && _FILAS[anterior].cells[_LABEL].getAttribute("tp")!='F' ){
if( parseInt(_FILAS[v].cells[_LABEL].id.substr(1)) > parseInt(_FILAS[anterior].cells[_LABEL].id.substr(1)) ){
if( LineaErronea==-1 ){
LineaErronea = v;
...
if( _FILAS[anterior].cells[_LABEL].getAttribute("tp")=="O" && _FILAS[anterior].cells[_LABEL].id.substr(1)<_FILAS[v].cells[_LABEL].id.substr(1) ){
if( LineaErronea==-1 ){
LineaErronea = v;
xError = 'El padre de la opción no puede ser otra opción:<br>Nivel "'+_FILAS[anterior].cells[_LABEL].id.substr(1)+'" de la opción <b>"'+_FILAS[anterior].cells[_LABEL].textContent+'"</b><br>Nivel "'+_FILAS[v].cells[_LABEL].id.substr(1)+'" de la opción <b><span style="color:red">"'+_FILAS[v].cells[_LABEL].textContent+'"</span></b>';
}
nErrores++;
_FILAS[v].cells[4].style.backgroundColor = 'rgba(252, 0, 0, 0.15)';
_FILAS[v].style.display = 'table-row';
...
if( _FILAS[v].getAttribute("dlt")==null ) anterior = v;
...
sHtm += "<FORM accept-charset='utf-8' METHOD='POST' ACTION='edes.php?E:$a/u/op.gs<?=eSessionAddUrl()?>' target=TLF>";
...
<?PHP
if( $_SESSION["_Development"] ) echo '_EditandoArbol = false;';
?>
_TreeON = false;
_NTree = null;
...
top.eCallSrv( window, 'edes.php?E:$a/u/op.gs&TreeCd='+Obj.parentNode.getAttribute("cd")+'&TreeOnOff='+((Obj.textContent.replace(/\s/g,'')==_ChrON) ? S.setup.checkOn : S.setup.checkOff) );
}
}
...
var sHtm = "<?=eHTML('','','',true)?></HEAD><BODY>";
sHtm += "<FORM accept-charset='utf-8' METHOD='POST' ACTION='edes.php?E:$a/u/op.gs"+((_SoloConsulta)? '&_SoloConsulta=1':'')+"<?=eSessionAddUrl()?>' target=TLF>";
...
<col style='width:10px;font-family:<?= $_FontFamily; ?>;cursor:var(--cPointer);'>
<col style='width:15px;text-align:center;cursor:var(--cPointer);'>
<col style='cursor:var(--cPointer)'>
<col style='width:10px;text-align:center;cursor:var(--cPointer);'>
<col style='width:10px;text-align:center;cursor:var(--cPointer);'>
<?PHP  if( $_SESSION["_D_"]!='' ){ ?>
<col style='width:10px;text-align:center;cursor:var(--cPointer);'>
<?PHP  }else{ ?>
...
<?PHP
$_DimIcon = array();
SS::query("select * from {$_ENV['SYSDB']}gs_op_ico where global<>'{$_ENV['ON']}' or global is null order by position");
while( $r = SS::get() ){
...
if( $r['mode']!='' ){
echo '<td id="mc'.$r['mode'].'">'.$_ModeOp[$r['mode']];
}else{
echo '<td>&nbsp;';
}
echo '<td>&nbsp;';
...
?>
<tr><td colspan=6 align=center style='cursor:var(--cPointer);text-align:center'>
...
<tr><td valign=top>
<table id=BROWSETH pk=3 width=100% st-yle="border-collapse:collapse"><TR>
<TH class=THOption align=left style="padding-left:10px;cursor:var(--cPointer);" nowrap>&nbsp;&nbsp;&nbsp;GESTOR DE <?= (($_GET['_GEST']=="ALL")?'ÁRBOLES':'OPCIONES')?>
<TH class=THOption width=1px style='cursor:var(--cPointer);'>&nbsp;&nbsp;
<?PHP if( $_GET['_GEST']=="ALL" ){ ?>
...
<TH class=THOption width=50px style='cursor:var(--cPointer);'>&nbsp;&nbsp;&nbsp;
<TH class=THOption style='font-weight:normal; width:1px;cursor:var(--cPointer);'>Buscar&nbsp;
...
<TH class=THOption width=1px><input name="ENCONTRADOS" SIZE=3 title="Nº de ocurrencias" style="cursor:var(--cPointer);width:25px" disabled>
...
<?PHP } ?>
<?PHP if( $_WithDevelopment && eSubstrCount('~AMPDH',$_SESSION["_D_"]) == 1 && $_GET['_GEST']=="ALL" ){ ?>
<TH class=THOption style='cursor:var(--cPointer);'><span style='width:30px'></span>
<?PHP  if( $_SESSION["_Development"] && $_GET['_GEST']=="ALL" ){ ?>
<TH class=THOption width=1px style='cursor:var(--cPointer);font-weight:normal;' title='Ejecutar Opción'>&nbsp;&nbsp;&nbsp;RunOp
<TH class=THOption width=1px title='Ejecutar Opción'><img src=g/check_0.gif id=OpEnSubVentana valor=0 onclick=_CambiaTF()>
<?PHP  } ?>
<TH class=THOption width=1px style='cursor:var(--cPointer);font-weight:normal;' title='Desarrollador'><i src=g/mapa_user.gif onclick=_Desarrollador() style="margin-left:10px;">q</i>
<TH class=THOption width=1px style='cursor:var(--cPointer);'>&nbsp;
<TH class=THOption width=1px style='cursor:var(--cPointer);font-weight:normal;display:<?=(($_WithEdit)?'block':'none')?>'>gsEdit <TH class=THOption width=1px style='display:<?=(($_WithEdit)?'block':'none')?>'><img src=g/check_0.gif id=OpGsEdit valor=0 onclick=_CambiaTF()>
<TH class=THOption width=1px style='cursor:var(--cPointer);font-weight:normal;display:<?=(($_WithStatus)?'block':'none')?>'>Status <TH class=THOption width=1px style='display:<?=(($_WithStatus)?'block':'none')?>'><img src=g/check_<?= (($_StatusON)?1:0)?>.gif id=VerStatus valor=1 onclick=_CambiaTF()>
<?PHP  } ?>
<TH class=THOption width=100% style='cursor:var(--cPointer);'>&nbsp;
</table>
...
<col style='width:15px;text-align:center;cursor:var(--cPointer)<?=(($_GET['_GEST']=="VIEW")?';display:none':'')?>'>						<?PHP  // Visibilidad ?>
<col style='width:15px;text-align:center;cursor:var(--cPointer)'>																		<?PHP  // Mode ?>
<col style='width:15px;text-align:center'>																						<?PHP  // Icon ?>
<col style='width:15px;text-align:center;font-family:<?= $_FontFamily; ?>;cursor:var(--cPointer);display:none;'>						<?PHP  // Sel ?>
<col class=Option>
...
<thead>
<TR>
<TH style='font-size:10px;cursor:var(--cPointer);<?=(($_GET['_GEST']=="VIEW")?';display:none':'')?>'>Show
<TH style='font-size:10px;cursor:var(--cPointer);'>Mode
<TH style='font-size:10px;cursor:var(--cPointer);'>Icon
<TH style='font-size:10px;cursor:var(--cPointer);'>Sel
<TH style='cursor:var(--cPointer);'>Caption
<TH style='cursor:var(--cPointer);'>Icons
<TH style='font-size:10px;cursor:var(--cPointer);<?=(($_StatusON==true)?'':'display:none;')?>'>Status
<TH style='font-size:10px;cursor:var(--cPointer);<?=(($_StatusON==true)?'':'display:none;')?>'>Fecha
<TH style='font-size:10px;cursor:var(--cPointer);<?=(($_StatusON==true)?'':'display:none;')?>'>Usuario
<TH style='font-size:10px;cursor:var(--cPointer);width:120px;<?=(($_GET['_GEST']=="ALL")?'':'display:none;')?>'>Alias
<TH style='font-size:10px;cursor:var(--cPointer);<?= (($_WithDoc  ==true)?'':'display:none;')?>'>Docs
</thead>
<tbody>
<?PHP
if( $_GET['TREE']==1 && !$_SESSION["_Development"] ){
SS::query("select * from {$_ENV['SYSDB']}gs_op where show_type<>'U' and show_type<>'D' order by seq");
}else{
SS::query("select * from {$_ENV['SYSDB']}gs_op order by seq");
}
$c = 0;
...
echo " HR=''><img src=g/linea.gif width=30px height=2px>".trim(mb_substr($txt,0))."<img src=g/linea.gif width=30px height=2px>";
}
}else{
...
echo "<td{$view}>";
$dt = $r['dt_status'];
if( mb_substr($dt,2,1)!='' && mb_substr($dt,2,1)!='-' ) $dt = mb_substr($dt,8,2).mb_substr($dt,4,4).mb_substr($dt,0,4);
if( isZero($dt) ) $dt = '&nbsp;';
echo $dt;
if( $r['cd_gs_user'] > 0 ){
...
<table id=SMenuIcons pk=4 width=1px oncontextmenu='return eClearEvent()' style='position:absolute;display:none;border-collapse:collapse;' onmouseleave='this.style.display="none"' onclick='this.style.display="none"'>
<TR>
<?PHP  	?>
...
<table id=TreeGroup pk=5 height=1px onclick="ShowOpTree()" oncontextmenu='return eClearEvent()'>
<col style='width:15px;text-align:center;font-family:<?= $_FontFamily; ?>;cursor:var(--cPointer)'>
<col style='width:15px;text-align:left;cursor:var(--cPointer)'>
<col style='width:15px;text-align:center;font-family:<?= $_FontFamily; ?>;cursor:var(--cPointer)'>
<tr>
<th class=THOption style='font-size:10px;cursor:var(--cPointer)'>Sel
<th>
...
<th class=THOption style='font-size:10px;cursor:var(--cPointer)'>On
<?PHP
SS::query("select * from {$_ENV['SYSDB']}gs_tree order by nm_gs_tree");
while( $r = SS::get() ){
echo '<tr cd='.$r['cd_gs_tree'].'><td>'.mb_chr($_ChrOFF);
$r['nm_gs_tree'] = str_replace('-','·',$r['nm_gs_tree']);
echo '<td nowrap>'.str_replace(' ','&nbsp;',$r['nm_gs_tree']);
echo '<td>'.(($r['permission']=='S')? mb_chr($_ChrON) : mb_chr($_ChrOFF));
}
?>
</table>
</div>
...
<col style='width:15px;text-align:center;font-family:<?= $_FontFamily; ?>;cursor:var(--cPointer)'>
<tr>
<th class=THOption style='font-size:10px;cursor:var(--cPointer);border-bottom-width:0px'>Sel
<th>
<table border=0px cellspacing=1px cellpadding=0px height=1px>
...
<th class=THOption style='font-size:10px;cursor:var(--cPointer);border-bottom-width:0px'>On
<?PHP
SS::query("select * from {$_ENV['SYSDB']}gs_tree order by nm_gs_tree");
while( $r = SS::get() ){
echo '<tr cd='.$r['cd_gs_tree'].'><td style="background:#ffffff">'.mb_chr($_ChrOFF);
$r['nm_gs_tree'] = str_replace('-','·',$r['nm_gs_tree']);
echo '<td nowrap style="background:#ffffff">'.str_replace(' ','&nbsp;',$r['nm_gs_tree']);
echo '<td style="background:#ffffff">'.(($r['permission']=='S')? mb_chr($_ChrON) : mb_chr($_ChrOFF));
}
?>
</table>
</div>
...
SS::query("select cd_gs_op,caption,caption_tip from {$_ENV['SYSDB']}gs_op_lng where cd_gs_language='{$Lenguaje}' order by cd_gs_op");
while( $r = SS::get("num") ){
$DimLabel[trim($r[2]).$r[0]] = Decode(trim($r[1]));
}
$txt = '';
SS::query("select * from {$_ENV['SYSDB']}gs_op where cd_gs_op in (select cd_gs_op from {$_ENV['SYSDB']}gs_tree_op where cd_gs_tree={$CdTree}) order by seq");
while( $r = SS::get() ){
...
$long = mb_strlen( $buff ) - 1;
$tmp = ltrim($buff);
$snivel = eSubstrCount(mb_substr( $buff, 0, mb_strpos($buff,$tmp[0])), mb_chr(9) );
for( $i=0; !strcmp(mb_chr(9), mb_substr($buff,$i,1)); $i++ );
$sType = 'O';
if( $long > $i && $long > 0 ){
if( $i > $nactual ){
$nactual = $i;
...
$narbol = trim($narbol);
$xIcon = trim(mb_substr($xIcon,1));
}else if( $narbol[0]=='[' ){
list($tmp) = explode(']',$narbol);
$tmp = mb_strtoupper($tmp);
if( eSubstrCount($tmp,'.GIF')==1 || eSubstrCount($tmp,'.JPG')==1 || eSubstrCount($tmp,'.PNG')==1 ){
list($xIcon,$narbol) = explode(']',$narbol);
$narbol = trim($narbol);
$xIcon = trim(mb_substr($xIcon,1));
}
...
if( mb_strlen($Accion)<4 ){
switch( mb_strtoupper(mb_substr($Accion,1,1)) ){
case 'C': case 'L':
$xMode = 'V';
break;
case 'B':
...
}else if( mb_substr($Accion,0,2)=='>$' || mb_substr($Accion,0,3)=='m>$' || mb_substr($Accion,0,16)=='window.external.' ) $xMode = 'S';
$Hoy = date('Y-m-d');
$q = "insert into {$_ENV['SYSDB']}gs_op (cd_gs_op, mode, seq, caption, indent, type, script_url, dt_add, icon, status, dt_status) values ({$prox}, '{$xMode}', {$carbol}, '{$narbol}', {$niv}, '{$sType}', '{$opcion}', '{$xFecha}', '{$xIcon}', '', '{$Hoy}')";
