...
$post = array(
'TO_UPDATE' => 'test',
'IDSRV' => $_SESSION["tmp"]['_ServerTarget'][$TO]
);
}else{
$post = array(
'TO_UPDATE' => 'test',
...
$tmp = explode("|", $result);
if( $tmp[0]!="Test OK" ){
eMessage("Error de conexión<br><br>Servidor: {$NomServer}<br>URL: {$target_url}<br>Error: ".$result."<span style='display:none'>".$tmp[1]."</span>", "HSE");
}else{
