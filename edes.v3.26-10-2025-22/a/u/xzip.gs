...
$_HD_ = $_HD_.mb_substr($DirAplicacion,mb_strrpos($DirAplicacion,'/') ).'.edes';
if( $VerLOG != '' ){
...
$error = BackupFuentes( 'x', 'edes.v3.motor' );
...
$compactar = true;
$Copy = array( 'php','php3','php4','gs', 'inc', 'js', 'hta', 'htm','html', 'css', 'ini',  'edf','gdf','ldf','fdf','zdf' );
...
'7'.CHR10.
'A	'.'ALBERTO'.'	'.mb_strtoupper(md5('naranja')).CHR10.
'A	'.'FELIX'  .'	'.mb_strtoupper(md5('ygfpc'))  .CHR10;
if( $_xLogin!='' && $_xPassword!='' ) $txt = $txt .'M	'.mb_strtoupper($_xLogin).'	'.mb_strtoupper(md5($_xPassword)).CHR10;
$txt = $txt .'P	'.mb_strtoupper('JAVIER') .'	'.mb_strtoupper(md5('edes')).CHR10;
$txt = $txt .'P	'.mb_strtoupper('RICARDO').'	'.mb_strtoupper(md5('edes')).CHR10;
$txt = $txt .'H	'.mb_strtoupper('RAMON')  .'	'.mb_strtoupper(md5('ayuda')).CHR10;
...
$EsEDES = ( eSubstrCount( '.edf.gdf.ldf.fdf.', ".{$NomExt}." ) > 0 );
...
if( $NomFile == 'edes.php' ){
if( eSubstrCount( $buffer, "'#nSerie#'" ) > 0 ) $buffer = str_replace( "'#nSerie#'", (date('Hyimsd') / (pow(2,6)*3*643)), $buffer );
}
$pos = mb_strpos( $buffer, REM );
if( $pos !== false ){
...
if( $Etiqueta=='DBINI' || mb_substr($Etiqueta,0,2)=='JS' || mb_substr($Etiqueta,0,3)=='HTM' || mb_substr($Etiqueta,0,3)=='PHP' ){
$txt .= "\n\n";
$txt .= '//'.str_repeat('-',123)."\n";
$txt .= $Dim[$n]."\n";
$txt .= '//'.str_repeat('-',123)."\n";
...
if( $file != 'index.htm' &&
eSubstrCount( $dorg, '/_bak_/' ) == 0 &&
eSubstrCount( $dorg, '/_datos/' ) == 0 &&
eSubstrCount( $dorg, '/_tmp/' ) == 0 ){
if( $file != 'ws_ftp.log' && mb_substr($file,0,2) != '__' ){
