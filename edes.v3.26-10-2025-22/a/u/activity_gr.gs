<?PHP
SS::query( "select user_name, user_surname from {$_ENV['SYSDB']}gs_user where cd_gs_user=".$_GET['cd_gs_user'] );
$r=SS::get("num");
...
SS::query( "select cdi from {$_ENV['SYSDB']}gs_activity where cdi>='".$Dia." 00:00:00' and cdi<='".$Dia." 23:59:59' order by cdi" );
}else{
SS::query( "select cdi from {$_ENV['SYSDB']}gs_activity where cd_gs_user=".$_GET['cd_gs_user']." and cdi>='".$Dia." 00:00:00' and cdi<='".$Dia." 23:59:59' order by cdi" );
}
while( $r=SS::get("num") ){
list($h,$m,$s) = explode(':',mb_substr($r[0],-8));
$t = $h*3600 + $m*60 + $s;
$t = round($t/300);
$Dim[$t]++;
}
