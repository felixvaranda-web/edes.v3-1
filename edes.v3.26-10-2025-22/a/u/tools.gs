<?PHP
if( S::$_User==-1 ) exit;
if( eSubstrCount($_SERVER['QUERY_STRING'], '&_PSOURCE')>0 ){
$_SERVER['QUERY_STRING'] = mb_substr($_SERVER['QUERY_STRING'],0,mb_strpos( $_SERVER['QUERY_STRING'], '&_PSOURCE' ));
}
if( eSubstrCount($_SERVER['QUERY_STRING'], '&_FORMBUTTONS')>0 ){
$_SERVER['QUERY_STRING'] = mb_substr($_SERVER['QUERY_STRING'],0,mb_strpos( $_SERVER['QUERY_STRING'], '&_FORMBUTTONS' ));
}
if( $_GET['LOG']!="" ){
...
$DirBase = mb_substr($_SERVER['DOCUMENT_ROOT'],0,mb_strrpos($_SERVER['DOCUMENT_ROOT'],'/'));
$DirWEB = mb_substr($DirBase,mb_strrpos($DirBase,'/')+1);
$NomDir = $DirBase.'.2p/';
...
if( WINDOW_OS ){
if( mb_substr($_Backup,-1) != '\\' ) $_Backup .= '\\';
if( $_Backup2!='' && mb_substr($_Backup2,-1) != '\\' ) $_Backup2 .= '\\';
}else{
if( mb_substr($_Backup,-1) != '/' ) $_Backup .= '/';
if( $_Backup2!='' && mb_substr($_Backup2,-1) != '/' ) $_Backup2 .= '/';
}
...
if( eSubstrCount( $_SERVER['HTTP_HOST'], '.' ) > 0 ){
$tmp = explode( '.',$_SERVER['HTTP_HOST'] );
$FileZip .= '_'.trim( $tmp[count($tmp)-1] );
}
$ZipEXE = ( WINDOW_OS ) ? 'edes\win\zip.exe' : 'zip';
...
<?PHP
$Dim = file( $Dir );
for( $n=0; $n<count($Dim); $n++ ) echo eSanitizeVar( $Dim[$n] ).'<br>';
?>
<script type="text/javascript">
...
if( eSubstrCount($buffer,'[error]')>0 ){
echo '<span id=E>';
}else if( eSubstrCount($buffer,'[warn]')>0 ){
echo '<span id=W>';
}else{
echo '<span id=N>';
}
