...
$Nivel++;
$sDir = $Dir;
if( mb_substr( $sDir,0,2 ) == './' ) $sDir = mb_substr( $sDir,2 );
if( $Tipo != 'G' ){
if( $Expandir ){
if( $Nivel > 1 ){
...
if( $Todo || in_array( mb_substr(mb_strtoupper($file), mb_strrpos($file, ".")+1), $_Extension ) ){
if( $Tipo == 'G' ){
$FileImg = $Dir.'/'.$file;
if( mb_substr($FileImg,0,2) == './' ) $FileImg = mb_substr($FileImg, 2 );
if( eSubstrCount( $FileImg,'.cur' ) ){
echo "<SPAN style='cursor:url(\"{$FileImg}\")' title='{$file}' style='border:1px solid #cccccc'> Cursor </span>";
}else{
echo "<IMG src='{$FileImg}' title='{$file}'>";
}
if( eSubstrCount($FileImg,'_0.')==0 ){
echo '<br>';
}else{
echo '&nbsp;';
}
