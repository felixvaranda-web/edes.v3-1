...
if( $r["orden"]==1 ){
SS::query("delete from {$_ENV['SYSDB']}gs_reply where cd_gs_poll='{$_POST['cd_gs_poll']}' and cd_gs_campaign='{$_POST['cd_gs_campaign']}' and cd_gs_reply_user='{$_POST['cd_gs_reply_user']}'");
SS::query("delete from {$_ENV['SYSDB']}gs_campaign_user where cd_gs_poll='{$_POST['cd_gs_poll']}' and cd_gs_campaign='{$_POST['cd_gs_campaign']}' and cd_gs_user='{$_POST['cd_gs_reply_user']}'");
}else if( SS::count("gs_reply",		  "cd_gs_poll='{$_POST['cd_gs_poll']}' and cd_gs_campaign='{$_POST['cd_gs_campaign']}' and cd_gs_reply_user='{$_POST['cd_gs_reply_user']}' and cd_gs_question='{$_POST['cd_gs_question']}'") ){
SS::query("delete from {$_ENV['SYSDB']}gs_reply where cd_gs_poll='{$_POST['cd_gs_poll']}' and cd_gs_campaign='{$_POST['cd_gs_campaign']}' and cd_gs_reply_user='{$_POST['cd_gs_reply_user']}' and cd_gs_question='{$_POST['cd_gs_question']}'");
}
}
...
SS::query("select mapa from {$_ENV['SYSDB']}gs_poll_img where cd_gs_poll_img={$pkImg}");
list($mapa) = SS::get("num");
$mapa = str_replace(
array("&#34;"),
array('"'    ),
$mapa
...
SS::query("insert into {$_ENV['SYSDB']}gs_reply (cd_gs_poll, cd_gs_campaign, cd_gs_reply_user, cd_gs_question, segundos, respuesta, aciertos, puntos) values ('{$_POST['cd_gs_poll']}', '{$_POST['cd_gs_campaign']}', '{$_POST['cd_gs_reply_user']}', '{$_POST['cd_gs_question']}', '{$_POST['segundos']}', '{$respuesta}', {$aciertos}, {$puntos})");
$pk = SS::id();
if( $r["respuesta_type"]=="F" ){
copy($_FILES["fichero"]["tmp_name"], eScript("/"."/poll/".SS::id().".".eFileExtension($_FILES["fichero"]["name"])));
}else if( $r["respuesta_type"]=="A" ||$r["respuesta_type"]=="T" ){
$NO = array();
$NOVARIAS = array();
SS::query("select palabra,varias from {$_ENV['SYSDB']}gs_tabu order by palabra", [], 1);
while($palabra=SS::get("num", 1)){
...
$respuesta = eStrtr($respuesta, "+-*=_<>{}[]()!¡¿?/\ºª#$'".'"', str_repeat(" ",25));
...
if( mb_strlen($palabra)==1 ) continue;
if( $NO[$palabra]!=1 ){
switch($palabra){
case ".":
$nPalabra += 30;
...
continue;
default:
SS::query("insert into {$_ENV['SYSDB']}gs_poll_dct (cd_gs_poll, cd_gs_campaign, cd_gs_question, cd_gs_reply, palabra, posicion) values ('{$_POST['cd_gs_poll']}', '{$_POST['cd_gs_campaign']}', '{$_POST['cd_gs_question']}', $pk, '{$palabra}', {$nPalabra})");
}
$nPalabra++;
}
}
