...
$question = $dim[$n];
SS::query("delete from {$_ENV['SYSDB']}gs_question where cd_gs_poll={$poll} and cd_gs_question={$question}");
}
}
$txt = $_POST["DATOS"];
$dim = $txt;
...
$dim[$n] = $dim[$n];
$json['pregunta'] = $json['pregunta'];
$json['pregunta'] = str_replace("'", '&#39;', $json['pregunta']);
$json['orden'] = $n+1;
$json['json'] = str_replace("'", '&#39;', $dim[$n]);
$preguntaLeng = mb_strlen($json['json']);
if( SS::count("{$_ENV['SYSDB']}gs_question", "cd_gs_poll={$poll} and cd_gs_question={$json['cd_gs_question']}")>0 ){
SS::query("update {$_ENV['SYSDB']}gs_question set
...
SS::query("insert into {$_ENV['SYSDB']}gs_question (cd_gs_poll,cd_gs_question,orden,pregunta,respuesta_type,respuesta_con_salto,pregunta_leng,json,marcar_tr) values ({$poll}, {$json['cd_gs_question']}, {$json['orden']},'{$json['pregunta']}', '{$json['respuesta_type']}', '{$json['respuesta_con_salto']}', {$preguntaLeng}, '{$json['json']}', '{$json['marcar_tr']}')");
}
}
die("Grabado");
?>