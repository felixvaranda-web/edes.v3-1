...
list($nmPoll) = DB::get();
eJS("try{ history.replaceState({foo:'bar'}, '-*-', 'ANALISIS_DE_ENCUESTA'); }catch(e){}");
...
list($iz, $dch) = explode(".", mb_substr($k,1));
$decimales = max($decimales, mb_strlen($dch));
if( $k[0]=="_" ) $total += $v;
}
...
echo "<span>Puntuación máxima: <b>".eNumberFormat($MaxPuntos, mb_strlen($dec))."</b></span>";
}
...
$icono = "";
if( $pre[$n][DATOS]["respuesta_conimg"]==$_ENV['ON'] && $pre[$n][DATOS]["ranking_img"]!="" ){
...
while(mb_strlen($xPuntos)>0 && mb_substr($xPuntos,-1)=="0"){
$xPuntos = mb_substr($xPuntos,0,-1);
}
if( mb_substr($xPuntos,-1)==","){
$xPuntos = mb_substr($xPuntos,0,-1);
}
$xPuntos = " ({$xPuntos}pts)";
}
