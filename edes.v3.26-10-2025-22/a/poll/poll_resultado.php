...
if( SS::count("gs_question", "cd_gs_poll={$pkPoll}")==0 ){
die("");
}
DB::query("select nm_gs_poll from {$_ENV['SYSDB']}gs_poll where cd_gs_poll={$pkPoll}");
list($nmPoll) = DB::get();
eJS("try{ history.replaceState({foo:'bar'}, '-*-', 'ESTADÍSTICA_DE_ENCUESTA'); }catch(e){}");
echo "<h3>ENCUESTA: {$nmPoll}</h3>";
if( SS::count("gs_reply", "cd_gs_poll={$pkPoll} and cd_gs_campaign={$pkCampaign}")==0 ){
eJSAnswer('window.opener.S.info("La encuesta no tiene ningún resultado.", 3, "warning"); window.close();');
}
$NumeroTotalDePreguntas = 0;
$dim = array();
SS::query("select orden, respuesta_type from {$_ENV['SYSDB']}gs_question where cd_gs_poll={$pkPoll} order by orden");
while( $r=SS::get("num") ){
...
SS::query("select P.orden, count(*), sum(R.segundos), min(R.segundos), max(R.segundos), P.respuesta_type
from {$_ENV['SYSDB']}gs_reply as R left join {$_ENV['SYSDB']}gs_question as P on R.cd_gs_poll=P.cd_gs_poll and R.cd_gs_question=P.cd_gs_question
...
echo "<caption><a name='RESMENU' id='RESMENU' style='cursor:var(--cAuto)'>Estadística de participación</a></caption>";
...
$TotalRespuestas += $r[1];
SS::query("select cd_gs_question from {$_ENV['SYSDB']}gs_question where cd_gs_poll={$pkPoll} and orden={$nPregunta} order by orden");
list($pkQuestion) = SS::get("num");
$sum2 = 0;
$varianza = 0;
$mediana = 0;
if( $pkQuestion>0 ){
$i = 0;
SS::query("select segundos from {$_ENV['SYSDB']}gs_reply where cd_gs_poll={$pkPoll} and cd_gs_campaign={$pkCampaign} and cd_gs_question={$pkQuestion} order by segundos");
while($r2=SS::get("num")){
$val = $r2[0];
...
echo "</tfoot>";
echo "</table>";
SS::query("select sum(puntos) from {$_ENV['SYSDB']}gs_reply where cd_gs_poll={$pkPoll} and cd_gs_campaign={$pkCampaign}");
$r = SS::get("num");
if( $r[0]>0 ){
SS::query("select punt,num from (select cd_gs_reply_user, sum(puntos) punt, count(*) num from {$_ENV['SYSDB']}gs_reply where cd_gs_poll={$pkPoll} and cd_gs_campaign={$pkCampaign} group by 1 order by 1) as x");
echo '<table border="0" cellspacing="1" cellpadding="3" style="display:table" class="col_0r col_1r col_2r col_3r col_4r col_5r col_6r">';
...
echo "</td></tr>";
$totalPreguntas = 0;
DB::query("select * from {$_ENV['SYSDB']}gs_question where cd_gs_poll={$pkPoll} order by orden", [], 1);
while( $pregunta = DB::get(1) ){
...
$dim = json_decode($pregunta["json"], true);
...
$nPregunta = 0;
$totalPreguntas++;
SS::query("select count(*), sum(segundos), min(segundos), max(segundos) from {$_ENV['SYSDB']}gs_reply where cd_gs_poll={$pkPoll} and cd_gs_question={$pkQuestion} and cd_gs_campaign={$pkCampaign}");
list($Registros, $TotalSg, $MinSg, $MaxSg) = SS::get("num");
...
$maxLen = max(mb_strlen($dim[$indice]), $maxLen);
}
}
...
if( $dim["respuesta_".$n."_img"]!="" && $dim["respuesta_conimg"]==$_ENV['ON'] ){
echo "<td width='50px'>";
...
echo "</table>";
SS::query("select respuesta, count(*) from {$_ENV['SYSDB']}gs_reply where cd_gs_poll={$pkPoll} and cd_gs_question={$pkQuestion} and cd_gs_campaign={$pkCampaign} group by respuesta order by respuesta");
while($r=SS::get("num")){
...
$icono = "";
if( $dim["respuesta_conimg"]==$_ENV['ON'] && $dim["ranking_img"]!="" ){
...
$nPregunta = 0;
SS::query("select aciertos, count(*) from {$_ENV['SYSDB']}gs_reply where cd_gs_poll={$pkPoll} and cd_gs_question={$pkQuestion} and cd_gs_campaign={$pkCampaign} group by aciertos order by aciertos");
while($r=SS::get("num")){
$nPregunta++;
if($_VerTrace)eTrace("Respuesta: ".$r[0].' = Votos: '.$r[1]);
$res[0][$r[0]+1] = $r[1];
...
echo "<tr>";
echo "<td>";
if( $dim["unir_".$n."_img"]!="" && $dim["respuesta_conimg"]==$_ENV['ON'] ){
...
if( $dim[$indice]!="" ){
if( $n>1 ) echo "<br><br>";
if( $dim["hueco_".$n."_img"]!="" && $dim["respuesta_conimg"]==$_ENV['ON'] ){
...
$maxRespuestas = 20;
$dimResultado = array();
SS::query("select respuesta, count(*) from {$_ENV['SYSDB']}gs_reply where cd_gs_poll={$pkPoll} and cd_gs_question={$pkQuestion} and cd_gs_campaign={$pkCampaign} group by 1 order by 2 desc,1");
$n=0;
while($r=SS::get("num")){
...
$maxRespuestas = 20;
$dimResultado = array();
SS::query("select palabra,count(*) from {$_ENV['SYSDB']}gs_poll_dct where cd_gs_poll={$pkPoll} and cd_gs_question={$pkQuestion} and cd_gs_campaign={$pkCampaign} group by 1 order by 2 desc,1");
$palabra = array();
$n=0;
while($r=SS::get("num")){
if( ++$n>$maxRespuestas ) break;
...
SS::query("select palabra,count(*) from {$_ENV['SYSDB']}gs_poll_dct where cd_gs_poll={$pkPoll} and cd_gs_question={$pkQuestion} and cd_gs_campaign={$pkCampaign}
and palabra<>'{$palabra[$p]}' and cd_gs_reply in (
select cd_gs_reply from {$_ENV['SYSDB']}gs_poll_dct where cd_gs_poll=8 and cd_gs_question={$pkQuestion} and palabra='{$palabra[$p]}'
...
case "MM":
$todo = array();
SS::query("select respuesta,count(*) from {$_ENV['SYSDB']}gs_reply where cd_gs_poll={$pkPoll} and cd_gs_question={$pkQuestion} and cd_gs_campaign={$pkCampaign} group by 1 order by 1");
while($r=SS::get("num")) $todo[] = $r;
...
$p = new TCPDF('P', 'pt', 'A4', false, 'UTF-8', false );
...
foreach ($sd as $sf) {
if (mb_strlen($sf)>(32+mb_strlen($tempfileheader)+mb_strlen($tempfiletimeseperator)+3)) { // if filename long enough
$t1=mb_substr($sf,(32+mb_strlen($tempfileheader)),mb_strlen($tempfiletimeseperator)); //searching time seperator
$t2=mb_substr($sf,0,mb_strlen($tempfileheader)); //searching file header
...
$thing = "<"."?php\n\n echo '22222222';\n?".">";
$filename = "php://memory";
echo mb_strlen($thing)."-".eTrace($thing);
$fp = fopen($filename, "r+b");
fwrite($fp, $thing);
rewind($fp);
include("php://memory");
