...
_RespuestasMin = 0;
_RespuestasMax = 1;
if( dim["respuesta_obligatoria"]==S.setup.checkOn ) _RespuestasMin = 1;
txt = "<table>";
for(n=1; n<99; n++){
...
if( dim[indice]!="" ){
tmp = "<tr>";
if( dim["respuesta_"+n+"_img"]!="" && dim["respuesta_conimg"]==S.setup.checkOn ){
tmp += "<td>";
...
_RespuestasMin = 0;
_RespuestasMax = 0;
if( dim["respuesta_obligatoria"]==S.setup.checkOn ){
...
break;
case "MM":
_RespuestasMax = ((dim["respuesta_obligatoria"]==S.setup.checkOn)?1:0);
...
if( dim["respuesta_obligatoria"]==S.setup.checkOn ) _RespuestasMin = 1;
var icono = "";
if( dim["respuesta_conimg"]==S.setup.checkOn && dim["ranking_img"]!="" ){
...
if( dim["unir_"+n+"_img"]!="" && dim["respuesta_conimg"]==S.setup.checkOn ){
...
if( dim["slide_type"]!=S.setup.checkOn ){
txt += '<span></span>';
}
...
case "LF":
txt = '<img id="pagina_img" src="poll/'+dim["pagina_img"]+'" style="cursor:var(--cAuto); float:left; margin-right:10px; margin-bottom:5px;">'+txt;
break;
case "RF":
txt = '<img id="pagina_img" src="poll/'+dim["pagina_img"]+'" style="cursor:var(--cAuto); float:right; margin-left:10px; margin-bottom:5px;">'+txt;
break;
case "U":
txt = '<center><img id="pagina_img" src="poll/'+dim["pagina_img"]+'" style="cursor:var(--cAuto); margin-bottom:3px;"></center>'+txt;
break;
case "D":
txt += '<center><img id="pagina_img" src="poll/'+dim["pagina_img"]+'" style="cursor:var(--cAuto); margin-top:3px;"></center>';
break;
case "L":
txt = '<table><tr><td><img id="pagina_img" src="poll/'+dim["pagina_img"]+'" style="cursor:var(--cAuto); margin-right:3px;"><td style="text-align:justify">'+txt+'</table>';
break;
case "R":
txt = '<table><tr><td style="text-align:justify">'+txt+'<td><img id="pagina_img" src="poll/'+dim["pagina_img"]+'" style="cursor:var(--cAuto); margin-left:3px;"></table>';
break;
}
}
break;
...
txt += "<input name='cd_gs_reply_user' value='<?=S::$_User?>' style='display:none'>";
...
_RespuestasMax = 1;
if( dim["respuesta_obligatoria"]==S.setup.checkOn ) _RespuestasMin = 1;
txt = '<input name="fecha" value="" class="EDITABLE FondoSelect" ewe="1" leng="10" maxlength="10" size="10" onfocus=S.key("F4",10,0) tc="F4" style="width:72px;"><i class="ICONINPUT" onclick="S(&quot;:fecha&quot;).calendar()" oncontextmenu="S(&quot;:fecha&quot;).calendar(S.lng(365)[0])" id="TOOLSDate">,</i>';
break;
case "H":
_RespuestasMin = 0;
_RespuestasMax = 1;
...
txt = "<table style='width:100%;display:table;'>";
for(k in dim){
if( /^user_/.test(k) && dim[k]==S.setup.checkOn ){
...
if( dim["hueco_"+n+"_img"]!="" && dim["respuesta_conimg"]==S.setup.checkOn ){
...
if( dim["respuesta_aleatoria"]==S.setup.checkOn ){
t = dimRes.length;
while( ok ){
n = Math.floor((Math.random()*t)+1)-1;
if( dimRes[n]!=null ){
...
if( dim["respuesta_type"]==S.setup.checkOn ){
S(':range_1').range();
}
...
case "D":
case "H":
if( _Dim["respuesta_obligatoria"]==S.setup.checkOn && S("#RESPUESTA *").val()=="" ){
xError = 'Falta responder a la pregunta';
}
break;
case "DH":
if( _Dim["respuesta_obligatoria"]==S.setup.checkOn && (S(":fecha").val()=="" || S(":hora").val()=="") ){
xError = 'Falta responder a la pregunta';
break;
}
...
break;
case "F":
if( _Dim["respuesta_obligatoria"]==S.setup.checkOn && S(":fichero").val()=="" ){
xError = 'Falta responder a la pregunta';
break;
}
...
if( _Dim["respuesta_obligatoria"]==S.setup.checkOn && xRespuesta=="" ){
xError = 'Falta responder a la pregunta';
}
break;
case "MM":
...
if( o.children.length ){
if( txt!="" ) txt += "~";
txt += (o.children[0].checked) ? S.setup.checkOn : S.setup.checkOff;
}else{
txt += "|"+o.innerText;
}
});
...
DB::query("select * from {$_ENV['SYSDB']}gs_campaign where cd_gs_campaign=".$_GET["cd_gs_campaign"]);
$cmp = DB::get();
if( $cmp["cd_gs_poll"]=="" ){
eInit();
eEnd();
...
DB::query("select * from {$_ENV['SYSDB']}gs_poll where cd_gs_poll=".$cmp["cd_gs_poll"]);
$r = DB::get();
...
DB::query("select * from {$_ENV['SYSDB']}gs_question where cd_gs_poll=".$cmp["cd_gs_poll"]." order by orden");
...
SS::query("select user_name,user_surname from {$_ENV['SYSDB']}gs_user where cd_gs_user=".S::$_User);
list($_UserName, $_UserSurname) = SS::get("num");
?>
<form accept-charset="utf-8" name="FRM1" action="edes.php?E:$a/poll/reply.php" style="height:100%;" method="post" enctype="multipart/form-data">
<?PHP
...
DB::query("select * from {$_ENV['SYSDB']}gs_poll where cd_gs_poll=".$pk);
$r = DB::get();
$txt = "";
if( $r[$texto]!="" ){
$txt = $r[$texto];
...
switch($type){
case "LF":
echo '<img id="" src="poll/'.$r[$img].'" style="cursor:var(--cAuto); float:left; margin-right:10px; margin-bottom:5px;">'.$txt;
break;
case "RF":
echo '<img id="" src="poll/'.$r[$img].'" style="cursor:var(--cAuto); float:right; margin-left:10px; margin-bottom:5px;">'.$txt;
break;
case "U":
echo '<center><img id="" src="poll/'.$r[$img].'" style="cursor:var(--cAuto); margin-bottom:3px;"></center>'.$txt;
break;
case "D":
echo $txt.'<center><img id="" src="poll/'.$r[$img].'" style="cursor:var(--cAuto); margin-top:3px;"></center>';
break;
case "L":
echo '<table><tr><td><img id="" src="poll/'.$r[$img].'" style="cursor:var(--cAuto); margin-right:3px;"><td style="text-align:justify">'.$txt.'</table>';
break;
case "R":
echo '<table><tr><td style="text-align:justify">'.$txt.'<td><img id="" src="poll/'.$r[$img].'" style="cursor:var(--cAuto); margin-left:3px;"></table>';
break;
default:
echo '<img id="" src="poll/'.$r[$img].'" style="cursor:var(--cAuto); float:left; margin-right:10px; margin-bottom:5px;">';
echo $txt;
}
}else{
echo $txt;
