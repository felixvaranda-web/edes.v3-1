...
if( $ID > 0 ){
SS::query( "update {$_ENV['SYSDB']}gs_op set status='{$STATUS}', cd_gs_user='{$_User}', dt_status='{$Hoy}' where id='{$ID}'" );
...
if( $USUARIO=='' ) $Hoy = '';
if( $ID > 0 ){
SS::query( "update {$_ENV['SYSDB']}gs_op set cd_gs_user='{$USUARIO}', dt_status='{$Hoy}' where id='{$ID}'" );
?>
<script type="text/javascript">
S("body").tip(top.eLng(27),1);
</script>
...
if( window.name=='IWORK' && !event.ctrlKey && !event.altKey ){
top.VerPag( HR );
}else{
...
<TH class=THOption width=1px><input name=ENCONTRADOS SIZE=3 title="<?= eLng('Nº de ocurrencias') ?>" style="cursor:var(--cAuto);width:25px">
...
<col class=Option style='cursor:var(--cPointer)'>
<?PHP
...
$r['indent'] = mb_strlen($Menu[$n])-mb_strlen(ltrim($Menu[$n]));
