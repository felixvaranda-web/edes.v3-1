...
SS::query( "insert into {$_ENV['SYSDB']}gs_op_status values( '{$ID}', '{$CDI}', '{$STATUS}', '{$_User}' )" );
?>
<script type="text/javascript">
...
if( $USUARIO=='' ) $Hoy = '';
if( $ID > 0 ){
SS::query( "update {$_ENV['SYSDB']}gs_op set cd_gs_user='{$USUARIO}', dt_status='{$Hoy}' where cd_gs_op='{$ID}'" );
?>
<script type="text/javascript">
S("body").tip(eLng(27),1);
</script>
...
SS::query( "update {$_ENV['SYSDB']}gs_op set status='9' where status='8' and dt_status<'{$HaceUnMes}'" );
eLink('status');
echo '<style>';
if( $_SESSION["_D_"]=="" ) echo '#SelEstado TD { cursor:var(--cAuto); }';
...
<TH class=THOption width=1px><input name=ENCONTRADOS SIZE=3 title="Nº de ocurrencias" style="cursor:var(--cAuto);width:25">
...
<?PHP  if( $_SESSION["_Development"] && $_SESSION["_D_"]<>"" ){ ?>
...
SS::query( "select cd_gs_user, user_name, user_surname from {$_ENV['SYSDB']}gs_user where cd_gs_user in ( select distinct cd_gs_user from {$_ENV['SYSDB']}gs_op where cd_gs_user>0 )" );
while( $r=SS::get("num") ){
$n = SS::count("{$_ENV['SYSDB']}gs_op", "cd_gs_user='{$r[0]}'", [], 2);
$_DimUser[] = array( $r[0], trim($r[1]).' '.trim($r[2]), $n );
$_DimNomUser['u'.$r[0]] = trim($r[1]).' '.trim($r[2]);
}
}
$Dim = array();
$nFalta = SS::count("{$_ENV['SYSDB']}gs_op", "( cd_gs_user='0' or cd_gs_user='' or cd_gs_user is null )", [], 2 );
...
$v = $r['dt_status'];
if( mb_strlen($v)==10 && mb_substr($v,4,1)=='-' && mb_substr($v,7,1)=='-' ){
$v = eDataFormat($v,"F4");
}
if( isZero($v) ) $v = '';
echo '<td>&nbsp;'.$v;
...
<?=(( $_SESSION["_D_"]<>"" ) ? ' onclick="SetEstado()"' : ' style="cursor:var(--cAuto)"')?> onmouseleave='this.style.display="none"'>
<COL width=20>
...
<TR><TH colspan=2 style='cursor:var(--cPointer)'>OPCIONES&nbsp;ASIGNADAS</TH></TR>
<?PHP
if( $_SESSION["_D_"]=='~' ){
...
$k = mb_substr($k,1);
if( mb_strlen(trim($v))>0 ) echo '<tr u='.$k.'><td>'.trim($v);
}
?>
</TABLE>
<script type="text/javascript">
