...
if( eSubstrCount($dorg,'/edes.v3/tcpdf/')>0 || eSubstrCount($dorg,'/edes.v3/_vb/')>0 || eSubstrCount($dorg,'/_bak_/')>0 || eSubstrCount($dorg,'/_vb/')>0 ) return;
...
$pLanguage = $n;
$_NumScript++;
list($DimLng,$oMd5) = explode(REM,mb_substr(trim($Dim[$n]),10));
...
$txt = trim($Dim[$p]);
if( $txt!='' && $txt[0]!='.' && $txt[0]!='¿' && mb_substr($txt,0,2)!=REM ){
$xMd5 .= $txt;
}
}
$nMd5 = md5($xMd5);
