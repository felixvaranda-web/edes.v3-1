...
if( preg_match("/".$Buscar."/iu", $file) && preg_match('/\.png*/iu', $file ) ){
if( $_GET['DELLOG']==$_ENV['ON'] ){
@unlink( $DirBase.$file );
}else{
$Dim[] = $file;
}
...
SS::query( "select user_name,user_surname from {$_ENV['SYSDB']}gs_user where cd_gs_user={$_GET['USER']}" );
$row = SS::get("num");
