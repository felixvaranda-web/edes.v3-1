...
<?PHP
SS::query("select A.*,T.container from {$_ENV['SYSDB']}gs_structure A, {$_ENV['SYSDB']}gs_tstructure T where A.cd_gs_structure_parent=0 and A.cd_gs_tstructure=T.cd_gs_tstructure order by T.sort,A.nm_gs_structure");
while( $r = SS::get() ){
$TieneHijos = ( (SS::count( 'gs_structure', "cd_gs_structure_parent={$r['cd_gs_structure']}", $p ) > 0 ) ? '_0' : '_1' );
$Sufijo = $TieneHijos;
$r['mfilter'] = trim($r['mfilter']);
if( $r['container']!=$_ENV['ON'] ) $Sufijo = '_1';
$SeCargo = (($TieneHijos=='_0')?0:1);
echo "<tr class='oC'><td colspan=2><img src='edes.php?R:/_datos/config/{$r['cd_gs_tstructure']}{$Sufijo}.gif' class='n0' cElemento={$r['cd_gs_structure']} cPadre={$r['cd_gs_structure_parent']} SC={$SeCargo} CNT='{$r['container']}' Tipo='{$r['cd_gs_tstructure']}' User='{$r['cd_gs_user']}' Hijos='".mb_substr($TieneHijos,1)."' Filter='{$r['mfilter']}'>".trim($r['nm_gs_structure']).'</td></tr>';
}
?>
</table>
</div>
