...
if( mb_strstr( $argv[0], '?') ) list( , $argv[0] ) = explode('?', $argv[0] );
if( $argv[0] == 'T' ){
?>
...
$txt = trim($Dim[$n]);
if( mb_substr( $txt,0,6)=='nobody' ) echo $Dim[$n]."\n";
}
echo "=========\n";
echo $salida;
echo '</pre>';
...
if( eSubstrCount( $Dim[$n], "You don't have permission to access" ) == 1 ){
$_ConPermiso = false;
}
}
if( $argv[0] == 'O' ){
...
if( 'Srv PID Acc ' == mb_substr( $txt,0,12 ) ){
$Activado = 1;
continue;
}
if( '_____' == mb_substr( $txt,0,5 ) && $Activado == 2 ) break;
if( 'Current Time: ' == mb_substr( $txt,0,14 ) ){
$tmp = explode( ' ',$txt);
$Hora = $tmp[4];
continue;
}
...
<FORM accept-charset="utf-8" METHOD=POST ACTION="" NAME="FRM">
...
if( '_____' == mb_substr( $txt,0,5 ) && $Activado == 2 ){
echo '<TR>';
