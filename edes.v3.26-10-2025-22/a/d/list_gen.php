<?PHP
if( $_GET['_dynamic_generation']==$_ENV['ON'] ){
$cd_gs_list_store = str_replace('cd_gs_list_store=', '', $_GET["_FILTER"]);
$Dim = SS::selectOne("select * from {$_ENV['SYSDB']}gs_list_store", ["cd_gs_list_store" => $cd_gs_list_store]);
foreach($Dim as $k=>$v){
...
$_POST[$k] = str_replace('&#92;' ,'\\',$_POST[$k]);
$_POST[$k] = str_replace('&amp;' ,'&' ,$_POST[$k]);
if( mb_substr($k,0,8)=='_colsop_' && !empty($v) ) $_POST[$k] = mb_strtoupper($v);
if( mb_substr($k,0,7)=='_field_' && !empty($v) ){
if( !empty($xFields) ) $xFields .= ',';
$xFields .= $v;
}
if( $k[0]=='_' ) $_PDFLABELHIDDEN[$k] = 'NOTTOSHOW';
}
if( !empty($xFields) ){
$_POST['_sql'] = str_replace('#', $xFields, $_POST['_sql']);
}
...
$_PDFLABELHIDDEN['time'] = "NotToShow";
include("../_datos/config/sql.ini");
if( !empty($_POST['_ddbb']) ){
$tmp = explode(',', $_POST['_ddbb']);
$_OtroDiccionario = true;
$tmp[0] = str_replace(' ', '', $tmp[0]);
if( mb_substr_count($tmp[0], ',')==0 ){
if( $tmp[0][0]=='>' ) $tmp[0] = trim(mb_substr($tmp[0],1));
$_DB = $tmp[0];
if( mb_substr_count(str_replace('\\', '/', $tmp[0]), '/')==0 ) $tmp[0] = '/_datos/config/'.$tmp[0];
if( mb_substr_count($tmp[0],'.')==0 ) $tmp[0] .= '.ini';
if( $tmp[0][0]=='~' ){
$_SqdDefinitionFile = str_replace('~', '../..', $tmp[0]);
}else{
$_SqdDefinitionFile = eScript($tmp[0]);
}
if( file_exists($_SqdDefinitionFile) ){
...
if( $_Sql=='pdo' && !empty($_SqlPDOConnect) ){
$_SqlPDOConnect = eval('return ("'.$_SqlPDOConnect.'");');
}
}
$_DimEDF[] = '[Expire]20'."\n";
$_DimEDF[] = '[Title]='.$_POST['nm_gs_list_store']."|l\n";
$_DimEDF[] = '[SubTitle]'."\n";
if( !empty($_POST['_dblimit']) ) $_DimEDF[] = '[DBLimit]'.$_POST['_dblimit']."\n";
$_DimEDF[] = '[PDFVar] PDF_Grid = true'."\n";
$xDivide = false;
if( !empty($_POST['_grid']) && $_POST['_grid'][0]=='·' ){
$_POST['_grid'] = mb_substr($_POST['_grid'],1);
$xDivide = true;
}
$xAncho = array();
$xDimAncho = array();
...
for( $n=0; $n<count($r); $n++ ){
$r[$n] = trim($r[$n]);
if( mb_strlen($r[$n])>$xAncho[$n] ) $xAncho[$n] = mb_strlen($r[$n]);
$xDimAncho[$n][mb_strlen($r[$n].'')]++;
if( !empty($r[$n]) && $xTipo[$n]!='N' ){
if( $xDebug ) eTrace($r[$n]);
if( is_numeric($r[$n]) ){
$numero = $r[$n]*1;
$xNumero = strval($numero);
if( ($xNumero."")==($r[$n]."") ){
if( mb_substr_count($r[$n], '.')==1 ){
list(,$nDecimales) = explode('.', $r[$n].".");
$xDecimales[$n] = mb_strlen(trim($nDecimales));
$xTipo[$n] = '+,';
if( $xDebug ) eTrace('1: +,');
}else{
if( $r[$n]=='0' ){
...
}else if( mb_strlen($r[$n])==10 && mb_substr_count($r[$n],'-')==2 ){
$xTipo[$n] = 'F4';
$xFormatearF4 = true;
...
$xCampo = 'campo'.$n;
if( !empty($_POST['_field_'.$n]) ) $xCampo = $_POST['_field_'.$n];
if( $_POST['_colsop_'.$n]=='G' ){
if( !empty($_POST['_grid']) ){
$ColTPC = '';
if( $_POST['_grid'][0]=='%' ){
$_POST['_grid'] = mb_substr($_POST['_grid'],1);
$ColTPC = '%';
...
if( $_POST['_colsop_'.$n]==$_ENV['ON'] && !empty($_POST['_th2_'.$n]) ){
$tmp = explode('/', $_POST['_th2_'.$n]);
if( !empty($tmp[0]) ) $xColsOpPrefix[] = trim($tmp[0]);
if( !empty($tmp[1]) ) $xColsOpSubTitle[] = trim($tmp[1]);
if( $n==0 && !empty($tmp[2]) ) $xColsOpSpan = '|'.trim($tmp[2]);
$_POST['_th2_'.$n] = '';
}
if( $_POST['_colsop_'.$n]=='+' ) $xCHARTROW .= ','.$n;
$xColsOp .= $_POST['_colsop_'.$n].',';
$xFormat .= $_POST['_format_'.$n].',';
if( !empty($_POST['_th2_'.$n]) ){
if( $_POST['_th2_'.$n]=='=' ){
...
$xTH = $_POST['_th_'.$n];
if( empty($xTH) ) $xTH = $xCampo;
if( !empty($xTH) && $xTH==strval(floatval($xTH)) ) $xTH = '&nbsp;'.$xTH;
if( empty($xTH) ) $xTH = '&nbsp;'.($n+1);
if( $xDivide && mb_substr_count($xTH,' ')>0 ){
$b = 0;
$min = 99;
for($i=0; $i<mb_substr_count($xTH,' '); $i++){
if( abs($m-mb_strpos($xTH,' ',$b)) < $min ){
$min = abs($m-mb_strpos($xTH,' ',$b));
$p = mb_strpos($xTH,' ',$b);
$b = $p+1;
}
}
$xTH = mb_substr($xTH,0,$p).'<BR>'.mb_substr($xTH,$p+1);
}
$_DimEDF[] = $xTH.'|'.$xCampo.'|'.$xTipo[$n].'|T|'.$xLong.'||-||'.$xCentrar.'|'."\n";;
}
if( !empty(str_replace(',', '', $xColsOp)) ){
$xColsOpSubTitle = implode(',', $xColsOpSubTitle);
$_DimEDF[] = '[ColsOp]'.$xColsOp."|{$xColsOpSubTitle}{$xColsOpSpan}\n";
}
if( !empty(str_replace(',', '', $xFormat)) ) $_DimEDF[] = '[Format]'.$xFormat."\n";
if( count($xColsOpPrefix)>0 ) $_DimEDF[] = '[ColsOpPrefix]'.implode('|',$xColsOpPrefix)."\n";
if( trim($_POST['_ReportRow'])==$_ENV['ON'] && !empty(str_replace(',', '', $xCHARTROW)) ){
$_DimEDF[] = '[ChartRow]'.mb_substr($xCHARTROW,1)."|100\n";
}
if( trim($_POST['_ReportCol'])==$_ENV['ON'] ) eChartCol();
if( !empty($xGrid) ) $_DimEDF[] = $xGrid;
$txt = '';
for($n=0; $n<count($xTHColSpan); $n++){
if( !empty($txt) ) $txt .= '|';
$txt .= $xTHColSpan[$n][0].','.$xTHColSpan[$n][1].','.$xTHColSpan[$n][2];
}
if( !empty($txt) ) $_DimEDF[] = '[THColSpan]'.$txt."\n";
if( $xFormatearF4 ){
for($f=0; $f<count($usuCursor); $f++){
for($c=0; $c<$xColumnas; $c++){
if( $xTipo[$c]=='F4' ){
if( empty($usuCursor[$f][$c]) ){
}else if( $usuCursor[$f][$c]=='0000-00-00' ){
$usuCursor[$f][$c] = '';
}else{
list($a,$m,$d) = explode('-', $usuCursor[$f][$c]);
if( mb_strlen($a)==4 ) $usuCursor[$f][$c] = "{$d}-{$m}-{$a}";
}
}
}
}
