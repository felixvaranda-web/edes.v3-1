...
<table id=TablaColores _style='cursor:var(--cPointer)' onclick=PonColor()>
<caption>Combinación de colores</caption>
<?PHP
for($n=0; $n<5; $n++){
'<col align=center style="cursor:var(--cPointer);font-family:monospace"><col style="cursor:var(--cAuto)">';
}
SS::query("select * from {$_ENV['SYSDB']}gs_color order by orden");
$Dim = array();
...
<table border=0px cellspacing=1px cellpadding=5px id=ColoresAuto style='cursor:var(--cPointer)' onclick=MemorizaColor()>
