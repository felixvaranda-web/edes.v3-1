...
$ids=trim($ids);
if( mb_substr($ids,mb_strlen($ids)-1,1)==',' )
...
$q="select * from {$_ENV['SYSDB']}gs_script where cd_gs_script in(
select cd_gs_script from {$_ENV['SYSDB']}gs_transchange where cd_gs_transchange in({$ids}) group by 1
)";
SS::query($q, [], 1);
$s=array();
while($r=SS::get(1)){
SS::query("select word_id from {$_ENV['SYSDB']}gs_transchange where cd_gs_script={$r['cd_gs_script']} and cd_gs_transchange in({$ids})", [], 2);
$wi=array();
while($rr=SS::get(2)){
$wi[] = $rr['word_id'];
}
...
SS::query("select cd_gs_script from {$_ENV['SYSDB']}gs_transchange where tf_script='{$_ENV['ON']}' group by cd_gs_script");
while($r=SS::get()){
...
SS::query("select nm_gs_script,filepath,type from {$_ENV['SYSDB']}gs_script where cd_gs_script='{$cd_gs_script}'", [], 1);
$r=SS::get(1);
...
SS::query("update {$_ENV['SYSDB']}gs_transchange set tf_script='' where cd_gs_script='{$cd_gs_script}'", [], 2);
$cta_actualizados++;
echo <<<EOD
...
$edespath = eScript('$');
$edespath = mb_substr($edespath,0,mb_strlen($edespath)-1).'_bak/';
...
$tag = array();
$lng=array();
SS::query("select cd_gs_language from {$_ENV['SYSDB']}gs_transchange where cd_gs_script='{$cd_gs_script}' group by cd_gs_language", [], 2);
while($r=SS::get(2)){
...
$t =array();
SS::query("select * from {$_ENV['SYSDB']}gs_transchange where cd_gs_script='{$cd_gs_script}' order by word_id,cd_gs_language", [], 2);
while( $r=SS::get(2) ){
...
$s = $word_id . str_repeat( $carRepeat , $max_word_len - mb_strlen($word_id));
$comment='';
foreach($av as $k=>$v){
$s .= $carRepeat.'|'.$carRepeat. $v['valor'] . str_repeat( $carRepeat ,  $ll[$v['cd_gs_language']]  - mb_strlen($v['valor'])  );
$comment = $v['comment'];
}
if( $comment!='' ){
...
$cdi_changed	= date('Y-m-d H:i:s');
$now=date('Y-m-d H:i:s');
$q="update {$_ENV['SYSDB']}gs_transchange set word_val='{$word_val}', word_val_md5='{$word_val_md5}', cdi_changed='{$cdi_changed}', tf_changed='', tf_script='{$_ENV['ON']}', comment='{$comment}'
where cd_gs_transchange in({$ids})";
SS::query($q);
$tf_changed = ($cd_gs_language==$deflang) ? $_ENV['ON'] : $_ENV['OFF'];
SS::query("select cd_gs_script,word_id from {$_ENV['SYSDB']}gs_transchange where cd_gs_transchange in({$ids})", [], 2);
while($r=SS::get(2)){
SS::query("update {$_ENV['SYSDB']}gs_transchange set comment='{$comment}', tf_changed='{$tf_changed}', tf_script='{$_ENV['ON']}'
where cd_gs_language<>'{$deflang}' and cd_gs_script='{$r['cd_gs_script']}' and word_id='{$r['word_id']}'", [], 3);
}
echo <<<EOD
<script type="text/javascript">
...
EOD;
SS::query("select * from {$_ENV['SYSDB']}gs_language where cd_gs_language='{$lng}'");
$r=SS::get();
$col[] = <<<EOD
...
$gs_op_lng=array();
$gs_op_deflang=array();
SS::query("select * from {$_ENV['SYSDB']}gs_op_lng order by md5");
while($r=SS::get()){
$r['flg'] = 0;
...
$t =array();
SS::query("select md5,count(*) as cta from {$_ENV['SYSDB']}gs_op_lng
...
$class_tscr = ( ($r0_gs_op_lng['tf_changed']==$_ENV['ON'])? 'touched_script':'untouched_script' );
$s =<<<EOD
<tr id='oneTR'>
...
SS::query("select * from {$_ENV['SYSDB']}gs_transchange where type='{$GS_TRANSCHANGE_TYPE}' order by word_val_md5");
while($rr=SS::get()){
$rr['flg'] = 0;
...
$t =array();
SS::query("select word_val_md5,count(*) as cta from {$_ENV['SYSDB']}gs_transchange
...
$class_tscr = ( ($r0_gs_transchange['tf_changed']==$_ENV['ON'])? 'touched_script':'untouched_script' );
$word_val = encod( urldecode( trim($r0_gs_transchange['word_val']) ) );
$comment  = encod( urldecode( trim($r0_gs_transchange['comment']) ) );
$class_wmtx = ( $r0_gs_transchange['tf_script']==$_ENV['ON'] )? 'touched_wmtx':'untouched_wmtx';
$s =<<<EOD
<tr id='oneTR'>
...
global $deflang;
$natratar=SS::count("{$_ENV['SYSDB']}gs_op", "");
echo <<<EOD
<script type="text/javascript">
...
EOD;
ob_flush();
SS::query("select * from {$_ENV['SYSDB']}gs_language where tf_translation='{$_ENV['ON']}'");
while($r=SS::get()){
$rlng[$r['cd_gs_language']] = $r;
}
$cta=0;
SS::query("select * from {$_ENV['SYSDB']}gs_op order by seq,seq_parent", [], 1);
while($r=SS::get(1)){
if( trim($r['caption'])=='' ) continue;
$r['caption'] = urlencode( trim( $r['caption'] ) );
$r['tip']     = urlencode( trim( $r['tip'] ) );
$mmd5         = md5($r['caption'].$r['tip']);
$tf_changed   = (($r['md5']==$mmd5) ? $_ENV['OFF'] : $_ENV['ON']);
foreach($rlng as $k=>$lenguas){
$cd_gs_language = $lenguas['cd_gs_language'];
$nr = SS::count("{$_ENV['SYSDB']}gs_op_lng", "id='{$r['cd_gs_op']}' and cd_gs_language='{$cd_gs_language}'");
if( $nr==0 ){
$q="insert into {$_ENV['SYSDB']}gs_op_lng (id,cd_gs_language,caption,tip,md5) values(
'{$r['cd_gs_op']}',
'{$cd_gs_language}',
'{$r['caption']}',
'{$r['tip']}',
...
if( $cd_gs_language==$deflang ){
SS::query("select md5 from {$_ENV['SYSDB']}gs_op_lng where id='{$r['cd_gs_op']}' and cd_gs_language='{$cd_gs_language}'", [], 2);
$rr=SS::get(2);
if( $rr['md5']!=$mmd5 ){
$q="update {$_ENV['SYSDB']}gs_op_lng
...
SS::query($q, [], 3);
$q="update {$_ENV['SYSDB']}gs_op_lng set tf_changed='{$tf_changed}'	where id='{$r['cd_gs_op']}'";
SS::query($q, [], 3);
}
}
}
...
global $deflang;
$gs_language=array();
SS::query("select * from {$_ENV['SYSDB']}gs_language order by cd_gs_language");
while($r=SS::get()){
...
if( SS::count("{$_ENV['SYSDB']}gs_script", "filepath='{$filepath}'")==0 ){
$nm_script	= basename($archivo);
$pinfo		= pathinfo($archivo);
$extension	= $pinfo['extension'];
SS::query("insert into {$_ENV['SYSDB']}gs_script(nm_gs_script,extension,filepath,type)
values('{$nm_script}','{$extension}','{$filepath}','{$type}')");
$cd_gs_script = SS::id();
}else{
SS::query("select * from {$_ENV['SYSDB']}gs_script where filepath='{$filepath}'");
$rs = SS::get();
...
if( SS::count("{$_ENV['SYSDB']}gs_language", "cd_gs_language='{$cd_gs_language}'")==0 ){
$err[] = "No existe el lenguaje '{$cd_gs_language}' descrito en la línea {$a['line']} en el archivo: ".str_replace(mCHR92'/',$filepath);
llog( "No existe el lenguaje '{$cd_gs_language}' descrito en la línea {$a['line']} en el archivo: ".str_replace(mCHR92'/',$filepath) );
}
}
if( count($err) ){
$err = implode('<br>',$err);
...
$word_val		= urlencode(trim($word_val));
$word_val_md5	= md5($word_val);
$nr = SS::count("{$_ENV['SYSDB']}gs_transchange", "cd_gs_script={$cd_gs_script} and cd_gs_language='{$cd_gs_language}' and word_id='{$word_id}' ");
if( $nr==0 ){
llog( str_replace('"',"\"","(N){$cd_gs_language} - {$word_id} - {$word_val}" ) );
$now = date('Y-m-d H:i:s');
$q = "insert into {$_ENV['SYSDB']}gs_transchange (cd_gs_script,cd_gs_language,word_id,word_val,word_val_md5,cdi_add,comment,type,word_val_old,word_val_md5_old)
...
SS::query("select * from {$_ENV['SYSDB']}gs_transchange where cd_gs_script={$cd_gs_script} and cd_gs_language='{$cd_gs_language}' and word_id='{$word_id}' ");
$rt = SS::get();
if( $word_val_md5 != $rt['word_val_md5'] ){
$q="update {$_ENV['SYSDB']}gs_transchange set tf_changed='{$_ENV['ON']}', word_val='{$word_val}', word_val_md5='{$word_val_md5}', word_val_old='{$word_val}', word_val_md5_old='{$word_val_md5}', comment='{$comment}'  where cd_gs_script={$cd_gs_script} and cd_gs_language='{$cd_gs_language}' and word_id='{$word_id}'";
llog($q);
SS::query($q);
$q="update {$_ENV['SYSDB']}gs_transchange set tf_changed='{$_ENV['ON']}' where cd_gs_script={$cd_gs_script} and word_id='{$word_id}'";
SS::query($q);
}
}
}
...
foreach( $a as $k=>$lin ){
$lin = trim($lin);
if( mb_strtoupper(mb_substr($lin,0,10))=='[LANGUAGE]' ){
$e = mb_substr($lin,10);
$e = explode('|',$e);
$flag = trim($e[1]);
$langs = trimArray(explode(',',$e[0]));
foreach($langs as $kk=>$vv)
$langs[$kk] = mb_strtolower($vv);
...
$lin = trim($a[$x]);
if( $lin=='' ) continue;
if( mb_substr($lin,0,2)=='//' ) continue;
$c = mb_substr($lin,0,1);
if( $c=='.' ) continue;
if( mb_strpos('[¿#',$c)===false ){
$savLin = $lin;
$ult = mb_strrpos($lin,'|');
$posComment = mb_strrpos($lin,'~',$ult);
if( $posComment!==false ){
$comment = trim(mb_substr($lin,$posComment+1));
$lin = mb_substr($lin,0,$posComment);
}else{
$comment = '';
}
...
foreach( $a['langs'] as $indexLng=>$cd_gs_language ){
if( SS::count("{$_ENV['SYSDB']}gs_language","cd_gs_language='{$cd_gs_language}'")==0 )
mif("No existe el lenguaje '{$cd_gs_language}' descrito en la línea {$a['line']} en el archivo: ".str_replace(mCHR92'/',$filepath) );
}
foreach( $a['langs'] as $indexLng=>$cd_gs_language ){
for( $x=1 ; $x<count($tag) ; $x++ ){
$word_id		= $tag[$x]['word_id'];
...
if( $nr==0 ){
$now = date('Y-m-d H:i:s');
$q = "insert into {$_ENV['SYSDB']}gs_transchange(cd_gs_script,cd_gs_language,word_id,word_val,word_val_md5,cdi_add,comment,type,word_val_old,word_val_md5_old,tf_changed)
values({$cd_gs_script},'{$cd_gs_language}','{$word_id}','{$word_val}','{$word_val_md5}','{$now}','{$comment}','{$type}','{$word_val}','{$word_val_md5}','{$_ENV['ON']}')";
...
SS::query("update {$_ENV['SYSDB']}gs_transchange set tf_changed='{$_ENV['ON']}', word_val='{$word_val}', word_val_md5='{$word_val_md5}', word_val_old='{$word_val}', word_val_md5_old='{$word_val_md5}', comment='{$comment}'  where cd_gs_script={$cd_gs_script} and cd_gs_language='{$cd_gs_language}' and word_id='{$word_id}'");
SS::query("update {$_ENV['SYSDB']}gs_transchange set tf_changed='{$_ENV['ON']}' where cd_gs_script={$cd_gs_script} and word_id='{$word_id}'");
...
if( $rt['tf_changed']==$_ENV['ON'] || $rt['word_val_md5']!=$rt['word_val_md5_old'] ){
SS::query("update {$_ENV['SYSDB']}gs_transchange set tf_changed='', word_val_old='{$rt['word_val']}', word_val_md5_old='{$rt['word_val_md5']}' where cd_gs_transchange={$rt['cd_gs_transchange']}",$p5);
}
}
}
}
...
if( count($w) ){
$aborrar=array();
SS::query("select cd_gs_transchange,cd_gs_language,word_id from {$_ENV['SYSDB']}gs_transchange where cd_gs_script={$cd_gs_script} and word_id not in (". implode(',',$w) .")", [], 2);
while($rt=SS::get(2)){
...
if( count($aborrar) ){
SS::query("delete from {$_ENV['SYSDB']}gs_transchange where cd_gs_transchange in(".implode(',',$aborrar).")");
}
}
