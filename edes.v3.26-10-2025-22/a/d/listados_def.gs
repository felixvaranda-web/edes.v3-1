...
$NmGsFormato = str_replace( "'", CHR92."'", trim($NmGsFormato) );
if( $NmGsFormato=='' ){
?>
...
$YaExiste = false;
if( $CdGsFormato==0 ){
$YaExiste = SS::count("{$_ENV['SYSDB']}gs_formato", "cd_gs_entidad='{$CdGsEntidad}' and nm_gs_formato='{$NmGsFormato}'");
}else{
$YaExiste = SS::count("{$_ENV['SYSDB']}gs_formato", "cd_gs_entidad='{$CdGsEntidad}' and nm_gs_formato='{$NmGsFormato}' and cd_gs_formato!='{$CdGsFormato}'");
}
...
if( $MODO=='a' && $ENTIDAD=='' ){
$_TReg = SS::count("{$_ENV['SYSDB']}gs_entidad");
if( $_TReg == 0 ){
eMessage('NO HAY NINGUNA ENTIDAD CREADA','HES');
}else if( $_TReg == 1 ){
SS::query("select cd_gs_entidad from {$_ENV['SYSDB']}gs_entidad");
$row = SS::get();
...
if( isset($_SEEK) ){
if( eSubstrCount($MODO, 'R')==0 ) $MODO = mb_strtolower($_SEEK).'R';
...
$Accion			= '<FORM accept-charset="utf-8" NAME=DATOS AUTOCOMPLETE="off" METHOD=POST ACTION="edes.php?E:$a/d/listados_def.gs&MODO=A'.$OtraDB.'">';
...
SS::query("select nm_gs_entidad from {$_ENV['SYSDB']}gs_entidad where cd_gs_entidad='{$cd_gs_entidad}'");
$row = SS::get();
...
SS::query("insert into {$_ENV['SYSDB']}gs_formato
(cd_gs_user, cd_gs_entidad, formato, ordenacion, cabecera, operacion, grupo, nm_gs_formato, titulo_list, descripcion, cd_gs_user2, cd_gs_position, cd_gs_node, cd_gs_tree, cd_scope, informe) values
'{$_User}', '{$cd_gs_entidad}', '{$formato}', '{$ordenacion}', '{$cabecera}', '{$operacion}', '{$grupo}', '{$nm_gs_formato}', '{$titulo_list}', '{$descripcion}', '{$cd_gs_user2}', '{$cd_gs_position}', '{$cd_gs_node}', '{$cd_gs_tree}', '{$cd_scope}', ''");
$Id = SS::id();
SS::query("select formato, ordenacion from {$_ENV['SYSDB']}gs_formato where cd_gs_formato={$Id}");
$r = SS::get();
$Formato = trim($r['formato']);
if( mb_substr($Formato,-1)!='#' ){
$Formato = mb_substr( $Formato, 0, mb_strrpos($Formato,'#')+1 );
SS::get("update {$_ENV['SYSDB']}gs_formato set formato='{$Formato}' where cd_gs_formato={$Id}");
}
$Ordenacion = trim($r['ordenacion']);
if( mb_substr($Ordenacion,-1)!='#' ){
$Ordenacion = mb_substr( $Ordenacion, 0, mb_strrpos($Ordenacion,'#')+1 );
SS::query("update {$_ENV['SYSDB']}gs_formato set ordenacion='{$Ordenacion}' where cd_gs_formato={$Id}");
}
...
case 'cR':
case 'mR':
$NewModo = str_replace( 'R', '', mb_strtoupper( $MODO ) );
if( $cd_gs_formato=="" ){
SS::query("select * from {$_ENV['SYSDB']}gs_formato");
$Formato = SS::get();
$cd_gs_formato = $Formato['cd_gs_formato'];
}else{
SS::query("select * from {$_ENV['SYSDB']}gs_formato where cd_gs_formato='{$cd_gs_formato}'");
$Formato = SS::get();
}
...
$Accion = '<FORM accept-charset="utf-8" NAME=DATOS AUTOCOMPLETE="off" METHOD=POST ACTION="edes.php?E:$a/d/listados_def.gs&MODO=M'.$OtraDB.'">';
...
$Accion = "<FORM accept-charset='utf-8' NAME=DATOS AUTOCOMPLETE='off' METHOD=POST ACTION='edes.php?Fc:".'$'."a/d/{$NmScript}{$OtraDB}&_ASSIGN=c&cd_gs_entidad={$cd_gs_entidad}'>";
...
$Accion = '<FORM accept-charset="utf-8" NAME=DATOS AUTOCOMPLETE="off" METHOD=POST ACTION="edes.php?E:$a/d/listados_def.gs&MODO=B'.$OtraDB.'">';
...
if( $_SESSION["_WebMaster"]==$_ENV['ON'] ) $updateUser = "";
$sTReg = SS::count("{$_ENV['SYSDB']}gs_formato", "cd_gs_formato='{$cd_gs_formato}'");
SS::query("update {$_ENV['SYSDB']}gs_formato set
{$updateUser}
...
where cd_gs_formato='{$cd_gs_formato}'"
);
...
if( mb_substr($Formato,-1)!='#' ){
$Formato = mb_substr( $Formato, 0, mb_strrpos($Formato,'#')+1 );
SS::query("update {$_ENV['SYSDB']}gs_formato set formato='{$Formato}' where cd_gs_formato={$Id}");
}
$Ordenacion = trim($r['ordenacion']);
if( mb_substr($Ordenacion,-1)!='#' ){
$Ordenacion = mb_substr( $Ordenacion, 0, mb_strrpos($Ordenacion,'#')+1 );
...
break;
case 'B':
SS::query("delete from {$_ENV['SYSDB']}gs_formato where cd_gs_formato='{$cd_gs_formato}'");
...
global $ListaCampos;
$Grupo = array();
SS::query("select cd_gs_grupo, count(*) from {$_ENV['SYSDB']}gs_campo where cd_gs_entidad='{$cd_gs_entidad}' and extraccion='{$_ENV['ON']}' and nivel>=0 and (nivel<={$_export_level_} or nivel is null) and (log_only<>'{$_ENV['ON']}' or log_only is null) group by cd_gs_grupo");
...
$TCampos = 0;
$TGrupos = 0;
SS::query( "select	 C.*,G.orden as g_orden,G.nm_gs_grupo as g_nm_grupo
from	 {$_ENV['SYSDB']}gs_campo as C, {$_ENV['SYSDB']}gs_grupo as G
where	 C.cd_gs_entidad='{$cd_gs_entidad}' and extraccion='{$_ENV['ON']}' and nivel>=0 and (C.nivel<={$_export_level_} or C.nivel is null) and (log_only<>'{$_ENV['ON']}' or log_only is null) and C.cd_gs_grupo=G.cd_gs_grupo
order by g_orden,g_nm_grupo,C.orden"
);
while( $row = SS::get() ){
$TCampos++;
...
echo ( eSubstrCount( $ListaCampos, '#'.$row['cd_gs_campo'].'#' ) > 0 ) ? 'SelectON' : 'SelectOFF';
echo " title='{$sTitle}'>";
...
$nLineas = 0;
$DimGrupo = array();
SS::query("select cd_gs_grupo,nm_gs_grupo from {$_ENV['SYSDB']}gs_grupo order by 1");
while( $r=SS::get("num") ) $DimGrupo[$r[0]] = trim($r[1]);
...
$sOrdenacion =' ';
if( eSubstrCount( $ListaOrdenacion, '#'.$DimCampo['cd_gs_campo'].'#' ) > 0 ){
...
$_TotalCampos++;
$_TotalAncho += $sAncho;
echo "<tr i={$sCodCampo} style='cursor:var(--cPointer)'>";
echo "<td class='SelectOFF' style='cursor:var(--cPointer)' id='C'>".$sOrdenacion;
echo "<td class='SelectOFF' style='cursor:var(--cPointer)' title='{$sTitulo}'>".$sEtiqueta;
...
S.eventClear(window);
<?PHP
if( mb_strtoupper($MODO[0])=='M' || mb_strtoupper($MODO[0])=='A' ){
...
<?PHP
SS::count("{$_ENV['SYSDB']}gs_campo", "cd_gs_entidad='{$cd_gs_entidad}' and extraccion='{$_ENV['ON']}' and nivel<={$_export_level_}");
if( $_TReg < 1 ){
...
<table id='SelCampos' class="col_2r" style='position:relative;top:-1px;left:-1px;cursor:var(--cPointer);'>
...
<table id='FORM' cellspacing=0px cellpadding=0px width=100% border=0px style='cursor:var(--cAuto)'>
...
<td id=TC onclick='top.eHelp("$listados_def")' style='cursor:var(--cPointer)'>Tipo de uso</td>
<td>
<INPUT TYPE="radio" NAME="cd_scope" VALUE=0<?=(($cd_scope==0)?' checked':'')?> style='border:0px;cursor:var(--cPointer)' onclick='VerUso()'> Privado&nbsp;&nbsp;&nbsp;
<INPUT TYPE="radio" NAME="cd_scope" VALUE=1<?=(($cd_scope==1)?' checked':'')?> style='border:0px;cursor:var(--cPointer)' onclick='VerUso()'> Compartido&nbsp;&nbsp;&nbsp;
<INPUT TYPE="radio" NAME="cd_scope" VALUE=2<?=(($cd_scope==2)?' checked':'')?> style='border:0px;cursor:var(--cPointer)' onclick='VerUso()'> Público&nbsp;&nbsp;&nbsp;
...
SS::query("select cd_gs_user, user_name, user_surname from {$_ENV['SYSDB']}gs_user where permission='S' order by user_name, user_surname");
while( $r = SS::get() ) echo '<option value='.$r['cd_gs_user'].(($cd_gs_user2==$r['cd_gs_user'])?' selected':'').'>'.trim($r['user_name']).' '.trim($r['user_surname']);
?></select>
</td>
</tr>
<tr id=COMPARTIR>
...
SS::query("select * from {$_ENV['SYSDB']}gs_position order by nm_gs_position");
while( $r = SS::get() ) echo '<option value='.$r['cd_gs_position'].(($cd_gs_position==$r['cd_gs_position'])?' selected':'').'>'.$r['nm_gs_position'];
?></select>
</td>
</tr>
<tr id=COMPARTIR>
...
SS::query("select cd_gs_node, nm_gs_node from {$_ENV['SYSDB']}gs_node order by nm_gs_node");
while( $r = SS::get() ) echo '<option value='.$r['cd_gs_node'].(($cd_gs_node==$r['cd_gs_node'])?' selected':'').'>'.$r['nm_gs_node'];
?></select>
</td>
</tr>
<tr id=COMPARTIR>
...
while( $r = SS::get() ) echo '<option value='.$r['cd_gs_tree'].(($cd_gs_tree==$r['cd_gs_tree'])?' selected':'').'>'.$r['nm_gs_tree'];
?></select>
</td>
</tr>
