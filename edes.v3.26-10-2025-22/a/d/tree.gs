...
SS::query( "update {$_ENV['SYSDB']}gs_op set status='{$STATUS}', cd_gs_user='{$_User}', dt_status='{$Hoy}' where cd_gs_op='{$ID}'" );
?>
...
SS::query( "update {$_ENV['SYSDB']}gs_op set subtree_opt='{$subtree_opt}' where id='{$id}' and cd_tree='{$cd_tree}'" );
}
if( $CONARBOLES==1 ){
make_Txt( $cd_tree );
...
$TTree = 0;
SS::query( "select nm_gs_tree, description from {$_ENV['SYSDB']}gs_tree order by cd_gs_tree" );
while( $r = SS::get("num") ){
$TTree++;
$VarMensa .= 'Mensa['.$TTree.'] = new Array("'.str_replace('"',"'",$r[0]).'","'.str_replace('"',"'",str_replace("\r\n",' ',$r[1])).'");';
$NomArbol[] = trim($r[0]);
}
if( $TTree == 0 || SS::count("{$_ENV['SYSDB']}gs_op", "")==0 ){
eInclude('message');
...
'<FORM accept-charset="utf-8" METHOD=POST ACTION="edes.php?E:$a/d/tree.gs" NAME="FRM1">'+
...
if( $TIPO==0 ){
?>
<table id=Header width=100% style='cursor:var(--cAuto)'>
<tr>
...
<td><img src='g/pdf.gif' onclick=eToPDF() style='cursor:var(--cPointer);margin-left:5px' title='Exportar a PDF'>
</table>
...
<td width=1px height=1px>
<table id=MENU border=0px cellspacing=0px cellpadding=0px><tr>
<td><img src='edes.php?R:$a/g/an_grabar.gif' onclick=Grabar(0) style='cursor:var(--cPointer);margin-left:5px' title='Grabar cámbios'>
<td><img src='edes.php?R:$a/g/arbol.gif' onclick=Grabar(1) style='cursor:var(--cPointer);margin-left:5px' title='Grabar cámbios y gererar árboles'>
<td><img src='g/pdf.gif' onclick=eToPDF() style='cursor:var(--cPointer);margin-left:5px' title='Exportar a PDF'>
</table>
<td width=1px style='padding-left:10px'>Arbol:
<td width=100% style='padding-left:5px;' valign=middle>
...
$OpEn = array();
SS::query("select cd_gs_op,cd_gs_tree from {$_ENV['SYSDB']}gs_tree_op");
while( $r = SS::get() ){
$OpEn[$r[0]][$r[1]] = 1;
}
$OpNo = "'U'";
...
SS::query("select * from {$_ENV['SYSDB']}gs_op where show_type not in ({$OpNo}) order by seq");
$c = 0;
while( $r = SS::get() ){
...
if( $_PrintToPDF ) $usuCursor[$nr][0] = str_repeat(mb_chr(8),$r['indent']*3).str_repeat('-',50-$r['indent']*3);
}else{
...
if( $_PrintToPDF ) $usuCursor[$nr][0] = str_repeat(mb_chr(8),$r['indent']*3).str_pad(' '.$txt.' ', 50-$r['indent']*3, "-", STR_PAD_BOTH);
}
}else{
$txt = trim($r['caption']);
if( $txt[0]=='{' ){
...
echo $txt;
if( $_PrintToPDF ) $usuCursor[$nr][0] = str_repeat(mb_chr(8),$r['indent']*3).$txt;
}
for( $n=0; $n<$TTree; $n++ ){
echo '<td>'.(($OpEn[$r['cd_gs_op']][$n+1]==1) ? 'P':'¬');
$usuCursor[$nr][$n+1] = (($OpEn[$r['cd_gs_op']][$n+1]==1) ? $_ENV['ON']:$_ENV['OFF']);
}
echo '<td>&nbsp;';
$nr++;
}
...
UnloadTabla("{$_ENV['SYSDB']}gs_tree", $TXT_generation_path.'_gs_tree_.txt');
UnloadTabla("{$_ENV['SYSDB']}gs_op"  , $TXT_generation_path.'_gs_op_.txt');
$qry = "select * from {$_ENV['SYSDB']}gs_op where cd_tree='{$cd_tree}' order by seq";
SS::query($qry);
while( $r = SS::get() ){
$ama[]=array('tipo'=>$r['type'],'indent'=>$r['indent'],'texto'=>$r['caption'],'tip'=>$r['tip'],'id'=>$r['id'],'surl'=>$r['script_url'],'subtree_opt'=>$r['subtree_opt']);
}
$indice=0;
$qry="select * from {$_ENV['SYSDB']}gs_tree where cd_tree='{$cd_tree}' order by cd_gs_tree";
SS::query($qry, [], 1);
while( $r = SS::get(1) ){
$cod=$r['cd_gs_tree'];
...
$quitar_indent_uno=0;
if( $r['rmvfstlvl'] == $_ENV['ON'] ){
$quitar_indent_uno=1;
}
$ac=count($ama);
for( $k=0; $k<$ac; $k++){
if( mb_substr($ama[$k]['subtree_opt'],$indice,1) != '1' ) continue;
...
SS::query( "SELECT * FROM {$sTABLA}" );
$fd = fopen( $File, 'w' );
$TReg = 0;
$Pipa = false;
while( $linea = SS::get("num") ){
$txt = '';
if( $Pipa ) $txt .= "\n";
$Pipa = false;
...
$valor = str_replace(CHR13.CHR10,'#%0A#%0B#',$valor);
