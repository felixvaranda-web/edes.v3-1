...
SS::query( "select cd_ts_trespuesta from ts_pregunta where cd_ts_test={$CdTest} order by orden" );
while( $r=SS::get("num") ) $DimTipoRespuesta[] = $r[0];
...
SS::query( "insert into ts_encuestado (cd_gs_user,cd_ts_encuesta,dt_encuesta) values ( {$uUser}, {$_POST['cd_ts_encuesta']}, '{$Fecha}' )" );
$ID = SS::id();
}
$nr = 0;
foreach( $DimRespuestas as $kP=>$vP ){
$Pregunta = mb_substr($kP,4);
foreach( $vP as $kR=>$vR ){
$Respuesta = mb_substr($kR,4);
$Valor = $vR;
if( $Probar ){
...
if( $DimTipoRespuesta[$nr]<>'T' ){
SS::query( "insert into ts_opinion (cd_ts_encuestado,cd_ts_encuesta,cd_ts_test,cd_ts_pregunta,cd_ts_respuesta,valor) values ( {$ID}, {$_POST['cd_ts_encuesta']}, {$_POST['cd_ts_test']}, {$Pregunta}, {$Respuesta}, '{$Valor}' )" );
}else{
if( !DB::isDriver("oci") ){
$Valor = str_replace( "'", "\\'", $Valor );
$Valor = str_replace( '"', '\\"', $Valor );
...
SS::query( "insert into ts_opinion_txt (cd_ts_encuestado,cd_ts_encuesta,cd_ts_test,cd_ts_pregunta,respuesta) values ( {$ID}, {$_POST['cd_ts_encuesta']}, {$_POST['cd_ts_test']}, {$Pregunta}, '{$Valor}' )" );
}
}
$nr++;
}
...
SS::query("select * from ts_encuesta where cd_ts_encuesta={$CdEncuesta}");
$e = SS::get();
$CdTest = $e['cd_ts_test'];
SS::query("select * from ts_test where cd_ts_test={$CdTest}");
$t = SS::get();
$Total = SS::count('ts_pregunta', "cd_ts_test={$CdTest}");
echo "<script type='text/javascript'>var _Total={$Total},_aPregunta = 1;</script>";
echo '<FORM accept-charset="utf-8" AUTOCOMPLETE="off" NAME="FRM1" METHOD="POST">';
...
$n = 1;
SS::query("select * from ts_pregunta where cd_ts_test={$CdTest} order by orden", [], 1);
while( $p = SS::get(1) ){
$DimRes = array();
...
SS::query("select * from ts_respuesta where cd_ts_pregunta={$p['cd_ts_pregunta']} order by cd_ts_pregunta,orden");
while( $r = SS::get() ){
...
Ele[n].checked = false;
Ele[n].value = S.setup.checkOff;;
}
if( Old != S.setup.checkOn ){
Obj.checked = true;
Obj.value = S.setup.checkOn;
}
}
