...
if( eSubstrCount($Dim,'$_FileLoading') == 1 ){
$i = mb_strpos($Dim,'$_FileLoading')+1;
$c = '';
$i1 = mb_strpos($Dim,"'",$i);
$i2 = mb_strpos($Dim,'"',$i);
if( $i1===false ){
...
if( $c=='' ) die('No');
$i = mb_strpos($Dim,$c, $i+1);
$f = mb_strpos($Dim,$c, $i+1);
$NewFile = '';
if( $_GET['SetLoading']!='' ){
if( mb_substr($aFile,-4)!='.swf' && mb_substr($aFile,-4)!='.gif' && mb_substr($aFile,-4)!='.jpg' && mb_substr($aFile,-4)!='.png' && mb_substr($aFile,-5)!='.jpeg' ){
$NewFile = $aFile;
}else{
$NewFile = 'g/'.$aFile;
}
}
$oFile = mb_substr( $Dim, $i+1, $f-$i-1);
eTrace( $oFile );
...
if( $NewFile!='' ){
$nFile = DIREDES.'a/g/loading/'.mb_substr($NewFile,2);
eTrace( $nFile );
...
$Dim = mb_substr($Dim,0,$i+1).$NewFile.mb_substr($Dim,$f);
...
$Dim2 = array();
$Dim = array();
$di = opendir(DIREDES.'a/g/loading/');
while( $file = readdir( $di ) ){
if( $file != '.' && $file != '..' ){
if( !is_dir(DIREDES."a/g/{$file}") ){
if( mb_strlen($file)==17 && mb_substr($file,0,7)=='loading' && mb_substr($file,9,1)=='_' && mb_substr($file,-3)=='gif' ){
$Dim[] = $file;
}else{
$Dim2[] = $file;
}
...
for( $n=0; $n<$tf; $n++ ){
list( ,$Color ) = explode('_',mb_substr($Dim[$n],0,-4));
if( $Color=='fff' ){
...
echo '</OBJECT>';
}else if( mb_substr($Dim2[$n],-4)=='.gif' && mb_substr($Dim2[$n],-4)!='.jpg' && mb_substr($Dim2[$n],-4)!='.png' && mb_substr($Dim2[$n],-5)!='.jpeg' ){
