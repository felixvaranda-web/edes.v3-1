...
if( DGI('OpEnSubVentana')!=null || event.ctrlKey || event.altKey ){
if( DGI('OpEnSubVentana').valor==1 || event.ctrlKey || event.altKey ) EnSubVentana = true;
}
...
<TH class=THOption width=1px><input name=ENCONTRADOS SIZE=3 title="Nº de ocurrencias" style="cursor:var(--cAuto);width:25px">
...
<col class=ModeCol style='width:1px;text-align:center;cursor:var(--cAuto)'><col class=Option style='width:100%'>
<?PHP
$Dim = array();
...
echo '<td id="mc'.$r['mode'].'" title="'.$_ModeLabel[$r['mode']].'">'.$_ModeOp[$r['mode']];
}
if( $r['type']=='L' ){
if( $r['caption'][0]=='-' ) $r['caption'] = trim(mb_substr($r['caption'],1));
if( $r['caption']=='' ){
