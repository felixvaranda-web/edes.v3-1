...
$_POST[$k] = str_replace('&#92;' ,'\\',$_POST[$k]);
$_POST[$k] = str_replace('&amp;' ,'&' ,$_POST[$k]);
if( mb_substr($k,0,8)=='_colsop_' && $v<>'' ) $_POST[$k] = mb_strtoupper($v);
if( mb_substr($k,0,7)=='_field_' && $v<>'' ){
if( $xFields<>'' ) $xFields .= ',';
$xFields .= $v;
}
...
}else if( mb_strlen($r[$n])==10 && eSubstrCount($r[$n],'-')==2 ){
$xTipo[$n] = 'F4';
$xFormatearF4 = true;
...
if( $_POST['_colsop_'.$n]==$_ENV['ON'] && $_POST['_th2_'.$n]<>'' ){
...
if( $xTH=='' ) $xTH = '&nbsp;'.($n+1);
if( $xDivide && eSubstrCount($xTH,' ')>0 ){
$b = 0;
$min = 99;
for( $i=0; $i<eSubstrCount($xTH,' '); $i++ ){
...
if( trim($_POST['_ReportRow'])==$_ENV['ON'] && str_replace(',','',$xCHARTROW)<>'' ) $_DimEDF[] = '[ChartRow]'.mb_substr($xCHARTROW,1)."|100\n";
