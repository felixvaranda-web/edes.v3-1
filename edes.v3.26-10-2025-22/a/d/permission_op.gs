...
switch( $Estado ){
case 'A':
SS::query( "insert into {$_ENV['SYSDB']}gs_permission_op values ( {$_User}, {$op}, '{$_ENV['OFF']}' )" );
break;
case 'P':
SS::query( "insert into {$_ENV['SYSDB']}gs_permission_op values ( {$_User}, {$op}, '{$_ENV['ON']}'  )" );
break;
case 'B':
SS::query( "delete from {$_ENV['SYSDB']}gs_permission_op where cd_gs_user={$_User} and option_id={$op}" );
break;
default:
exit;
}
...
$TTree = 0;
SS::query( "select nm_gs_tree, description from {$_ENV['SYSDB']}gs_tree where cd_tree='{$cd_tree}' order by cd_gs_tree" );
while( $r = SS::get("num") ){
$TTree++;
}
...
SS::query("select * from {$_ENV['SYSDB']}gs_op where cd_tree='{$cd_tree}' and mb_substr(subtree_opt,{$TreePersonal},1)='1' order by seq");
}else{
SS::query("select * from {$_ENV['SYSDB']}gs_op where cd_tree='{$cd_tree}' and subtree_opt[{$TreePersonal},{$TreePersonal}]='1' order by seq");
}
while( $r = SS::get() ) $uTree[$r['id']] = 1;
$uOpcion = array();
SS::query("select * from {$_ENV['SYSDB']}gs_permission_op where cd_gs_user={$_User} order by option_id");
while( $r = SS::get() ){
$uOpcion[$r['option_id']] = ( (trim($r['visible'])=='') ? 'N' : trim($r['visible']) );
}
if( SS::isDriver('mysql,mysqli') ){
SS::query("select * from {$_ENV['SYSDB']}gs_op where cd_tree='{$cd_tree}' and mb_substr(subtree_opt,1,1)='1' order by seq");
}else{
SS::query("select * from {$_ENV['SYSDB']}gs_op where cd_tree='{$cd_tree}' and subtree_opt[1,1]='1' order by seq");
}
$c = 0;
while( $r = SS::get() ){
