...
DGI("TABGroupInner").style.height = DGI("TABGroupInner").parentNode.offsetHeight+"px";
...
DGI("TABGroupInner").style.width = "1px";
DGI("TABGroupInner").style.width = DGI("TABGroupInner").parentNode.offsetWidth+"px";
if( _TABGroupEmpty=='TABHeader' ) TABGroupEmpty.style.width = (TABHeader.offsetWidth + top.eStyleWidth(S("#TABBodyForm").css("marginTop")))+"px";
}
function _TabMenu(){
var Obj = S.toTag(S.event(window), "TD", 0);
__TabMenu(Obj);
}
function __TabMenu(Obj){
var n, i=1;
...
if( DGI('HOJAzona')!=null ) DGI("HOJAzona").style.display = (_ZoneHide[i])?'none':'table-row';
if( _ZoneHide[i] ) DGI('TABBodyForm').style.height = _ZoneHideHeight[i];
DGI('TABNumber'+i).style.display = 'table-row';
if( !_WinCaption ){
...
S(window).windowCaption(_DimTITLE[i], 1);
}
S(".ISubListBODY", DGI('TABNumber'+i)).each(function(k,o){
o.style.height = S(o).attr("eHeight")+"px";
});
_nSolapa = i;
eTabResize();
...
DGI('TABNumber'+a).style.display = 'table-row';
S(".AddButton[eTabView]").none();
S(".AddButton[eTabView*=',"+a+",']").block("table");
if( _Zona && (_nSolapa==1 || a==1) ) AnulaFRM1((_nSolapa==1));
TABMenu.rows[0].cells[a*2-1].className = 'TABMenuOn';
TABMenu.rows[0].cells[_nSolapa*2-1].className = 'TABMenuOff';
if( !_WinCaption ){
if( S(".WINDOWTITLETEXT").length ){
...
eTabResize();
if( TABMenu.rows[0].cells[sa*2-1].Func ) eval(TABMenu.rows[0].cells[sa*2-1].Func)();
SetControl(_nSolapa);
xFRM = "FRM"+_nSolapa;
_eTabOnClick(_nSolapa);
}
...
,errorMessage = o.contentDocument.body.className=="BODYMESSAGE"
,idToView = (errorMessage)? "#RESPUESTA":"#BROWSE"
,hijo = S.screen(S(idToView, o.contentDocument.body).obj)
;
...
n=(n*2)-1;
if( TABMenu.rows[0].cells[n].ScriptRun!=undefined ){
eval(TABMenu.rows[0].cells[n].ScriptRun)((n+1)/2);
if( TABMenu.rows[0].cells[n].ScriptClear!=undefined ) TABMenu.rows[0].cells[n].ScriptRun = undefined;
}
}
...
TABMenu.rows[0].cells[ns*2-1].ScriptRun = Func;
if( UnaVez==undefined || UnaVez ) TABMenu.rows[0].cells[ns*2-1].ScriptClear = 1;
}
}
function AnulaFRM1( Op ){
var Hasta = DGI('TABNumber1').sourceIndex, el, i;
...
var ns =_nOpTab(vNs), Celda = TABMenu.rows[0].cells;
Celda[ns*2-1].style.display = Celda[ns*2-2].style.display = (ver) ? 'table-cell' : 'none';
Hojas[ns] = (ver) ? 1 : 0;
if( ver==2 ) PonSolapa(ns);
}
function _TabWheel(){
var  nf = xFRM.substring(3)
,Obj = S.toTag(S.event(window.event), "TABLE", 0)
