...
global $_Include, $_TmpInclude, $php_errormsg, $__eLINE__, $__iSCRIPT__, $__iniSCRIPT__, $_DEBUG, $_TRACELABEL, $_TmpPhpFile;
$__iSCRIPT__ .= $NomEti.', ';
$_Include = $NomEti;
...
if( $NomFuncCall=="" ){
$Contenido = '<'.'?PHP /'.'/ '.$NomEti.CHR10.$Contenido.' ?'.'>';
}else{
$Contenido = '<'.'?PHP '							.CHR10.
...
if( $NomFile=='' ){
$_TmpPhpFile++;
$_TmpInclude = '../_tmp/php/'.S::$_User.'_'.$_TmpPhpFile;
}else{
...
global $_Include, $_TmpInclude, $php_errormsg, $__eLINE__, $__iSCRIPT__, $__iniSCRIPT__, $_DEBUG, $_TRACELABEL;
$__iSCRIPT__ .= $NomEti.', ';
$_Include = $NomEti;
if( func_num_args()==3 ) $Long = mb_strlen(ob_get_contents());
if( $_SESSION["_Development"] && !preg_match("/FUNCTION_EXISTS/iu", $Contenido ) ){
...
$Contenido = '<'.'?PHP '						.CHR10.
'function __SelInfo__(){'	.CHR10.
$Contenido				.CHR10.
'}'							.CHR10.
'?'.'>';
$_TmpInclude = tempnam('../_tmp/php',S::$_User);
list(,$Eti) = explode('_',$NomEti);
if( $_DEBUG==2 && mb_strtoupper($Eti)==$_TRACELABEL ){
$Contenido = '<'.'?PHP '. '$__iniSCRIPT__ = "'.$_Include.'";'.' ?'.'>'."\n".$Contenido."\n";
$Dim = explode( "\n", trim($Contenido) );
...
if( mb_substr($NomFile,0,3)=="%3E" ){
$_ExeTab = true;
$NomFile = mb_substr($NomFile,3);
}
$argv = array();
...
if( eSubstrCount($Parametro,',') == 0 ){
if( $Parametro[0]=='>' ) $Parametro = trim(mb_substr($Parametro,1));
$tmp2 = $Parametro;
$_DB = $tmp2;
...
$Linea = trim($Dim[$n]);
if( $n==0 ){
if( mb_strtoupper($Linea)=='DEBUG' || mb_strtoupper($Linea)=='DEBUG:' ){
$_DEBUG = 1;
continue;
}else if( mb_strtoupper($Linea)=='SAVESQL' || mb_strtoupper($Linea)=='SAVESQL:' ){
$_DEBUG = 4;
continue;
}else if( mb_strtoupper($Linea)=='SAVESQLH' || mb_strtoupper($Linea)=='SAVESQLH:' ){
$_DEBUG = 10;
continue;
}else if( mb_strtoupper($Linea)=='TRON' || mb_strtoupper($Linea)=='TRON:' ){
eTron( 'QUERY_STRING: '.$_SERVER['QUERY_STRING'] );
$ConRastroTron = true;
continue;
}else if( mb_strtoupper($Linea)=='TRACE' || mb_strtoupper($Linea)=='TRACE:' ){
eTrace( 'QUERY_STRING: '.$_SERVER['QUERY_STRING'] );
$ConRastroTrace = true;
continue;
}
}
if( $Linea=='' || $Linea[0]=='.' || mb_substr($Linea,0,2)=='//' ) continue;
if( mb_substr($Linea,0,2)=='/'.'*' && eSubstrCount( $Linea, '*'.'/' ) == 0 ){
$EsRem = true;
}else if( $EsRem && eSubstrCount( $Linea, '*'.'/' ) > 0 ){
$EsRem = false;
$Linea = trim(mb_substr($buffer,mb_strpos($Linea, '*'.'/')+2 ));
if( $Linea=='' ) continue;
}
if( $EsRem ) continue;
if( eSubstrCount($Linea,'//')>0 ){
$Linea = trim(mb_substr($Linea,0,mb_strpos($Linea,'//')));
if( $Linea=='' ) continue;
}
if( mb_strtoupper($Linea)=='[NOTE]' ) break;
$Linea = str_replace("\t",' ',$Linea);
$Linea = str_replace('  ',' ',$Linea);
if( $ConRastroTrace ) eTrace('Original: '.$Linea );
if( $ConRastroTron ) eTron('Original: '.$Linea );
if( preg_match("/^SELECT /iu", $Linea) ){
if( !$FuenteOk ) continue;
$EsPHP = false;
$CadenaABuscar = '';
while( eSubstrCount($Linea,'{$') > 0 && eSubstrCount($Linea,'}') > 0 ){
$p = mb_strpos( $Linea, '{$' );
$tmp = mb_substr($Linea,$p,mb_strpos($Linea, '}')-$p+1);
$Variable = mb_substr($tmp,2,-1);
if( mb_substr($Variable,-1)==']' ){
list($Variable,$Indice) = explode('[',$Variable);
list($Indice) = explode(']',$Indice);
$Indice = trim($Indice);
if( $Indice[0]=="'" || $Indice[0]=='"' ) $Indice = mb_substr($Indice,1,-1);
if( $Indice=='' ) $Indice = 0;
...
}else if( preg_match("/^count /iu", $Linea) || preg_match("/^count:/iu", $Linea) || preg_match("/^count :/iu", $Linea) ){
if( !$FuenteOk ) continue;
$EsPHP = false;
$Linea = str_replace(':','',$Linea);
}else if( preg_match("/^empty /iu", $Linea) || preg_match("/^empty:/iu", $Linea) || preg_match("/^empty :/iu", $Linea) ){
if( !$FuenteOk ) continue;
$EsPHP = false;
$Linea = str_replace(':','',$Linea);
}else if( preg_match("/^put /iu", $Linea) || preg_match("/^put :/iu", $Linea) || preg_match("/^put:/iu", $Linea) ){
if( !$FuenteOk ) continue;
$Linea = str_replace('+','.',$Linea);
$EsPHP = false;
}else if( preg_match("/^clear /iu", $Linea) || preg_match("/^clear :/iu", $Linea) || preg_match("/^clear:/iu", $Linea) ){
if( !$FuenteOk ) continue;
$EsPHP = false;
}else if( preg_match("/^onchange /iu", $Linea) || preg_match("/^onchange:/iu", $Linea) || preg_match("/^onchange :/iu", $Linea) ){
if( !$FuenteOk ) continue;
$EsPHP = true;
}else if( preg_match("/^list /iu", $Linea) || preg_match("/^list :/iu", $Linea) || preg_match("/^list:/iu", $Linea) ){
if( !$FuenteOk ) continue;
$EsPHP = false;
}else if( preg_match("/^tab /iu", $Linea) || preg_match("/^tab :/iu", $Linea) || preg_match("/^tab:/iu", $Linea) ){
if( !$FuenteOk ) continue;
$EsPHP = false;
if( $_ExeTab ){
echo '<script>';
$Linea = trim(mb_substr($Linea,3));
if( $Linea[0]==':' ) $Linea = trim(mb_substr($Linea,1));
CalcVariables($Linea);
if( $ConRastroTron ){
if( eSubstrCount($Linea, '.gdf')==1 ){
...
if( eSubstrCount($Linea, '_FILTER=')>0 ){
...
if( mb_substr($script,-1)=='&' ) $script = mb_substr($script,0,-1);
if( mb_substr($script,-1)=='?' ) $script = mb_substr($script,0,-1).'&';
if( $filtro[0]=='=' ) $filtro = mb_substr($filtro,1);
if( mb_substr($filtro,-1)=='&' ) $filtro = mb_substr($filtro,0,-1);
if( $asignar[0]=='=' ) $asignar = mb_substr($asignar,1);
$asignar = str_replace(';',',',$asignar);
...
}else if( preg_match("/^function :/iu", $Linea) || preg_match("/^function:/iu", $Linea) ){
$EsPHP = false;
}else if( preg_match("/^EditField /iu", $Linea) || preg_match("/^EditField :/iu", $Linea) || preg_match("/^EditField:/iu", $Linea) ){
if( !$FuenteOk ) continue;
$EsPHP = false;
}else if( preg_match("/^NoEditField /iu", $Linea) || preg_match("/^NoEditField :/iu", $Linea) || preg_match("/^NoEditField:/iu", $Linea) ){
if( !$FuenteOk ) continue;
$EsPHP = false;
}else if( preg_match("/^HideFields /iu", $Linea) || preg_match("/^HideFields :/iu", $Linea) || preg_match("/^HideFields:/iu", $Linea) ){
if( !$FuenteOk ) continue;
$EsPHP = false;
}else if( preg_match("/^ShowFields /iu", $Linea) || preg_match("^ShowFields :/iu", $Linea) || preg_match("^ShowFields:/iu", $Linea) ){
if( !$FuenteOk ) continue;
$EsPHP = false;
}else if( preg_match("/^ShowTR /iu", $Linea) || preg_match("/^ShowTR :/iu", $Linea) || preg_match("/^ShowTR:/iu", $Linea) ){
if( !$FuenteOk ) continue;
$EsPHP = false;
}else if( preg_match("/^HideTR /iu", $Linea) || preg_match("/^HideTR :/iu", $Linea) || preg_match("/^HideTR:/iu", $Linea) ){
if( !$FuenteOk ) continue;
$EsPHP = false;
}else if( preg_match("/^ShowGrpTR /iu", $Linea) || preg_match("/^ShowGrpTR :/iu", $Linea) || preg_match("/^ShowGrpTR:/iu", $Linea) ){
if( !$FuenteOk ) continue;
$EsPHP = false;
}else if( preg_match("/^HideGrpTR /iu", $Linea) || preg_match("/^HideGrpTR :/iu", $Linea) || preg_match("/^HideGrpTR:/iu", $Linea) ){
if( !$FuenteOk ) continue;
$EsPHP = false;
}else if( preg_match("/^ShowGrpTR- /iu", $Linea) || preg_match("/^ShowGrpTR- :/iu", $Linea) || preg_match("/^ShowGrpTR-:/iu", $Linea) ){
if( !$FuenteOk ) continue;
...
if( !$FuenteOk ) continue;
$EsPHP = false;
}else if( preg_match("/^ShowTab /iu", $Linea) || preg_match("/^ShowTab :/iu", $Linea) || preg_match("/^ShowTab:/iu", $Linea) ){
if( !$FuenteOk ) continue;
$EsPHP = false;
}else if( preg_match("/^HideTab /iu", $Linea) || preg_match("/^HideTab :/iu", $Linea) || preg_match("/^HideTab:/i", $Linea) ){
if( !$FuenteOk ) continue;
$EsPHP = false;
}else if( preg_match("/language :/iu", $Linea) || preg_match("/language:/iu", $Linea) ){
global $_LANGUAGE, $_LngPublic;
$tmp = explode(':', $Linea);
$Idiomas = explode(',', str_replace(' ', '', mb_strtolower($tmp[1])));
$p = 1;
...
continue;
}else if( preg_match("/^message /iu", $Linea) || preg_match("/^message :/iu", $Linea) || preg_match("/^message:/iu", $Linea) ){
if( !$FuenteOk ) continue;
if( isset($_LANGUAGE) ){
for($i=0; $i<count($_LANGUAGE); $i++){
$Linea = str_replace($_LANGUAGE[$i][0], $_LANGUAGE[$i][1], $Linea);
}
}
foreach($_LngPublic as $k=>$v){
$Linea = str_replace($k, $v, $Linea);
}
$EsPHP = false;
}else if( preg_match("/^help /iu", $Linea) || preg_match("/^help :/iu", $Linea) || preg_match("/^help:/iu", $Linea) ){
if( !$FuenteOk ) continue;
if( isset($_LANGUAGE) ){
ePrintR("LAN:",$_LANGUAGE, "oo:", $_LngPublic);
for($i=0; $i<count($_LANGUAGE); $i++){
$Linea = str_replace($_LANGUAGE[$i][0], $_LANGUAGE[$i][1], $Linea);
}
}
...
$EsPHP = false;
}else if( preg_match("/^focus /iu", $Linea) || preg_match("/^focus :/iu", $Linea) || preg_match("/^focus:/iu", $Linea) ){
if( !$FuenteOk ) continue;
$EsPHP = false;
}else if( preg_match("/^source /iu", $Linea) || preg_match("/^source :/iu", $Linea) || preg_match("/^source:/iu", $Linea) ){
if( $FuenteYaOk ){
$FuenteOk = false;
continue;
}
$EsPHP = false;
$Linea = str_replace(' ','',$Linea);
if( preg_match("/^source:/iu",$Linea) ){
$NmVar = trim(mb_substr($Linea,7));
if( mb_substr($NmVar,0,1)=='$' ){
$Linea = 'SOURCE:'.mb_substr($Linea,7);
}else{
$Linea = 'SOURCE:,'.mb_substr($Linea,7).',';
}
}else{
$NmVar = trim(mb_substr($Linea,6));
if( mb_substr($NmVar,0,1)=='$' ){
$Linea = 'SOURCE:'.mb_substr($Linea,6);
}else{
$Linea = 'SOURCE:,'.mb_substr($Linea,6).',';
}
}
...
}else if( eSubstrCount( $Linea, ",{$_SOURCE}," ) > 0 ){
$FuenteOk = true;
$FuenteParticular = true;
}else{
$FuenteOk = false;
...
continue;
}else if( preg_match("/^elsesource /iu", $Linea) || preg_match("/^elsesource :/iu", $Linea) || preg_match("/^elsesource:/iu", $Linea) ){
$EsPHP = false;
if( $FuenteParticular ){
$FuenteOk = false;
}else{
...
continue;
}else if( preg_match("/^php/iu", $Linea) || preg_match("/^php:/iu", $Linea) || preg_match("/^php :/iu", $Linea) ){
$EsPHP = true;
continue;
}else{
if( $EsPHP ){
...
$EjecutarDesde = 0;
$Linea = $Comando[0];
if( mb_substr($Linea,-1)==')' && !isset($SelInfoWidthTab) ){
if( eSubstrCount($Linea,'(')==1 && eSubstrCount($Linea,')')==1 && mb_substr($Linea,-1) == ')' && $FuentePHP!='' ){
$_TRACELABEL = '#';
...
continue;
}else if( preg_match("/^count/iu", $Linea) ){
$oLinea = $Linea;
$Linea = preg_replace( '/count/iu', $TReg, $Linea );
if( eSubstrCount( $Linea, '=' ) == 1 ){
if( eSubstrCount( $Linea, '!' ) == 0 && eSubstrCount( $Linea, '<' ) == 0 && eSubstrCount( $Linea, '>' ) == 0 ){
$Linea = str_replace('=','==',$Linea);
}
}
if( $ConRastroTrace ) eTrace( $Linea );
...
eHTML('$selinfo.gs');
echo '<script>top.S.edes(window);</script>';
echo '</HEAD><BODY>'."\n";
...
if( $ConRastroTrace ) eTrace('> '.$Linea );
if( $ConRastroTron ) eTron('> '.$Linea );
if( preg_match("/^put /iu", $Linea) || preg_match("/^put :/iu", $Linea) || preg_match("/^put:/iu", $Linea) ){
$Linea = trim(mb_substr($Linea,3));
if( $Linea[0]==':' ) $Linea = trim(mb_substr($Linea,1));
CalcVariables( $Linea );
$Linea = trim($Linea);
...
for( $nc=0; $nc<mb_strlen($Linea); $nc++ ){
$c = mb_substr($Linea,$nc,1);
switch( $c ){
case '"':
if( $EsCadena ){
...
$Dim[$nc][0] = 'C';
$Dim[$nc][2] = mb_substr($Dim[$nc][1],1,-1);
}else{
if( $Rastro ) eTrace( $Dim[$nc][1] );
$Dim[$nc][0] = 'X';
$Dim[$nc][1] = str_replace( 'substr (', 'mb_substr(', $Dim[$nc][1] );
$oCampo = $Dim[$nc][1];
if( $ConRastroTrace ) eTrace( '2:================= oCampo: '.$oCampo );
if( $ConRastroTron ) eTron( '2:================= oCampo: '.$oCampo );
if( mb_substr( $Dim[$nc][1],0,7 ) == 'mb_substr(' ){
...
$oCampo = trim($oCampo);
if( $Hasta=='' ){
$txt .= eQuote(mb_substr($_vF[$oCampo],$Desde));
}else{
$txt .= eQuote(mb_substr($_vF[$oCampo],$Desde,$Hasta));
}
}
if( $ConRastroTrace ) eTrace( '25:================= oCampo: '.$oCampo );
if( $ConRastroTron ) eTron( '25:================= oCampo: '.$oCampo );
}else{
if( eSubstrCount($oCampo, "||")>0 ){
...
}else if( preg_match("/^ShowTab /iu", $Linea) || preg_match("/^ShowTab :/iu", $Linea) || preg_match("/^ShowTab:/u", $Linea) ){
$Linea = trim(mb_substr($Linea,7));
if( $Linea[0]==':' ) $Linea = trim(mb_substr($Linea,1));
...
}else if( preg_match("/^HideTab /iu", $Linea) || preg_match("/^HideTab :/iu", $Linea) || preg_match("/^HideTab:/iu", $Linea) ){
$Linea = trim(mb_substr($Linea,7));
if( $Linea[0]==':' ) $Linea = trim(mb_substr($Linea,1));
