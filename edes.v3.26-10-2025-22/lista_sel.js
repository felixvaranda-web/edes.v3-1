...
_WasPulse = null;
}else if( window.event==null || ( window.event.type!='click' && window.event.type!='contextmenu' ) ){
...
if( window.event!=null || el!=undefined ){
...
if( Obj.tagName=='DIV' ){
var iCode = S.eventCode(window.event);
Obj = _RowEdit;
...
function _PagNormal(){
if( window.event && S(S.event(window.event)).class("?OFF") ){
return;
}
var Obj = DGI('LstSetup');
if( Obj!=null ){
...
break;
case 'F':
Desde = DGI("HASTA").getAttribute("TPAG")*1;
break;
case '?':
...
if( _MAXRECFULL ){
if( !S.pagination(Desde) ){
return;
}
}
MaxRec = sMaxRec;
vDesde = (Desde-1)*MaxRec;
if( vDesde<0 ) vDesde = 0;
...
top.eSWIResize(window, document.body.scrollWidth);
}
if( S("#MENUTRFLOAT").length ){
S("#MENUTRFLOAT").hidden();
}
return true;
}
...
if( window.event && S(S.event(window.event)).class("?OFF") ) return;
if( DGI("HASTA")==null ){
eHideBusy();
return;
}
...
d = location.href+'',
sContext = S.url(window, "_CONTEXT");
if( ISCLI ){
var o = S("META[gsScript]").obj,
nScript = S(o).attr("gsScript"),
nOp = S(o).attr("gsOp");
if( nOp!="" ){
nScript = nScript[0]+nOp+S.mid(nScript,1,0);
}
var urlTemp = S.mid(location.href, "?", ".html")+".html";
d = S.replace(d, urlTemp, nScript);
}
if( sContext==null ){
d += "&_CONTEXT="+S(":_CONTEXT").val();
}else{
d = d.replace("&_CONTEXT="+sContext, "&_CONTEXT="+S(":_CONTEXT").val());
}
...
if( MasParametros!='' ){
document.FieldCondi.action = S.accessHref(window, window.location.href+'&'+MasParametros);
}
...
if( o.tagName!="TR" ) o = o.parentNode;
addTitle = " "+o.cells[0].innerHTML+" "+o.cells[1].innerHTML;
}
S.window('edes.php?Fa:$a/d/export_doc.edf&CONFIG='+PER+'&TIPO='+tipo+'&VIEW='+view, {title:S.lng(194)+addTitle, modal:true, maximize:false, printer:false});
}
function gsExpor(tipo){
if( window.event!=null && (window.event.ctrlKey || window.event.altKey) ){
return false;
}
S.eventClear(window);
if( tipo!='C' ){
...
var Mas = '';
if( window.event.altKey ) Mas = 'a';
if( window.event.ctrlKey ) Mas = 'c';
if( window.event.shiftLeft ) Mas = 's';
if( k==17 ) return true;
...
break;
case 112:
S.help(window, window.event);
break;
default:
if(k>=49 && k<=57 && document.activeElement.tagName!="INPUT"){
k-=48;
...
var n = xn || S.eventCode(window.event),
obj = xObj || S.event(window),
Char = String.fromCharCode(n);
if( obj.name=="MAXREC" && DGI("oMAXREC")!=null ){
...
if( _MAXRECFULL ){
_VisibleRows *= 1;
if( !S.pagination(S("#filterTR").val(), "pageSize", _VisibleRows) ){
return;
}
}
setTimeout(function(){
...
function eMarkVisited(o, v=_MarkVisited){
if( v==undefined || v=="" ){
return;
}
o.className = v;
...
_SortTD;
function SeleccionaLineaPor(){
var oTD = S.event(window);
S(oTD).menu([
["-"+S.lng(322)]
,[S.lng(323), "A", "C"]
,[S.lng(324), "E", "W"]
...
SeleccionaLinea(null, arg.arg, null, (op=="W"));
}}, oTD);
return eClearEvent();
}
function SeleccionaLinea(no, extObj=null, xSortAsc=null, withCtrl=false){
if( _Seleccionado ){
return eClearEvent();
}
var eObj = extObj ?? S.event(window);
if( DGI('UtilList')!=null && UtilList.contains(eObj) ){
return;
}
var _AUX_= _Aux,
Destino = _Destino,
...
oClick = (window.event) ? S.event(window): null;
...
if( eObj.tagName!='IMG' && eObj.name!=undefined ){
return S.eventClear(window);
}
...
SePulso();
setTimeout(function(){
var sortAsc = xSortAsc ?? !S.is("underline",S(eObj).css("textDecoration")),
type = S(eObj).attr("TD");
if( !type ){
S("#PROCESANDO",window).none();
return;
...
if( S(oBROWSE).attr("eMultiSort")!=null ){
ctrl = false;
}
if( withCtrl ){
ctrl = true;
...
,sortAsc ? "asc" : "desc"
,ctrl
,S(":DESDE").length && !_MAXRECFULL
);
if( S(":DESDE").length && !_MAXRECFULL ){
S(":DESDE").val(0);
Paginar("I");
}
S("#PROCESANDO", window).none();
...
_Seleccionado = true;
ePublic(1);
var oTH = S("#BROWSE TH[nc='"+oTDReal.cellIndex+"']");
if( oTH.attr("te")=="CHAR" ){
_Seleccionado = false;
S.ppublic(window, 1);
if( _MarkVisited!="" ) S(_oTR).class("+Visited");
if( SelecUsu ){
SelecUsu(oClick || oTDReal, oTH.attr("campo"));
}
S.ppublic(window);
eHideBusy();
return
...
if( typeof(_PERSISTENTVAR)!='undefined' ){
for(let pv=0; pv<_PERSISTENTVAR.length; pv+=2) dir += '&'+_PERSISTENTVAR[pv]+'="'+_PERSISTENTVAR[pv+1]+'"';
}
if( typeof(_SWTitle)=="undefined" ) _SWTitle = "";
var ConCtr = (window.event && (window.event.ctrlKey || window.event.altKey)) || (typeof(_InSubWin)!="undefined" && _InSubWin);
if( withCtrl ) ConCtr = true;
...
if( _WINSPLITE ){
eMarkVisited(_oTR);
var _iframeDoc = top.eGetIFrameById(window, _WINSPLITE);
if( _WINSPLITEURL!=null ){
if( /\(.*\)/.test(_WINSPLITEURL) ){
let [funcName, args] = _WINSPLITEURL.split('(')
,parsedArgs = args.slice(0, -1).split(',').map(arg => arg.trim());
window[funcName](...parsedArgs, _iframeDoc, dir, oClick);
eHideBusy();
_Seleccionado = false;
return;
}
dir = _WINSPLITEURL;
for(let name in window["_$_"]){
dir = S.replace(dir, `{$${name}}`, window["$"+name]);
}
}
_iframeDoc.location.href = S.urlAdd(dir+'&_CLOSE_=1&_PSOURCE='+_Source+((window.name=='_SUBLISTADO')? '&_WSList=1':'')+_TargetUrl);
}else if( _INSUBWIN_ ){
