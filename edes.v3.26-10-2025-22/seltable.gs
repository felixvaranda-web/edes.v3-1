...
$cdi = $_GET["eID"];
$SQL = file_get_contents("../_tmp/php/{$_ENV['user']}_{$cdi}.sql");
if($Test){
eTron("file: ../_tmp/php/{$_ENV['user']}_{$cdi}.sql");
eTron("pk: ". "_sql_".$_GET['_CC'].' - ALTO['.$_GET['ALTO'].']');
eTron("SQL:1: ".$SQL);
}
$SQL = str_replace(CHR92, "", $SQL);
...
$SQL .= ' limit '.((int)$_GET['ALTO']+1);
}elseif( SS::isDriver("informix") ){
}elseif( DB::isDriver("oci") ){
}
if($Test) eTron("SQL:2: ".$SQL);
...
<?PHP
$Buscar = $VALOR;
$Long = mb_strlen($VALOR);
if($Test){
eTron("SQL:4: ".$SQL);
eTron('Buscar['.$Buscar.'] Long['.mb_strlen($Buscar).']['.$Buscar.']');
}
if( $Long==0 ){
$TReg = 0;
$Dim = array();
$aSQL = str_replace('#', "'".$Buscar."%'", $SQL);
if($Test) eTron("SQL:5: ".$aSQL);
SS::query($aSQL);
while( $r=SS::get("num") ){
if( trim($r[0])=="" ) $r[0] = "&nbsp;";
$Dim[$TReg] = $r;
...
if( $TReg>0 ) break;
$AnularChr = true;
$Buscar = mb_substr($Buscar, 0, --$Long);
$valorOLD = mb_substr($valorOLD, 0, -1);
}else{
$TReg = 0;
$Dim = array();
break;
...
echo $r[0].'</td>';
for($n=1; $n<count($r); $n++){
echo '<td'.(($NCols>$n)?'>':' style="display:none">').$r[$n].'</TD>';
}
echo '</TR>';
$EsElPrimero = false;
}
...
if($Test) eTron('Buscar['.$Buscar.'] Long['.mb_strlen($Buscar).']['.$Buscar.']');
?>
</tbody>
</TABLE></SPAN>
