...
if( isset($CLAVE) && mb_strlen($CLAVE)==32 ) NewLP( $CLAVE );
$Debug = 0;
$HR = urldecode($HR);
if( $E==2 ){
$_Editar = 2;
...
if( eSubstrCount( $File, '=' ) > 0 || eSubstrCount( $File, '*' ) > 0 ) $Ext = '.edf';
...
if( $File != '' && eSubstrCount( $File, '.' ) == 0 && $Ext != '.' ) $File .= $Ext;
...
var _Editar = '<?= $_Editar; ?>';
var _NumBak = '<?= $NumBak; ?>';
var _Desbloquea = <?= (eSubstrCount('~AM',$_TipoUsu)==1 ) ? 'true':'false'; ?>;
function Minimizar(){
}
<?PHP
if( $_TipoUsu=='~' ){
...
<tr><td id=CargaINI valign=middle align=center><?= $_T[35]; ?></td></tr>
</table>
<?PHP
if( $_eDesLogo!='' ){
$IconoEdes = $_eDesLogo;
...
<tr><th id=gsFondoTITLE><IMG SRC="<?= gsIMG($IconoEdes); ?>" style="float:left;cursor:var(--cPointer);" onclick='window.external._DesktopFocus()' title="Ir al Desktop"><div class=gsTITLE>gsEdit&nbsp;·&nbsp;eDes</div></th></tr>
<?PHP
?>
...
if( $HR!='' ){
if( eSubstrCount('~',$_TipoUsu)==1 ) echo '<TD><IMG SRC="'.gsIMG('2_definition').'" onclick="alert(\':Sin implantar\');" TITLE="'.$_T[17].'">';
$_File = eScript( $_File );
...
if( eSubstrCount('~',$_TipoUsu)==1 && $sFile!= '' ) echo '<TD><IMG SRC="'.gsIMG('2_tip').'"		onclick="EdTip()"		TITLE="'.$_T[4].'">';
if( eSubstrCount('~',$_TipoUsu)==1 && $sFile!= '' ) echo '<TD><IMG SRC="'.gsIMG('2_tabla').'"	onclick="EdTabla()"	TITLE="'.$_T[5].'">';
if( eSubstrCount('~',$_TipoUsu)==1 && $sFile!= '' ) echo '<TD><IMG SRC="'.gsIMG('2_excel').'"	onclick="EdExcel()"	TITLE="'.$_T[6].'">';
echo '<TD id=GRUPO>&nbsp;';
}
...
if( eSubstrCount('~',$_TipoUsu)==1 ) echo '<TD><IMG SM SRC="'.gsIMG('2_almacen').'" onclick="MenuON(\'opAlmacen\')" TITLE="'.$_T[22].'">';
if( eSubstrCount('~AM',$_TipoUsu)==1 ){
...
echo '<TD onclick="_CheckSQL.fireEvent(\'onclick\')" style="cursor:var(--cPointer)" TITLE="'.$_T[42].'">Sql&nbsp;</td>';
echo '<TD><IMG id=_CheckC SRC="'.gsIMG('2_check_0').'" onclick="OnOff()" TITLE="'.$_T[43].'"></td>';
echo '<TD onclick="_CheckC.fireEvent(\'onclick\')" style="cursor:var(--cPointer)">'.str_replace(' ','&nbsp;',$_T[43]).'</td>';
echo '<TD id=GRUPO>&nbsp;';
...
$NomExt = '.'.$_User;
$LongExt = mb_strlen($NomExt)*-1;
$ScriptLocal = false;
$di = opendir( '../_tmp' );
while( $file = readdir( $di ) ){
if( ($file != ".") && ($file != "..") ){
if( mb_substr($file,$LongExt)==$NomExt ){
...
<table WIDTH=100% HEIGHT=1 cellspacing=1 cellpadding=0 border=0>
<tr><td class=InfInterprete id=HelpTitle align=center><?= $_T[59]; ?></td>
</table>
</td></tr>
<TR>
<TD id=UtilMaxiArbol height=1 width=180>
<TABLE border=0 width=100% cellspacing=0 cellpadding=0><TR>
<TD><IMG SRC="<?= gsIMG('2_b_back'); ?>" onclick="gsEditar('H');" title='<?= $_T[36]; ?>'></TD>
<?PHP  if( eSubstrCount('~AM',$_TipoUsu)==1 ){ ?>
<TD><IMG SRC="<?= gsIMG('2_h_edes'); ?>" onclick="MenuON('opHEdes2')" title='<?= $_T[21]; ?>'></TD>
<TD><IMG SRC="<?= gsIMG('2_h_lenguajes'); ?>" onclick="MenuON('opLenguaje2')" title='<?= $_T[23]; ?>'></TD>
<?PHP  } ?>
<?PHP  if( eSubstrCount('~',$_TipoUsu)==1 ) echo '<TD><IMG id=_CheckEdHelp SRC="'.gsIMG('2_check_0').'" onclick="OnOff()" TITLE=":Editar Ayuda"></td><TD onclick="_CheckEdHelp.fireEvent(\'onclick\')" style="cursor:var(--cPointer)">:Editar</td>'; ?>
...
if( trim($File)=='' ) return;
$File = eScript( $File );
$Tabla = "<TH id=0 onclick='' style='cursor:var(--cPointer)'>&nbsp; GFC &nbsp;</TH>";
$ok = true;
$n = 0;
if( file_exists($File) ){
$fd = fopen( $File, "r" );
...
if( $buffer[0] == '[' ){
$eti = explode( ']', $buffer );
$eti = mb_strtoupper(mb_substr($eti[0],1));
$_EtiGFC .= $buffer.'<br>';
if( $eti == 'NOTE' ) $ok = false;
if( $eti == 'TAB' && $ok && eSubstrCount( $buffer, '|' ) > 0 ){
$tmp = mb_substr($buffer,mb_strpos($buffer,']')+1);
if( eSubstrCount( $tmp, '|' ) == 2 ){
...
$tmp2 = trim($tmp2);
if( $tmp2[0] == '-' ) $tmp2 = mb_substr( $tmp2, 1 );
$tmp2 = trim($tmp2);
if( eSubstrCount( $tmp2, '.' ) == 0 ) $tmp2 .= '.edf';
if( $tmp2[0] != '$' ){
$gFile = "edes.php?E:".'$'."t/31.gs&HR={$tmp2}";
}else{
$gFile = "edes.php?E:".'$'."t/31.gs&HR={$tmp2}";
}
if( eSubstrCount( $tmp2, '/' ) > 0 ){
$stmp2 = mb_strtoupper( str_replace('.edf','',mb_substr( $tmp2, mb_strrpos( $tmp2,'/' )+1 ) ) );
}else{
$stmp2 = mb_strtoupper( str_replace('.edf','', $tmp2 ) );
}
$_Pagina[$n] = $gFile;
$_Nombre[$n] = $stmp2;
$n++;
...
$OldPass = mb_strtoupper(md5($GLOBALS['PHP_AUTH_PW']));
if( mb_substr($NewUser,0,2)=='C#' ) $NewUser = mb_substr($NewUser,2);
if( eSubstrCount($NewUser,'#')==1 ) list($NewUser,) = explode('#', $NewUser);
