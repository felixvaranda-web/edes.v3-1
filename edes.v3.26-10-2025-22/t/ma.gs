...
<form accept-charset='utf-8' action='edes.php?gsmain' method="POST" autocomplete="false">
...
$_SESSION["tmp"]['_PSDV'] = mb_strtoupper(md5($_POST["Pass"]));
$_SESSION["_WEB_"] = true;
$_SESSION["_Node"] = 1;
$_SESSION["_D_"] = '~';
$_ENV[SYS]['context'] = 10;
...
if( $_SESSION["_gsACCESO"]['TIPO']=='~' ) $_SESSION["_D_"] = '~';
...
echo 'if( event.ctrlKey || event.altKey ){';
...
<?PHP  if( $_SESSION["_gsACCESO"]['TIPO'] == '~' ){ ?>
...
function eLoading(){}
<?PHP  } ?>
<?PHP  if( $_SESSION["_gsACCESO"]['TIPO'] == '~' || $_SESSION["_gsACCESO"]['TIPO'] == 'A' ){ ?>
function ReInstalar(){
...
<?PHP  if( eSubstrCount('~AM',$_SESSION["_gsACCESO"]['TIPO']) == 1 ){ ?>
...
<FORM accept-charset="utf-8" METHOD=POST ACTION="" NAME="FRemoto">
...
<FORM accept-charset="utf-8" METHOD=POST ACTION="" NAME="Traducir">
