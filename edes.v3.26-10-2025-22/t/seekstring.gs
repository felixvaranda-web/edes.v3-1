...
<td style="width:100%; padding-left:10px;">Result seek string: <?=$_GET["SeekString"]?></td>
...
list($buscar, $buscar2) = explode(" >> ", $buscar);
$buscar  = trim($buscar);
$buscar2 = trim($buscar2);
$_xBuscar2 = '/('.str_replace(
array( "[", "]", "(", ")", "<", ">", "{", "}", ".", ",", ":", "-", "!", "+", "*", "^",   " ", '$'),
array("\[","\]","\(","\)","\<","\>","\{","\}","\.","\,","\:","\-","\!","\+","\*","\^", "\\s",'\$'),
$buscar2
).')/'.$sensible;
...
WOPENER.DimScript[<?=$_GET["tab"]?>] = ["RESULT:Seek String","",""];
WOPENER._ScriptActual = <?=$_GET["tab"]?>;
var obj = S("#TABCONTENEDOR", WOPENER).obj.children[0];
S("<div class='TABSCRIPT'>Seek String</div>").nodeEnd(obj);
...
global $_xBuscar, $_xBuscar2, $_ToatlFicheros, $_TotalDeOcurrecias, $_TotalMarcas, $_DirectoriesToSkip;
if( $DirBase=='../http' || mb_substr_count($DirBase, "/http/")>0 ) return;
...
SeekString("{$DirBase}/{$file}", $buscar);
}else{
$NomFile = mb_substr($DirBase.'/'.$file, 2);
if( preg_match('/^(edf|gdf|idf|fdf|ldf|sdf|sel|gs|php|inc|js|css|json|lst|ini|var|mark)$/iu', eFileType($file)) ){
$verFile = true;
...
if( $tmp[0]=="[" && (mb_strtoupper(mb_substr($tmp,0,6))=="[NOTE]" || mb_strtoupper(mb_substr($tmp,0,6))=="[EXIT]")  ) break;
}
if( preg_match($_xBuscar, $tmp) && (empty($_xBuscar2) || preg_match($_xBuscar2, $tmp)) ){
if( $verFile ){
$verFile = false;
echo "<div id=FILE>{$DirBase}/{$file}</div>";
}
preg_match_all($_xBuscar, $dim[$n], $matches, PREG_OFFSET_CAPTURE);
$tmp = str_replace("\t"," ",$dim[$n]);
$tmp = str_replace(
array(  '<'  ,   '>'  ),
array('&#60;', '&#62;'),
$tmp
);
if( mb_strlen($tmp)>200 ) $tmp = mb_substr($tmp,0,200)."...";
$txt = "";
$dim2 = preg_split($_xBuscar, $tmp);
$t = count($dim2);
for($i=0; $i<$t; $i++){
$txt .= $dim2[$i];
if( ($i+1)<$t ){
$token = $matches[0][$i][0];
$col = $matches[0][$i][1];
$txt .= "<span id=MARCA posicion={$_TotalMarcas} col={$col}>{$token}</span>";
$_TotalMarcas++;
}
}
$tmp = trim($txt);
...
if( mb_substr($tmp[0],0,12)=="Binary file " ) continue;
