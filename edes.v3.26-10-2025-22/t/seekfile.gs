...
global $_xBuscar, $_ToatlFicheros, $_DirectoriesToSkip;
if( WINDOW_OS ){
...
if( mb_substr($txt,0,10)=="Directorio" ){
$dir = mb_substr($txt,14);
}else{
if( mb_substr($txt,2,1)=="/" ){
eExplodeLast($txt, " ", $iz, $dch);
$file = str_replace($dirbase, "..", str_replace(CHR92, "/", $dir)."/".$dch);
if( eSubstrCount($file, ".NO/")==0 && eSubstrCount($file, "/_bak_/")==0 ){
$dim[] = $file;
}
}
}
