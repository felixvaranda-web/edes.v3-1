...
$GLOBALS['_DesktopTreeType'] = SETUP::$Desktop["DesktopTreeType"];
$_gsMaster = $_SESSION["_D_"];
$_gsNomUser = $_SESSION["_UserEMail"];
$_SESSION["_Development"] = ($_SESSION["_D_"]!='');
$tmp = array();
RecuperaArboles($tmp);
...
$caption = trim($tmp[$n]["caption"]);
if( empty($caption) || $caption[0]=="." ) continue;
$indentLine = $tmp[$n]["indent"]*1;
if( $indentLine > $indentDanger ){
continue;
}
$indentDanger = 9999;
$url = trim($tmp[$n]["script_url"]);
$letra = mb_substr($url,0,1);
if( $letra=="¿" ){
list($command, $url) = explode("?", $url);
$command = mb_substr($command, 1, 999);
$command = str_replace('\\"', '"', $command);
$command = str_replace("\\'", "'", $command);
$ok = eval("return({$command});");
if( !$ok ){
...
$index = str_pad($n, 4, "0", STR_PAD_LEFT);
if( mb_substr($url,0,1)==":" ){
$padre[$indent] = mb_substr($url,1);
}
...
list($sUrl) = explode("&", $sUrl);
$letra = mb_substr($sUrl,0,1);
if( $letra=="?" ){
$sUrl = mb_substr($sUrl,1);
$letra = mb_substr($sUrl,0,1);
}
if( $letra=="2" ){
$sUrl = mb_substr($sUrl,1);
...
if( $letra=="#" ){
$sUrl = "F".mb_substr($sUrl,1);
$letra = mb_substr($sUrl,0,1);
}
if( $letra=="@" ){
$sUrl = "G".mb_substr($sUrl,1);
$letra = mb_substr($sUrl,0,1);
}
if( $letra=="=" ){
$sUrl = "L".mb_substr($sUrl,1);
$letra = mb_substr($sUrl,0,1);
}
if( $letra==">" ){
$sUrl = "E:".mb_substr($sUrl,1);
$letra = mb_substr($sUrl,0,1);
}
if( $letra!="^" && !empty($sUrl) ){
...
echo "];\n";
$txt = file_get_contents('../_tmp/php/'.$_SESSION["_G_"].'menu.'.$_User).implode(CHR10, $addOptions);
file_put_contents('../_tmp/php/'.$_SESSION["_G_"].'menu.'.$_User, $txt);
if( $_GET["Development"]=="Tree" ){
$_GET["Development"] = "System";
}
?>
if( op.length==0 ){
...
if( S.getCookie()["eDesURL"]!=undefined ){
top._AccesosDirectos(1);
}
S("#_TreeUserIcon", top).block();
top.S.toDo();
}
<?PHP
if( $_GET["Development"] && file_exists("../_d_/usr/opworking.".S::$_User) ){
setcookie("eDesURL", trim(file_get_contents("../_d_/usr/opworking.".S::$_User)));
}
echo '</script>';
echo '</body></html>';
eEnd();
...
if( eSubstrCount('~AMP', $_gsMaster)==0 ){
return;
}
...
if( $_GET["Development"]=="Tree" ){
$GLOBALS["_AllLanguages"] = SS::count("{$_ENV['SYSDB']}gs_language", ["tf_translation" => $_ENV['ON']]);
if( $_gsMaster=='~' ){
RecuperaArbol2($Dir_.'t/__edes.arb', $tmp);
}
if( eSubstrCount('~A', $_gsMaster)==1 ){
RecuperaArbol2($Dir_.'t/__analista.arb', $tmp);
}
...
RecuperaArbol2($Dir_.'t/__programador.arb', $tmp);
}
}
if( $_GET["Development"]=="Personal" ){
if( eSubstrCount('~AMP', $_gsMaster)==1 ){
RecuperaArbol2('../tree/__personal.'.str_replace(' ', '', mb_strtolower($_SESSION["_UserLogin"])), $tmp);
}
}
}
...
if( eSubstrCount($NomFile, "__personal")>0 ){
$oEmail = $dim[0];
...
$indent = mb_strlen($item)-mb_strlen(ltrim($item));
$icon = "";
$item = trim($item);
...
$indent2 = mb_strlen($dim[$n+1])-mb_strlen(ltrim($dim[$n+1]));
...
"type"=>$tipoOp,
"caption"=>$item,
"script_url"=>'R:$h/'.mb_strtolower($item).".htm",
"cd_gs_op"=>0,
"tip"=>"",
"icon"=>$icon,
"show_type"=>"",
