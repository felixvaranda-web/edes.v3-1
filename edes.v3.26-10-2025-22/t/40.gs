...
for($n=0; $n<count($Dim); $n++){
$Dim[$n] = str_replace('@LNGFILE@', '_'.$_SESSION["_LANGUAGE_"], $Dim[$n]);
list($txt) = explode(REM, trim(mb_strtoupper($Dim[$n])));
$txt = trim($txt);
if( empty($txt) ){
continue;
...
echo '<td>'.$DimImgLeng[$_SESSION["_LanguageDefault"]];
}
...
echo '<tr>';
echo '<td>'.$DimImgLeng[$_SESSION["_LANGUAGE_"]];
...
echo '<br><span style="font-size:100%">TRADUCCIONES: '.str_replace($_SESSION["_LANGUAGE_"], mb_strtoupper($_SESSION["_LANGUAGE_"]), $xLenguajes).'</span>';
