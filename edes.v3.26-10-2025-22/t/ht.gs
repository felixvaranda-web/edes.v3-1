<?PHP //[_PROTECCION_]		---sin uso---
if( S::$_User==-1 ) exit;
if( $_gsTron ) eTron('{12}'.$Dir_.'t/ht.gs');
$_VerTLF = false;
if( $_SERVER['QUERY_STRING']=='BROWSE' || $_SERVER['QUERY_STRING']=='BROWSE-NEW' || eSubstrCount($_SERVER['QUERY_STRING'],'A:BROWSE')==1 ){
...
$Valor = mb_substr( $_Accion, 3 );
if( mb_substr($_SERVER['QUERY_STRING'],0,3)=='AE:' && $Valor[0] == '$' ) $Valor = $Dir_.mb_substr($Valor,1);
$Op = mb_strtoupper( mb_substr( $_Accion, 1, 1 ) );
$_Tipo = $T;
...
if( mb_substr($Valor,0,mb_strlen($Dir_)) == $Dir_ ) $Valor = '$'.mb_substr($Valor,mb_strlen($Dir_));
if( $_Rastro ) eTron('B: '.$Valor);
$Valor = eStrtr( $Valor, array('/'=>'_', '\\'=>'_', ' '=>'_') );
...
if( $_Rastro ) eTron('4');
$quitar = '.?23?mMF>';
for( $n=0; $n<mb_strlen($quitar); $n++ ) if( mb_substr($Valor,0,1) == mb_substr($quitar,$n,1) ) $Valor = mb_substr($Valor,1);
$divide = '?&';
for( $n=0; $n<mb_strlen($divide); $n++ ){
list($Valor) = explode(mb_substr($divide,$n,1),$Valor);
}
$TObjeto = '';
if( eSubstrCount($Valor,':')==1 ){
list($TObjeto,$NomFile) = explode(':',$Valor);
$Modo = mb_substr($TObjeto,1);
}else{
$NomFile = $Valor;
$Modo = '';
}
...
if( eSubstrCount($NomFile,'.')==0 && $NomFile!='' ){
switch( $TObjeto[0] ){
case '=':
case '#':
$NomFile .= '.edf';
...
if( mb_strtoupper(mb_substr( $Dim[$n],0,6))=='[HELP]' ){
list($sModo,$xNomHelp) = explode('|',mb_substr( $Dim[$n],6));
$sModo = trim($sModo);
if( $sModo == '*' || eSubstrCount( ",{$sModo},", ",{$Modo}," )==1 ){
$NomHelp = trim($xNomHelp);
if( $NomHelp == '=' ){
$NomHelp = eStrtr( $NomFile, array('/'=>'_', '\\'=>'_', ' '=>'_') );
}
...
if( $NomHelp=='' ){
if( mb_substr($NomHelp,0,mb_strlen($Dir_)) == $Dir_ ) $NomHelp = '$'.mb_substr($NomHelp,mb_strlen($Dir_));
...
if( $NomHelp[0]=='$' ){
$_File = DIREDES.'lng/help/'.mb_substr($NomHelp,1);
}else{
$_File = '../help/tip/'.$NomHelp;
}
$_File = mb_strtolower($_File);
if( $Op == 'L' ){
...
if( mb_strlen($txt) < 200 ){
$sTxt = trim(strip_tags($txt));
$sLeng = mb_strlen($sTxt);
$sTxt = str_replace('&nbsp;','',$sTxt);
$sTxt = str_replace(' ','',$sTxt);
if( mb_substr($sTxt,0,4)=='&gt;' && $sLeng==mb_strlen($sTxt) ){
$_File = '../help/tip/'.mb_substr($sTxt,4);
$fd = fopen( $_File, 'r' );
$txt = fread( $fd, filesize($_File) );
fclose($fd);
}
...
if( mb_substr($_File,-5)==".mark" ){
include_once(DIREDES."markdown.inc");
$txt = eMarkdown($txt);
}
echo $txt;
eEnd();
...
$_File = mb_strtolower($_File);
$Base = mb_substr($_File,0,mb_strrpos($_File,'/')+1).'i/'.$PLANTILLA;
$EsAyuda = ($_Tipo=='A');
if( $Op == 'L' || $Op == 'E' ){
...
$_Limpio = str_replace( CHR13,'', $_Limpio );
$_Limpio = str_replace( CHR10,'', $_Limpio );
fclose($fd);
}
$txt = $_Limpio;
}else{
...
$_Limpio = str_replace( CHR10,'', $_Limpio );
fclose($fd);
$i = mb_strpos($txt,'<'.'!-- [HelpIni] --'.'>');
$f = mb_strpos($txt,'<'.'!-- [HelpEnd] --'.'>');
$txt = mb_substr( $txt, $i+18, $f-$i-18 );
}
}
}else if( $Op == 'G' ){
if( $EsAyuda ){
...
$i = mb_strpos($txt,'<'.'!-- [HelpIni] --'.'>');
$f = mb_strpos($txt,'<'.'!-- [HelpEnd] --'.'>');
$FUENTE = mb_substr( $txt, 0,$i+18 ) .$FUENTE. mb_substr( $txt, $i+18 );
}else{
$fd = fopen( $Base.'.body', 'r' );
$_Limpio = fread( $fd, filesize($Base.'.body') );
fclose($fd);
...
$i = mb_strpos($txt,'<'.'!-- [HelpIni] --'.'>');
$f = mb_strpos($txt,'<'.'!-- [HelpEnd] --'.'>');
if( eSubstrCount( $FUENTE, '<'.'!-- [HelpIni] --'.'>' ) > 0 && eSubstrCount( $FUENTE, '<'.'!-- [HelpEnd] --'.'>' ) > 0 ){
$i2 = mb_strpos($FUENTE,'<'.'!-- [HelpIni] --'.'>');
$f2 = mb_strpos($FUENTE,'<'.'!-- [HelpEnd] --'.'>');
$FUENTE = mb_substr( $FUENTE, $i2+18, $f2-$i2-18 );
}
$FUENTE = mb_substr( $txt, 0,$i+18 ) .$FUENTE. mb_substr( $txt, $i+18 );
}
include_once( DIREDES."{$_Sql}.inc" );
$Cdi = date('Y-m-d H:i:s');
list( ,$HFile ) = explode( '/edes.v3/', str_replace(CHR92.CHR92,'/',$_File) );
$HFile = '$'.$HFile;
SS::query( "insert into {$_ENV['SYSDB']}gs_activity (cd_gs_user, cdi, script, edes, email) values ({$_User}, '{$Cdi}', '{$HFile}', '{$_ENV['ON']}', '".$_SESSION["_UserEMail"]."')");
if( mb_strlen(trim($FUENTE))==72 ){
unlink( $_File );
...
<!DOCTYPE HTML>
<HTML XMLNS:IE>
<HEAD><META http-equiv="Content-Type" content="text/html; charset=UTF-8"><TITLE>gsHTML</TITLE>
<?PHP
if( $_Tipo=='P' ){
include_once( '../_datos/config/desktop.ini' );
$NumCSS = ($_SESSION["_Desktop"]==2)?'2':'';
...
list( ,$Base ) = explode('/edes.v3/',$Base);
$Base = '$'.$Base;
...
d.Content = x;
d.CheckSpelling();
if( event.ctrlKey || event.altKey ) d.CheckGrammar();
var nx = d.Content+'';
d.Close(false);
...
'<FORM accept-charset="utf-8" METHOD=POST ACTION="edes.php?AG" NAME="FRM1" accept-charset="UTF-8">'+
...
echo "<META NAME='Copyright' CONTENT='".SETUP::$System['CopyRight']."'>";
?>
<style>
BODY {
font-family: ARIAL;
...
echo '<TH style="padding-left:10px"><img title="Genera las ayudas en Word" src="'.gsIMG('help_doc').'" onClick="GenWmb_ord()"></TH>';
...
if( $file != '.' && $file != '..' ){
if( !is_dir("$dorg/$file") && eSubstrCount($file,'.bak')==0 ) $Dim[] = $file;
}
}
sort($Dim);
...
unlink( $FileDes );
$pd = fopen( $FileDes, 'w' );
fputs( $pd, '<HTML><HEAD><META http-equiv="Content-Type" content="text/html; charset=UTF-8"><TITLE> eDes - Manual aplicación </TITLE>' );
...
fputs( $pd, 	'<TD>'.SETUP::$System['CopyRight'].'</TD>' );
...
if( eSubstrCount( trim($Arbol[$p]).'#', '~'.$Dim[$n] ) == 1 ){
fputs( $pd, ': ' );
$s = $p;
$D = mb_strlen($Arbol[$s]) - mb_strlen(ltrim($Arbol[$s]));
do{
if( $D == mb_strlen($Arbol[$s]) - mb_strlen(ltrim($Arbol[$s])) ){
...
fclose($fd);
$txt = '<TABLE'.mb_substr($txt,4,mb_strlen($txt)-10).'</TD></TR></TABLE>';
$p = mb_strpos($txt,'>')+1;
$txt = mb_substr($txt,0,$p).'<TR><TD>'.mb_substr($txt,$p);
fputs( $pd, $txt );
fputs( $pd, "<br>\n" );
}
fputs( $pd, '</BODY></HTML>' );
...
$Valor = trim($Op);
if( $Valor == '' ) continue;
if( mb_substr($Valor,-1) == ':' ){
$t = ( mb_strlen($Arbol[$p]) - mb_strlen(ltrim($Arbol[$p])) ) - 1;
for( $i=$p-1; $i>=0; $i-- ){
if( $t == mb_strlen($Arbol[$i]) - mb_strlen(ltrim($Arbol[$i])) ){
list(,$Op ) = explode('|',$Arbol[$i]);
list( $Op ) = explode('~',$Op);
$Op = trim($Op);
$Valor .= mb_substr($Op,1);
break;
}
}
}
