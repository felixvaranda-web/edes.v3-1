...
$cdi	 = date('Y-m-d H:i:s');
$oScript = "/_datos/config/core{$_Sufijo}.css";
$tipo	 = "C";
...
for($n=0; $n<count($fileCompactar); $n++){
$text = compactarCss($cssTemplatePath.$fileCompactar[$n]);
grabarFichero("css/".$fileCompactar[$n], $text);
}
$cdi = "";
$campo = "text_".$_SESSION["_LANGUAGE_"];
$where = "";
if( $_GET['e_cdi_lng']!="" ){
$where = "where cdi>'{$_GET['e_cdi_lng']}'";
}
SS::query("select * from {$_ENV['SYSDB']}gs_storage {$where} order by cdi");
while($r = SS::get()){
if( $r['type_storage']=="c" ){
