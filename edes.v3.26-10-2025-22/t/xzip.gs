...
unlink('../../edes.v3.motor/edesweb/web/edesweb/_d_/cfg/ed.lp');
}
if( $Peticiones==1 ){
...
$_SqlPassword = 'felix';
$_SqlDiccionario = 'e_gesoft';
SS::update('descarga', ["ok" => "S", "cdi_gen" => date('Y-m-d H:i:s')], ["cd_descarga" => $IdReg]);
SS::close();
...
$error = LicenciaCrear( 'x', 'edes.v3', true );
...
chdir( '../../edes.v3.motor' );
exec( "edes.v3\win\zip -9 -r {$FileZip} edes.v3/".'*' );
$error = '';
passthru( "edes.v3\win\zip -T {$FileZip}", $error );
...
$error = BackupFuentes( 'z', 'edes.v3.motor', false );
Pie( true, $error );
exit;
...
if( $buffer[0].$buffer[1] == REM ) continue;
if( $NomFile == 'main.gs' ){
if( eSubstrCount( $buffer, "<META NAME='Generator' CONTENT='gsEdit " ) == 1 ){
$i = mb_strpos( $buffer, "<META NAME='Generator' CONTENT='gsEdit " );
$f = mb_strpos( $buffer, "'>", $i )+2;
$buffer = str_replace( mb_substr($buffer,$i,$f-$i ), "<META NAME='Generator' CONTENT='gsEdit ".date('y.m')."a'>", $buffer );
}
}
if( $TIPO_VERSION=='D' && $NomFile == 'login.gs' ){
if( eSubstrCount( $buffer, '$_i_ = -1;' ) == 1 ){
$buffer = str_replace( '$_i_ = -1;', '$_i_ = 1;', $buffer );
}
}
if( $NomFile == 'edes.php' ){
if( eSubstrCount( $buffer, "'#nSerie#'" ) > 0 ){
...
error_log(	'Fecha....: '.date('Y-m-d H:i:s').CHR10.
'Versión..: '.$_VERSION_.CHR10.
'S.O. ....: '.(( $LINUX ) ? 'LINUX':'WINDOWS').CHR10.
'KeyCode..: '.round($NumLicencia*(pow(2,6)*3*643)).CHR10.
'Usuario..: '.$USUARIO.CHR10.
'MacAddres: '.$MACKADRESS.CHR10.
'EMail....: '.$EMAIL.CHR10.
CHR10, 3, $FileLicencias );
}
}
$pos = mb_strpos( $buffer, "\t".'//' );
if( $pos !== false ){
$buffer = trim(mb_substr( $buffer, 0, $pos ));
$sBuf = trim(mb_substr( $buffer, 0, $pos ));
if( eSubstrCount( '*;:{}', mb_substr( $sBuf, mb_strlen( $sBuf )-1 ) ) == 1 ){
if( mb_strtoupper(mb_substr( $sBuf, mb_strlen( $sBuf )-5 )) != 'HTTP:' && mb_strtoupper(mb_substr( $sBuf, mb_strlen( $sBuf )-6 )) != 'HTTPS:' ){
$buffer = trim(mb_substr( $buffer, 0, $pos ));
}
}
}
$pos = mb_strpos( $buffer, '/'.'*' );
if( $pos !== false ){
$buff = mb_substr( $buffer, 0, $pos );
$pos = mb_strpos( $buffer, '*'.'/' );
while($pos === false && !feof($fdo)){
$buffer = fgets( $fdo, 4096 );
$pos = mb_strpos( $buffer, '*'.'/' );
}
$buffer = $buff.' '.mb_substr( $buffer, $pos+2 );
$buffer = trim( $buffer );
if( empty($buffer) ) continue;
}
$pos = mb_strpos( $buffer, '<'.'!--' );
if( $pos !== false && ( mb_substr($buffer,4,1)==' ' || mb_ord(mb_substr($buffer,4,1))==CHR10 ) ){
$buff = mb_substr( $buffer, 0, $pos );
$pos = mb_strpos( $buffer, '--'.'>' );
while($pos === false && !feof($fdo)){
$buffer = fgets( $fdo, 4096 );
...
$buffer = $buff.' '.mb_substr( $buffer, $pos+3 );
$buffer = trim( $buffer );
if( empty($buffer) ) continue;
}
fwrite( $fdd, $buffer.CHR10 );
}
fclose( $fdo );
fclose( $fdd );
if( $NomExt == '.js.' ){
...
if( mb_substr($file,0,2)=='__' && mb_substr($file,-4)!='.arb' ) @unlink( "$Dir/$file" );
}
}
}
closedir( $di );
...
if( mb_substr($file, mb_strrpos($file,'.')+1) == 'css' ){
echo "<TR><TD id=0>".$file;
$fichdest = "$dorg/$file";
$fdo = fopen( $fichdest, 'r' );
$buffer = fread( $fdo, filesize($fichdest) );
...
$Basura = mb_ord(mb_substr($cTxt,0,1));
...
?>
<!DOCTYPE HTML>
<HTML><HEAD><META http-equiv="Content-Type" content="text/html; charset=UTF-8"><TITLE> Para Generar la Versión eDes con el ENCODER </TITLE>
<style>
BODY {
BACKGROUND: #f2f2f2;
FONT-FAMILY: ARIAL;
