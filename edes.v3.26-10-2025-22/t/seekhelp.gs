...
if( preg_match('/^['.$dimBuscar[$i].']$/u', $dim[$n]) ){
$dim[$n] = "[".$dimBuscar[$i]."]";
}
}
}
...
if( eSubstrCount($DirBase, "/http/")>0 ) return;
if( eSubstrCount($DirBase, "/_")>0 ) return;
...
if( preg_match('/^(htm)$/iu', eFileType($file)) ){
$verFile = true;
...
if( $verFile ){
$verFile = false;
echo "<div id=FILE>".mb_substr($file,0,-4)."</div>";
}
...
echo "<div id=LINEA{$rem} mas=".(mb_strlen($dim[$n])-mb_strlen(ltrim($dim[$n]))).">".($n+1).": ".$tmp.'</div>';
$_TotalDeOcurrecias++;
}
}
}
