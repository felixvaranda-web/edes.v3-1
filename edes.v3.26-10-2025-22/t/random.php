...
$tmp2 = preg_split('/(<>|<=|>=|!=|<|>|===|==|=)/u', $tmp[3], null, PREG_SPLIT_DELIM_CAPTURE);
if( !is_numeric($where) ) $where = "'{$where}'";
$where = " and ".$tmp2[0].$tmp2[1].$where;
}else if( $tmp[3]!="" ){
if( eSubstrCount($tmp[3], '$')>0 ){
...
SS::query("select {$tmp[1]} from {$tmp[2]} where {$tmp[1]} is not null and {$tmp[1]}>'' {$where} order by rand() limit 1");
die( SS::get("num")[0] );
}
}
}
