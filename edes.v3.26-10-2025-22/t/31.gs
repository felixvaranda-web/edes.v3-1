...
$txt = trim($dim[$n]);
if( preg_match('/^(\[NOTE\]|\[EXIT\])/iu', $txt) ) break;
if( preg_match('/^(\[DBTABLE\])/iu', $txt) ){
$padreDBTable = trim(explode("|",str_replace("]","|",$txt))[1]);
}
if( preg_match('/^(\[DBINDEX\])/iu', $txt) ){
$padreDBIndex = trim(explode("|",str_replace("]","|",$txt))[1]);
}
if( preg_match('/^(\[DBSERIAL\])/iu', $txt) ){
...
$pos = mb_strrpos($scriptPadre, "/");
eTrace($pos);
if( $pos===false ){
$pre = "";
}else{
$pre = mb_substr($scriptPadre, 0, $pos+1);
}
$scriptSubWin = str_replace("\\", "/", $scriptSubWin);
$pos = mb_strrpos($scriptSubWin, "/");
eTrace($pos);
if( $pos===false ){
}else{
$scriptSubWin = mb_substr($scriptSubWin, $pos+1);
}
...
if( trim($Fila[3])=="S" ) $Fila[1] = trim($Fila[1]).":".mb_substr($Fila[1],1);
$Field[] = $Fila;
if( $scriptSubWin=="FormOnLine" ){
...
$txt = "";
for($n=0; $n<count($Dim); $n++){
if( preg_match('/^(\[FormStatic\])/iu', trim($Dim[$n])) ){
continue;
}
if( preg_match('/^(\[DBSerial\])/iu', trim($Dim[$n])) ){
continue;
}
if( preg_match('/^(\[Fields\])/iu', trim($Dim[$n])) ){
$txt .= "[FormStatic] [__{$TABLA}] | ".implode(",", $dimListCampos)." | | | | [u][d][v]\n\n";
}
$tmp = explode("|", $Dim[$n]);
if( trim($tmp[1])==$padreDBIndex ) $Dim[$n] = ".".mb_substr($Dim[$n],1)."\t\t/"."/ puntero al padre";
$txt .= $Dim[$n]."\n";
}
...
$HR = mb_substr($Restore,0,mb_strrpos($Restore,'.'));
$MasInfo = 'alert("'.$_T[63].'");';
}
}
if( isset($HELP) ){
list( $HELP, ) = explode('.',$HELP);
$Fichero = $Dir_.'h/'.mb_strtolower(trim($HELP)).'.htm';
$fd = fopen( $Fichero, 'r' );
...
$txt = str_replace( '<body ', "<body style='margin:5px' ", $txt );
echo $txt;
EnviaGZip(1,0);
exit;
}
...
$Fichero = '../_tmp/edes_'.mb_strtolower(mb_substr($OP,1)).'.'.$_User;
...
echo "txt = txt.replace(/&#10;/g, '&#10;');";
echo "top.i".mb_substr($OP,1).".document.all.DocX.Text = txt;";
echo "top.Ver('i".mb_substr($OP,1)."');";
echo '}';
echo '</SCRIPT>';
echo '</HEAD><BODY onload="UnZip();" style="display:none">';
echo '<FORM accept-charset="utf-8" METHOD=POST ACTION="" NAME="FRM1">';
echo '<TEXTAREA NAME="FUENTE">'.$Todo.'</TEXTAREA>';
echo '</FORM></BODY></HTML>';
exit;
}
if( $OP=='HTM' || $OP=='SQL' ){
$Fichero = '../_tmp/edes_'.mb_strtolower($OP).'.'.$_User;
if( $NOMSQLHTM!='' ) $Fichero = '../_tmp/'.$NOMSQLHTM.'.'.$_User;
$pnt = fopen( $Fichero, 'w' );
...
if( mb_substr($FUENTE,0,5)=='//C:'.CHR13 ){
...
if( mb_substr($FUENTE,0,5)=='#SQL'.CHR13 ){
$FUENTE = trim(mb_substr($FUENTE,5));
}
...
$Todo = fread( $fd, filesize($Fichero) );
fclose($fd);
if( $_SESSION["_gsACCESO"]['LOGEAR'] > 0 ) gsLogear( 'PHP','L', $Fichero.' '.mb_strlen($Todo) );
$Todo = str_replace( '&nbsp;','&nbsp;', $Todo );
$Todo = str_replace( '&#46;','&#46;', $Todo );
$Todo = str_replace( '&#44;','&#44;', $Todo );
$Todo = str_replace( '&#35;','&#35;', $Todo );
...
$Todo = str_replace( '&#13;' ,'&#13;' , $Todo );
$Todo = str_replace( '&#10;' ,'&#10;' , $Todo );
$nFUENTE = str_replace( CHR13 ,'' , $Todo );
if( mb_strlen($nFUENTE) < 50 ){
$EstaRelleno = str_replace(' ','',str_replace( CHR10,'' ,$nFUENTE ));
if( mb_strlen($EstaRelleno)==4 ) exit;
}
if( $NOMPHP=='' ){
...
$_SeGrabo = false;
if( isset($Fichero) && mb_substr($Fichero,0,10)=='PROCEDURE-' ){
...
$oDoc = '';
if( $_SESSION["_gsACCESO"]['LOGEAR'] > 0 ) gsLogear( 'ED','W', $xFichero.' '.mb_strlen($Doc).'/'.(mb_strlen($Doc)-filesize($xFichero) ) );
gsActivity( $xFichero );
$df = fopen($xFichero,'r');
$oDoc = fread($df,filesize($xFichero));
fclose($df);
if( SS::count( 'sysprocedures', "procname='".$Fichero."'" ) > 0 ) {
SS::query( 'drop procedure '.$Fichero );
}
$_DEBUG = 30;
if( SS::query( $Doc ) == -1 ){
if( $oDoc != '' ) SS::query( $oDoc );
...
gzwrite($zp, CHR13.CHR10.str_repeat('·',75).' '.date('Y-m-d H:i:s').CHR13.CHR10 );
}
$df = fopen($xFichero,'r');
$oDoc = fread($df,filesize($xFichero));
gzwrite( $zp, $oDoc );
...
}else if( isset($Fichero) && mb_substr($Fichero,0,8)=='TRIGGER-' ){
$Doc = str_replace( '+','%2B', $Doc );
$Doc = urldecode($Doc);
$Doc = str_replace( '&nbsp;','&nbsp;', $Doc );
$Doc = str_replace( '&#46;' ,'&#46;' , $Doc );
...
$oDoc = fread($df,filesize($xFichero));
fclose($df);
if( SS::count( 'systriggers', "trigname='".$Fichero."'" ) > 0 ) {
SS::query( 'drop trigger '.$Fichero );
}
$_DEBUG = 30;
if( SS::query( $Doc ) == -1 ){
if( $oDoc != '' ) SS::query( $oDoc );
...
if( $Fichero[0]=='>' ) $Fichero = mb_substr($Fichero,1);
$OriFile = $Fichero;
$_Directorio = $D;
$Fichero = eScript( $Fichero, $BakFile, $FicheroEdes );
$File = $Fichero;
if( mb_substr($Fichero,0,13) == '../_doc_/edf/' ) CrearDirectorios( $Fichero );
$_SeGrabo = true;
$Editar = 1;
$Arbol = 1;
$SeHaGrabado = true;
...
$txt = fputs( $fd, $txt );
fclose( $fd );
if( $Fichero!='../_tmp/__tron.'.$_User && eSubstrCount($File,'../_doc_/')==0 ){
...
if( '[LockFile]' != mb_substr($Bloque,0,10) ){
gzwrite($zp, CHR13.CHR10.str_repeat('·',75).' '.date('Y-m-d H:i:s').CHR13.CHR10 );
}else{
gzwrite($zp, CHR13.CHR10.str_repeat(mb_chr(47).CHR92,29).' No actualizado = '.date('Y-m-d H:i:s').CHR13.CHR10 );
}
}
gzwrite($zp, $txt );
gzclose($zp);
}
if( eSubstrCount('~AM',$_TipoUsu)==0 && '[LockFile]' == mb_substr($Bloque,0,10) ){
if( eSubstrCount( $Bloque, '('.$_User.')' ) == 0 ){
ShowMensaje( "ERROR: Fichero bloqueado por el usuario \"{$_User}\".", false );
}
}
if( eSubstrCount( $Fichero, '../_doc_/' ) > 0 || ( eSubstrCount( $Fichero.'#', '../_datos/config/sql.ini#' ) > 0 && !$EsPhp ) ){
$Long = mb_strlen(gzuncompress($txt));
}else if( ( eSubstrCount( $Fichero, '/edes.v3/t/__' ) > 0 && eSubstrCount( $Fichero, '.arb' ) > 0 ) || eSubstrCount( $Fichero, '/tree/__' ) > 0 ){
$Long = mb_strlen(gzuncompress($txt));
}
$txt = '';
clearstatcache();
...
$Doc = str_replace( '&#10;' ,'&#10;' , $Doc );
$Doc = str_replace( CHR13 ,'' , $Doc );
$EsPhp = ( mb_substr(trim($Doc),0,2)=='<'.'?' );
if( !$EsPhp && $FORMATEAR==1 ) $Doc = FormateaTexto( $Doc );
if( eSubstrCount( $Fichero, '../_doc_/' ) > 0 || ( eSubstrCount( $Fichero.'#', '../_datos/config/sql.ini#' ) > 0 && !$EsPhp ) ){
fputs( $pnt, gzcompress($Doc,1) );
}else if( ( eSubstrCount( $Fichero, '/edes.v3/t/__' ) > 0 && eSubstrCount( $Fichero, '.arb' ) > 0 ) || eSubstrCount( $Fichero, '/tree/__' ) > 0 ){
$Doc = trim(mb_substr( $Fichero, mb_strrpos($Fichero,'/')+1))."\n".$Doc;
fputs( $pnt, gzcompress($Doc,1) );
}else{
fputs( $pnt, $Doc );
}
fclose( $pnt );
clearstatcache();
if( $_SESSION["_gsACCESO"]['LOGEAR'] > 0 ) gsLogear( 'ED','W', $Fichero.' '.mb_strlen($Doc).'/'.(mb_strlen($Doc)-$Long) );
gsActivity( $Fichero );
...
if( $FORMATEAR != 1 ){
$tmp = explode('/',$Fichero);
if( mb_substr($Fichero,0,19)=='../_d_/usr/apuntes.' ){
...
if( $File[0]=='>' ) $File = mb_substr($File,1);
if( $File == '/tree/master.txt' && eSubstrCount('~AM',$_TipoUsu)==0 ) $Editar = 0;
if( mb_substr($File,0,7+mb_strlen($_SESSION["_PathCSS"])) == "/http/".$_SESSION["_PathCSS"]."/" && eSubstrCount('~AM',$_TipoUsu)==0 ) $Editar = 0;
if( mb_substr($File,0,15) == '/_datos/config/' && eSubstrCount('~AM',$_TipoUsu)==0 ) $Editar = 0;
$File = eScript( $File );
if( mb_substr($OriFile,0,15) == '/_datos/config/' && eSubstrCount('~AM',$_TipoUsu)==0 ) $Editar = 0;
...
$Ext = '.';
if( eSubstrCount( $File, '#' ) > 0 ) $Ext = '.edf';
if( eSubstrCount( $File, '@' ) > 0 ) $Ext = '.gdf';
if( eSubstrCount( $File, '=' ) > 0 ) $Ext = '.edf';
if( $Ext == '.' ) $Ext = mb_substr( $File, mb_strrpos( $File,'.' )+1 );
if( eSubstrCount( $File, ':' ) > 0 ) $File = mb_substr( $File, mb_strpos( $File, ':' )+1 );
if( eSubstrCount( $File, '&' ) > 0 ) $File = mb_substr( $File, 0, mb_strpos( $File, '&' ) );
if( eSubstrCount( $File, '?' ) > 0 ) $File = mb_substr( $File, 0, mb_strpos( $File, '?' ) );
$File = trim($File);
if( $Debug ) echo '<br>3|'.$File.'|';
if( eSubstrCount( $File, '.' ) == 0 ) $File .= $Ext;
if( $Debug ) echo '<br>4|'.$File.'|'.eSubstrCount( $File, '>' );
...
$Comprimido = ( eSubstrCount( $File, '../_doc_/edf/' ) == 1 );
...
$Todo = "\n  ".str_repeat( '-', mb_strlen($fd))."\n  {$fd}\n  ".str_repeat( '-', mb_strlen($fd));
}else{
if( mb_substr($OriFile,0,10)=='PROCEDURE-' ){
$Todo = LeerInformixProcedure(mb_substr($OriFile,10));
}else if( mb_substr($OriFile,0,8)=='TRIGGER-' ){
$Todo = LeerInformixTrigger(mb_substr($OriFile,8));
}else if( $File!="../d/" ){
eTron("::>> file:  {$File}");
$fd = fopen( $File, 'r' );
...
gzclose($zp);
$File = str_replace('../_bak_','',mb_substr($File,0,mb_strrpos($File,'.')));
$OriFile = $File;
}
$_CDI = date('Y-m-d H:i:s',filemtime($File));
if( ( eSubstrCount( $File, '/edes.v3/t/__' ) > 0 && eSubstrCount( $File, '.arb' ) > 0 ) || eSubstrCount( $File, '/tree/__' ) > 0 ){
$Todo = gzuncompress( $Todo );
$Todo = mb_substr( $Todo, mb_strpos($Todo,"\n") + 1 );
}
if( eSubstrCount( $File.'#', '../_datos/config/sql.ini#' ) > 0 ){
if( mb_substr($Todo,0,2) != '<'.'?' ) $Comprimido = true;
}
if( $Debug ) exit;
...
if( $_SESSION["_gsACCESO"]['LOGEAR'] > 0 ) gsLogear( 'ED','W', $File.':'.$TABLA.' '.mb_strlen($Todo).'/-'.mb_strlen($Todo) );
}
...
$_LoadIni = '';
if( eSubstrCount( mb_strtoupper($Todo), '[LOADINI]' ) > 0 && !$_SeGrabo && eSubstrCount( $File, '.lng' ) == 0 ){
$n = mb_strpos( mb_strtoupper($Todo), '[LOADINI]' );
if( mb_substr($Todo,$n-1,1)!="'" && mb_substr($Todo,$n-1,1)!='"' ){
$tmp = explode( CHR10, mb_substr($Todo,$n,200) );
$tmp[0] = str_replace(' ','',$tmp[0]);
$tmp[0] = str_replace(mb_chr(9),'',$tmp[0]);
$tmp = explode( ']', $tmp[0] );
$tmp = explode( ',', $tmp[1] );
...
<?PHP
if( isset($FORMATEAR) && $FORMATEAR==1 ){
}
?>
</SCRIPT>
<?PHP
if( $_SESSION["_Development"] || $_SESSION["_D_"]!='' ) echo "<SCRIPT type='text/javascript' SRC='".gsJS('desgrana')."'></SCRIPT>";
...
echo '<FORM accept-charset="utf-8" NAME="FRM1" METHOD="POST" ACTION="edes.php?E:$t/31.gs">';
...
if( eSubstrCount( $File, '.doc' ) == 0 ){
$READONLY = ( $Editar == 0 ) ? ' READONLY' : '';
if( !isset($ANCHO) ) $ANCHO = "auto";
if( !isset($ALTO) ) $ALTO = "auto";
...
var Dir = '<?= str_replace( '../','/',mb_substr($File,0,mb_strrpos($File,'/')+1) ); ?>';
...
function UnZip(){
<?PHP
if( eSubstrCount( $File, '../_doc_/edf/' ) > 0 ){
if( mb_strlen(trim($Todo))==0 ){
...
echo "txt = txt.replace(/&#13;/g, '&#13;');";
echo "txt = txt.replace(/&#10;/g, '&#10;');";
$tmp = explode('.',mb_strtoupper($OriFile));
$l = '';
...
if( isset($nFrame) && $nFrame == 0 && $nFrame!='' ) echo 'top.VerFuente(1);';
if( isset($MasInfo) && $MasInfo!='' ) echo $MasInfo;
?>
top.LineasDeColor( DocX );
<?PHP
if( $_LoadIni!='' ) echo $_LoadIni;
if( isset($EsPhp) && !$EsPhp && isset($FORMATEAR) && $FORMATEAR==1 ) echo 'top.RestoreLinea()';
?>
}
</SCRIPT>
...
if( mb_substr($FilePrivado,0,11) == $GLOBALS['Dir_'] ) return;
...
if( eSubstrCount( $NomFile, 'edes.php?' ) == 1 ){
...
if( $NomFile=='' ) break;
if( eSubstrCount( $NomFile, '.' ) == 0 ){
if( mb_substr($sNomFile,0,9)=='edes.php?G' ){
$NomFile .= '.gdf';
}else if( mb_substr($sNomFile,0,9)=='edes.php?F' || mb_substr($sNomFile,0,9)=='edes.php?L' ){
$NomFile .= '.edf';
}else{
$NomFile .= '.php';
}
...
global $_User;
return mb_substr( $FilePrivado,0,mb_strrpos( $FilePrivado,'.'))."_u{$_User}".mb_substr( $FilePrivado,mb_strrpos( $FilePrivado,'.'));
}
...
if( ($file != '.') && ($file != '..') ){
if( !is_dir($dorg.'/'.$file) && mb_substr($file,mb_strrpos($file,'.')+1) == $_User ){
unlink( $dorg.'/'.$file );
}
}
}
...
foreach( $Leer as $valor ){
if( eSubstrCount( $valor, "_u{$_User}." ) > 0 ){
...
global $_T;
if( eSubstrCount( $Dim[$n], '$' ) > 0 || eSubstrCount( $Fichero, '/edes.v3/' ) > 0 ) die('<script type="text/javascript">alert("'.$_T[80].'");</SCRIPT>');
...
if( mb_substr($_PathVersion,-1)!='/' ) $_PathVersion .= '/';
...
if( $Tmp == 'debug' ){
if( file_exists( '../_tmp/log/sql.'.S::$_User ) ){
$Dim = file( '../_tmp/log/sql.'.S::$_User );
for( $n=0; $n<count($Dim); $n++ ){
$txt = htmlentities(chop($Dim[$n]));
$txt = str_replace(' ','0',mb_substr($txt,0,8)).mb_substr($txt,8);
...
if( $uExt!='htm' && $usu == mb_substr($file,0,mb_strlen($usu)) && ( $Tmp=='' || mb_strstr($file,$Tmp) ) && date('Y-m-d')==date('Y-m-d',filemtime($dorg.'/'.$file)) ){
$Dim = file( $dorg.$file );
...
$Color = '#001296';
$Sg = '1000';
if( mb_substr( $txt,0,6)=='ERROR:' ){
$Color = '#CC0000';
$Sg = '5000';
}
if( mb_substr( $txt,0,8)=='GRABADO:' ){
}
$txt = str_replace('-','&#8722;',$txt);
?>
<SCRIPT type="text/javascript">
...
<?PHP
$DirRoot = eGetCWD();
if( mb_substr($DirRoot,-1) != '/' ) $DirRoot .= '/';
$DirRoot = str_replace( '/http/', '.file/', $DirRoot );
$File = str_replace( $DirRoot, '//', $File );
if( $sCDI!='' && eSubstrCount($File,'../_doc_/')==0 ) echo "top.PutCDI('{$sCDI}','{$File}');";
...
<?PHP
if( $it ){
echo "<BODY scroll=auto onhelp='return false' oncontextmenu='return false'>";
}else{
echo '</HEAD><BODY>';
}
DBTable::viewStructureTables($VerSQL);
if( $Multi ){
?>
<SCRIPT type="text/javascript">
...
echo '</BODY></HTML>';
if( $GeneraSentencia ){
$GenSQL[0] = trim(mb_strtolower($GenSQL[0]));
...
SS::query("select b.procname, a.data, a.seqno from sysprocbody a, sysprocedures b where a.procid=b.procid and a.datakey='T' and b.procname='{$NomProce}' order by 1,3" );
$txt = '';
while( $row=SS::get("num") ) $txt .= $row[1];
return $txt;
}
function LeerInformixTrigger( $NomTrig ){
SS::query( "SELECT c.tabname,b.trigname, a.seqno, a.data FROM systrigbody a, systriggers b,systables c WHERE b.trigname='{$NomTrig}' and a.datakey = 'D' AND a.trigid = b.trigid and b.tabid = c.tabid ORDER by 1,2,3" );
$txt = '';
while( $row=SS::get("num") ) $txt .= $row[3];
$txt .= "\n";
SS::query( "SELECT c.tabname,b.trigname, a.seqno, a.data FROM systrigbody a, systriggers b,systables c WHERE b.trigname='{$NomTrig}' and a.datakey = 'A' AND a.trigid = b.trigid and b.tabid = c.tabid ORDER by 1,2,3" );
while( $row=SS::get("num") ) $txt .= $row[3];
return $txt;
}
...
$_DimFields[] = mb_strtolower(OCIResult($_Result,'COLUMN_NAME'));
