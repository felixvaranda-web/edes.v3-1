...
$_lngToTranslate = isset($_GET["_lngTranslate"]) ? $_GET["_lngTranslate"] : $_SESSION["_LANGUAGE_"];
$xLenguajes = $_lngToTranslate;
$DimLeng = array();
$DimImgLeng = array();
$DimTxtLeng = array();
SS::query("select * from {$_ENV['SYSDB']}gs_language where tf_translation='{$_ENV['ON']}' order by cd_gs_language");
while( $r = SS::get() ){
...
if( $r['cd_gs_language']!=$_lngToTranslate ) $xLenguajes .= ','.$r['cd_gs_language'];
}
...
SS::query("insert into {$_ENV['SYSDB']}gs_activity (cd_gs_user, cdi, script, edes, email) values ({$_User}, '".date('Y-m-d H:i:s')."', '{$File}', NULL, '".$_SESSION["_UserEMail"]."')");
...
if( $_GET['_EDF']=="/_datos/config/login.lng" && !file_exists($File) ){
copy(DIREDES."lng/login_web.lng", "..".$_GET['_EDF']);
}
...
$LngBase = ( $_lngToTranslate==$_SESSION["_LanguageDefault"] );
echo '<center><br>';
if( $LngBase ){
echo ('Poner textos en lenguaje base "'.trim($DimTxtLeng[$_SESSION["_LanguageDefault"]]).'('.$_SESSION["_LanguageDefault"].')"');
}else{
echo ('Traducir del "'.trim($DimTxtLeng[$_SESSION["_LanguageDefault"]]).'('.$_SESSION["_LanguageDefault"].')" al "'.trim($DimTxtLeng[$_lngToTranslate]).'('.$_lngToTranslate.')"');
}
echo '</center>';
$Dim = file($File);
...
$x = trim($Dim[$n]);
if( $x[0]=="[" ) $NumCorchetes++;
$Dim[$n] = str_replace('@LNGFILE@', '_'.$_lngToTranslate, $Dim[$n]);
list( $txt ) = explode(REM, trim(mb_strtoupper($Dim[$n])));
if( mb_substr($txt,0,8)=='#INCLUDE' ){
list(,$NomSubFile) = explode(')',$txt);
if( mb_strtoupper(trim($NomSubFile))=='LNG' ){
...
if( $tmp2[$i]==$_lngToTranslate ) $_LNGCOL = $i+1;
...
echo '<form accept-charset="utf-8" name="FRM1" method="post" action="">';
echo '<input type="hidden" OT=D name="DEFINICION" value="'.$File.'|'.$_SESSION["_LanguageDefault"].'|'.$_lngToTranslate.'|'.$PosLANGUAGE.'"" readonly>';
$np = 0;
...
echo '<td style="background:transparent"><input type="text" eAccent=1 value="" id="_Buscar" size=25 maxLength255 class=LngTraslate onfocus=S.key("#",255,0) tc="#" -onkeydown=ControlTeclado() title="" -onfocus="CntON()" -onblur="CntOFF()"></td>';
...
echo '<tr'.($_GET["_KEYOFF"] ? ' style="display:none"':'').'>';
$SinClave = '';
if( $LngBase ){
if( $TieneClave ){
echo '<td style="text-align:center"><i>&#250;</i>';
}else{
echo '<td>';
$SinClave = ' SinClave';
}
...
echo '<td><input type="text" eAccent=1 value="'.$Promp1.'" id="p'.$np.'" name="p'.$np.'__'.$DimTranslate[$n][3].'__'.$Tipo.'" size=90 maxLength255 class="LngBase'.$SinClave.'" readonly onfocus=S.key("#",255,0) tc="#" -onfocus="SiguienteCampo()" -style="width:100%">';
echo '<tr>';
echo '<td><img src="g/'.$DimImgLeng[$_lngToTranslate].'">';
echo '<td><input type="text" eAccent=1 value="'.$Promp2.'" ovalue="'.$Promp2.'" id="r'.$np.'" name="r'.$np.'__'.$DimTranslate[$n][3].'" size=90 maxLength255 class=LngTraslate NLinea='.($DimTranslate[$n][3]+1).(($Tipo=='T')?' readonly style="color:#AAAAAA"':'').' onfocus=S.key("#",255,0) tc="#" -onkeydown=ControlTeclado() title="'.$Ayuda.'" -onfocus="CntON()" -onblur="CntOFF()">';
echo '</table>';
}
$np++;
}
...
echo '<br><span style="font-size:100%">TRADUCCIONES: '.str_replace($_lngToTranslate,mb_strtoupper($_lngToTranslate),$xLenguajes).'</span>';
...
echo '<td><img class=lngSVG src="'.eImg64('$a/g/lng_'.$_SESSION["_LanguageDefault"].'.png').'">';
$Tipo = 'T';
}
echo '<td><input type="text" eAccent=1 value="'.$Promp1.'" id="lp'.$np.'" name="lp'.$np.'__'.$DimTranslate[$n][3].'__'.$Tipo.'" size=90 maxLength255 class="LngBase'.$SinClave.'" readonly onfocus=S.key("#",255,0) tc="#" -onfocus="SiguienteCampo()" -style="width:100%">';
echo '<tr>';
echo '<td><img class=lngSVG src="'.eImg64('$a/g/lng_'.$_lngToTranslate.'.png').'">';
echo '<td><input type="text" eAccent=1 value="'.$Promp2.'" id="lr'.$np.'" name="lr'.$np.'__'.$DimTranslate[$n][3].'" size=90 maxLength255 class=LngTraslate title="'.$Ayuda.'" onchange="Repercusion()" onfocus=S.key("#",255,0) tc="#" -onfocus="CntON()" -onblur="CntOFF()">';
echo '</table>';
$np++;
}
echo '</table>';
...
if( $txt[0]=='+' ) $txt = trim(mb_substr($txt,1));
if( $txt[0]*1==$txt[0] && $txt[0]>0 ) $txt = trim(mb_substr($txt,1));
if( $txt[0]=='<' ) $txt = trim(mb_substr($txt,1));
