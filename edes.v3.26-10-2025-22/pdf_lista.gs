...
set_time_limit(60*5);
if( $_POST['_ReportAll']==$_ENV['ON'] && !function_exists('gsActivity') ){
$_POST['_GRAPHS'] = '*';
$_GRAPH = '*';
...
$_Summary = ($_POST["_Summary"]==$_ENV['ON']);
...
$LenMax = max(mb_strlen($PDF_TxtTitulo), mb_strlen($PDF_TxtOrdenacion));
$LenMax = max($LenMax, mb_strlen($PDF_TxtCondicion));
...
if( $PDF_InfoTitulo[0]=='=' ) $PDF_InfoTitulo = trim(mb_substr($PDF_InfoTitulo,1));
if( $PDF_InfoTitulo=='' ) $PDF_InfoTitulo = ' ';
if( $_eDesTitle=='' ) $_eDesTitle = ' ';
PDF_set_info( $p, 'Title'	 , $PDF_InfoTitulo );
PDF_set_info( $p, 'Author'	 , SETUP::$System['ApplicationName']." " );
...
if( ($sx+(1*$FontPuntos)+mb_strlen($_DimCondicion[$n])*$FontPuntos)>($HojaAncho-$MargenHoja) ){
...
if( $_Form[$n][3] != 'H' ) $Celda = ePDF_ReplaceChr($Celda);
if( preg_match('/^(P4|F4|CDI)$/u', $_Form[$n][2]) ){
