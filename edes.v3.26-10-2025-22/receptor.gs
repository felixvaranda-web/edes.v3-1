...
$_SERVER['QUERY_STRING'] = str_replace( mb_chr(126).'R','',$_SERVER['QUERY_STRING'] );
$Help = true;
if( mb_strtoupper($_SERVER['QUERY_STRING'])=='NOHELP' ) $Help = false;
...
<?PHP
chdir('../../'.$DIR_DESTINO);
if( eSubstrCount( eGetCWD(), '/'.$DIR_DESTINO ) == 0 || trim($DIR_DESTINO)=='' ){
...
$OkFile[ $tmp[0] ] = 1;
$sFile = '';
if( !$Help && $DIR_DESTINO == 'edes' && mb_substr( $tmp[0],0,2 ) == 'h/' ){
}else{
...
if( $NomFile[$n] != $sFile ){
if( eSubstrCount( $NO_ACTUALIZAR,','.$tmp[0].',') == 0 ){
$LeerFile[] = $NomFile[$n];
...
$SeGrabo = true;
if( $FICHERO=='_gs_op_.txt' ) ImportarTABLA("{$_ENV['SYSDB']}gs_op", $FICHERO);
}
}
if( !$SeGrabo ){
if( mb_strrpos($FICHERO,'/') === false ){
$FICHERO = '___'.$FICHERO;
}else{
$FICHERO = mb_substr($FICHERO,0,mb_strrpos($FICHERO,'/')+1).'___'.mb_substr($FICHERO,mb_strrpos($FICHERO,'/')+1);
}
@unlink( $FICHERO );
$fd = fopen( $FICHERO, 'w' );
...
$NomDir = mb_substr( $DirBase.'/'.$file, 2);
if( $OkDir[ $NomDir ] != 1 ){
...
$txt = str_replace('#%0A#%0B#',CHR13.CHR10,$txt);
$txt = str_replace( "'", '"', $txt );
$Valores = "'".str_replace( '|', "','", chop($txt) )."'";
SS::query("insert into {$TABLA} values ( $Valores )");
$TReg++;
}
fclose($fd);
