...
if( !preg_match('/\.pdf$/u', $CopiarComo) ) $CopiarComo .= ".pdf";
$oNomPDF = $CopiarComo;
...
SS::query("select * from {$_ENV['SYSDB']}gs_form where token='{$Registro}'");
}else{
SS::query("select * from {$_ENV['SYSDB']}gs_form where cd_gs_form={$Registro} && cd_gs_node=".$_SESSION["_Node"]);
}
$r = SS::get();
...
$dim = explode("|", $txt);
do{
$dim[0] = str_replace(array("V","H"), array("P","L"), mb_strtoupper($dim[0]).",V");
...
if( $v==0 && trim(mb_strtoupper($dat[0])[0])!="R" ) continue;
if( $v==1 && trim(mb_strtoupper($dat[0])[0])=="R" ) continue;
switch( trim(mb_strtoupper($dat[0])[0]) ){
case '-':
$p->SetLineWidth(0.1);
...
$NextPagina = false;
if( $NextForm!="" ){
SS::query("select * from {$_ENV['SYSDB']}gs_form where cd_gs_form={$NextForm} && cd_gs_node=".$_SESSION["_Node"]);
$r = SS::get();
