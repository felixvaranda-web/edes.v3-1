...
if( $_SESSION["xls_private"]!=$_ENV['ON'] && $_SESSION["xls_public"]!=$_ENV['ON'] ) eEnd();
eInclude("xls");
...
if( (isset($_PDFNoCondition) && $_PDFNoCondition) || (isset($PDF_NoCondition) && $PDF_NoCondition) ){
$_DimCondicion = array();
}
...
if( !empty($PDF_TxtCondicion) ) $PDF_TxtCondicion .= ':';
$Indice = -1;
if( isset($_PDFTH) ){
for($n=1; $n<=9; $n++){
...
if( !empty($_PDFCOL[$n]) && mb_strtoupper($_PDFCOL[$n])=='XLS' ) $Visible[$n] = true;
$_ALIGN[$n] = trim(str_replace('id=', '', $_ALIGN[$n]));
if( !empty($_FORMAT[$n]) ){
if( mb_strtoupper($_FORMAT[$n])=='IMG'  ) $Visible[$n] = false;
if( mb_strtoupper($_FORMAT[$n])=='ICON' ) $Visible[$n] = false;
}
if( isset($_ToExcel) && $_ToExcel===true ) $Visible[$n] = true;
}
$Titulos = array();
for( $n=0; $n<count($_Form); $n++ ){
$Titulos[$n] = '';
...
if( $_LISTCHECKBOX['X']=='' ){
$_SelVirtual[$_Form[$n][1]][$_ENV['ON'] ] = $_CheckBox['X']['ON'];
$_SelVirtual[$_Form[$n][1]][$_ENV['OFF']] = $_CheckBox['X']['OFF'];
}else{
$_SelVirtual[$_Form[$n][1]][$_ENV['ON'] ] = strip_tags($_LISTCHECKBOX['X'][0]);
$_SelVirtual[$_Form[$n][1]][$_ENV['OFF']] = strip_tags($_LISTCHECKBOX['X'][1]);
}
}
}
}
...
setFileWatermark($NomFile, $_SubMode, $_FileDF);
if( !empty($_BrowseInTemporary) ){
fclose($_FileDownload);
}
$NomFile = str_replace('..','',$NomFile);
if( isset($_BackgroundReport) && $_BackgroundReport ){
$NomFile = '/_tmp/pdf/lst_'.S::$_User.'.xls';
_DownloadEnd(eScript($NomFile), eScript($_DownloadPath).$_CdGsExpFile.'.zip');
eEnd();
}
if( isset($_ToExcel) && $_ToExcel===true ){
eInit();
...
if( isset($_THCOLSPAN) && count($_THCOLSPAN)>0 ){
...
$Celda = $row[$n];
$field = "";
if( !empty($_Form[$n][1]) ) $field = $_Form[$n][1];
if( !empty($_ADDOPTION[ $field ]) ){
if( isset($_SelVirtual[$field][$Celda]) ){
$Celda = $_SelVirtual[$field][$Celda];
}else{
$Celda = "";
}
$_Form[$n][2] = 'D';
...
$Celda = trim($Celda);
if( !empty($_Form[$n][3]) && $_Form[$n][3]=='H' ){
$Celda = str_replace(CHR13 ," ",$Celda);
$Celda = str_replace(CHR10 ," ",$Celda);
$Celda = str_replace('&#39;' ,"'",$Celda);
$Celda = str_replace('&quot;','"',$Celda);
}else if( !empty($_Form[$n][2]) && $_Form[$n][2]=='#' ){
...
}else if( !empty($_Form[$n][2]) && $_Form[$n][2]=='o' ){
if( !empty($Celda) ){
if( isset($_CHECKLIST[ $field ]) ){
if( !empty($_CHECKLIST[ $field ][2]) ){
if( $_CHECKLIST[ $field ][2]=='WRAP' ) $_CHECKLIST[ $field ][2] = 'NOWRAP';
$tmp = explode(',',$Celda);
$Celda = '';
for( $i=1; $i<count($tmp)-1; $i++ ){
if( $i>1 ){
if( $_CHECKLIST[ $field ][2]=='NOWRAP' ) $Celda .= ', ';
else if( $_CHECKLIST[ $field ][2]=='WRAP' ) $Celda .= "\n";
}
$Celda .= $_SelVirtual[$field][$tmp[$i]];
}
}
}else if( isset($_RADIOLIST[ $field ]) ){
if( !empty($_RADIOLIST[ $field ][2]) ){
if( empty($_RADIOLIST[ $field ][2]) ){
}else if( $_RADIOLIST[ $field ][2]=='TEXT' ){
$Celda = $_SelVirtual[$field][$Celda];
}else if( $_RADIOLIST[ $field ][2]=='ALL' ){
$Celda .= '·'.$_SelVirtual[$field][$Celda];
}
}
}
}
}else if( !empty($_SELECTMULTIPLE[$field]) && $_SELECTMULTIPLE[$field]>0 && !empty($Celda) ){
if( $_SelVirtualType[ $field ] != 'T' ){
$tmp = explode(',',$Celda);
$Celda = '';
for( $i=1; $i<count($tmp)-1; $i++ ){
if( $i>1 ) $Celda .= ", ";
...
if( $_HayRadio && isset($_RADIO[$field]) ){
if( count($_RADIO[$field])>0 ){
$Celda = $_RADIO[$field][$Celda];
$_Form[$n][2] = 'D';
}
}
}
...
if( preg_match('/^(P4|F4|CDI|T)$/u',$_Form[$n][2]) ){
$Celda = eDataFormat($Celda, $_Form[$n][2]);
}else if( !empty($_FORMAT[$n]) ){
$Celda = (float)$Celda;
if( mb_substr($_FORMAT[$n],0,14)=='eNumberFormat(' ){
list(,$dec) = explode(",", $_FORMAT[$n]);
$dec = str_replace(array(")",";"), "", $dec);
$Celda = number_format($Celda, (int)$dec, '.', '');
}else{
...
if( isset($_DimCondicion) && count($_DimCondicion)>0 ){
...
if( $Ini < $Fin && mb_substr($txt,$Ini+1,1) > ' ' ) $txt = mb_substr($txt,0,$Ini).mb_substr($txt,$Fin+1);
}
}
$txt = preg_replace( '/#BR#/i'  ,'<BR>', $txt );
