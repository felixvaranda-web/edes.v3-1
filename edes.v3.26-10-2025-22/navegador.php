<?PHP
_HeaderAdd();
if( empty($_COOKIE['e-language']) ){
setcookie("e-language", SETUP::$System["LanguageDefault"]);
}
$nomIframe = "IFRAME".rand(10000, 99999);
$tmpFile = '../_datos/config/sql.ini';
include($tmpFile);
eHTML('$navegador.php', '', eFileGetVar('System.TabTitle'));
$logo = '';
...
echo "localStorage.setItem('s-device', '{$_POST['new-device']}');";
}
?>
</script>
<form accept-charset="utf-8" method="post" action="edes.php" style="display:none" target="<?=$nomIframe?>">
<input type="text" name="platform">
<input type="text" name="explorer">
<input type="text" name="resolution">
<input type="text" name="dirweb">
<input type="text" name="idioma">
<input type="text" name="versionLocal">
<input type="text" name="_DirEDes">
<input type="text" name='device'>
<input type="text" name='context' value="<?=$_SESSION["context"]?>">
<input type="text" name='_SESS_' value="<?=$_SESSION["_SESS_"]?>">
</form>
<iframe frameborder=0 src="about:blank" name="<?=$nomIframe?>" eNORESIZE=true style="width:100%; height:100%; border:0px solid red"></iframe>
<script type="text/javascript">
var href;
...
document.forms[0].versionLocal.value = localStorage.getItem('e-eDes') || "";
document.forms[0].device.value = localStorage.getItem("s-device") ?? "";
var Dim = document.forms[0].elements, i,
txt = '<!DOCTYPE HTML><HTML><HEAD><META http-equiv="Content-Type" content="text/html; charset=UTF-8"></HEAD><BODY style="display:none">'+
'<FORM accept-charset="utf-8" METHOD=POST action="'+href+'?login1">';
for(i=0; i<Dim.length; i++){
txt += '<INPUT TYPE="text" NAME="'+Dim[i].name+'" value="'+Dim[i].value+'">';
}
txt += '</FORM></BODY></HTML>';
