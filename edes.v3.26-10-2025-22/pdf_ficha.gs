...
case 'P':
return( $PDF_xMargen + mb_substr($x,1) );
case 'T':
global $_TabH;
$_uTab = mb_substr($x,1);
return( $_TabH[ mb_substr($x,1) ] );
case '%':
return( $PDF_xMargen + (($GLOBALS[HojaAncho] * mb_substr($x,1)) / 100 ) );
case 'X1G';
return( $GLOBALS[_x1G] );
case 'C':
$x = mb_substr($x,1);
...
$y = mb_strtoupper(trim($y));
switch( $y[0] ){
case '';
return( $GLOBALS[_yT] );
case 'P':
return( mb_substr($y,1) - $PDF_yMargen );
case 'T':
global $_TabV;
return( $_TabV[ mb_substr($y,1) ] - $PDF_yMargen );
case '%':
return( $HojaAlto - ( ( $HojaAlto * mb_substr($y,1) ) / 100 ) - $PDF_yMargen );
case '+':
if( mb_strlen($y) == 1 ){
return( $GLOBALS[_yT] - ( $PDF_AltoLetra * $SaltoLinea ) );
}else{
return( $GLOBALS[_yT] - ( mb_substr($y,1) * $PDF_AltoLetra * $SaltoLinea ) );
}
break;
case 'L':
$y = mb_substr($y,1);
...
return( $_TabH[ mb_substr( $x, 1 ) ] );
case '%':
return( ($GLOBALS[HojaAncho] * mb_substr( $x, 1 ) ) / 100 );
case 'C':
$x = mb_substr( $x, 1 );
default:
...
switch( $y[0] ){
case 'P':
return mb_substr( $y, 1 );
case 'T':
global $_TabV;
return( $_TabV[ mb_substr( $y, 1 ) ] );
case '%':
return( $HojaAlto - ( ( $HojaAlto * mb_substr( $y, 1 ) ) / 100 ) );
case '+':
if( mb_strlen($y) == 1 ){
}else{
}
...
if( eSubstrCount( $txt, '#' ) > 0 ) $t = str_repeat( ' ', eSubstrCount( $txt, '#' ) );
$t = trim(@eval( 'return '.mb_substr($txt,1).';' )).$t;
}else{
if( $ConFormato && !$Def && $_DefLabel[$_uTab]!='' ){
if( $_DefLabel[$_uTab][0] == 'R' ){
$v[0] = mb_substr( $v[0], 1 );
$t .= str_pad($txt, mb_substr($_DefLabel[$_uTab],1)-1, '.').': ';
}
}else{
$t = $txt.$t;
}
...
if( eSubstrCount( $txt,'$[' ) > 0 ) $txt = str_replace( '$[','$row[', $txt );
$tmp = explode( '$', $txt );
for( $i=0; $i<count($tmp); $i++ ){
$NewTxt = '';
for( $p=0; $p<mb_strlen($tmp[$i]); $p++ ){
$c = mb_substr( $tmp[$i], $p, 1 );
if( eSubstrCount( '.+-*/[ ', $c ) == 0 ){
$NewTxt .= $c;
}else{
break;
...
if( mb_substr($NewTxt,0,3) != 'row' ) $tmp[$i] = 'GLOBALS['.$NewTxt.']'.mb_substr( $tmp[$i], mb_strlen($NewTxt) );
}
$NewTxt = '';
...
$x = cX($d[1]); $y = cY($d[2]);
$t = cT($d[3]);
if( count($_DefVariable) > 0 && eSubstrCount( $d[3], '$' ) > 0 ){
...
if( $d[4] != '' ){
$t = cT( $d[4] );
if( count($_DefVariable) > 0 && eSubstrCount( $d[4], '$' ) > 0 ){
$sfont = PDF_findfont( $p, $_TipoLetra[$_DefVariable[1]], 'host', 0 ); PDF_setfont( $p, $sfont, $_DefVariable[0] );
PDF_show( $p, $t );
$font = PDF_findfont( $p, $_TipoLetra[C], 'host', 0 ); PDF_setfont( $p, $font, 8 );
}else{
...
case 't':
$t = cT( $d[1] );
if( count($_DefVariable) > 0 && eSubstrCount( $d[1], '$' ) > 0 ){
$sfont = PDF_findfont( $p, $_TipoLetra[$_DefVariable[1]], 'host', 0 ); PDF_setfont( $p, $sfont, $_DefVariable[0] );
PDF_show( $p, $t );
$font = PDF_findfont( $p, $_TipoLetra[C], 'host', 0 ); PDF_setfont( $p, $font, 8 );
}else{
...
$t = cT( $d[2] );
if( $t != '' ){
if( count($_DefVariable) > 0 && eSubstrCount( $d[2], '$' ) > 0 ){
$sfont = PDF_findfont( $p, $_TipoLetra[$_DefVariable[1]], 'host', 0 ); PDF_setfont( $p, $sfont, $_DefVariable[0] );
PDF_show( $p, $t );
$font = PDF_findfont( $p, $_TipoLetra[C], 'host', 0 ); PDF_setfont( $p, $font, 8 );
}else{
...
$ConBorde = false;
if( eSubstrCount( $d[5], '-' ) > 0 ){
$d[5] = str_replace('-','',$d[5]);
$ConBorde = true;
}
$d[5] = mb_strtoupper(trim($d[5]));
$x = cX($d[1]); $y = cY($d[2]-1);
$w = cW($d[3]); $h = cH($d[4]);
$t = cT($d[6]);
if( $d[7] != '' ) $t .= cT($d[7]);
