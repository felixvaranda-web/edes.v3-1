...
el = eGO(NmField),
acc = "&_CC="+S(el).attr("e-cc");
acc += "&eID="+S(el).attr("eID");
if( ((window.event.detail<0)? 1 : (window.event.wheelDelta>0)? 1 : -1)<0 ){
if( SelBROWSER.children[0].getAttribute("HayMas")==1 ){
S(el).incr("ePag");
top.eCallSrv(window, 'edes.php?E:$seltable.gs&VALUE='+escape(_SelBrowserBuscar)+'&DESTINO='+NmField+'&ALTO='+el.getAttribute("eHeight")+'&COLS='+el.getAttribute("eViewCols")+eParameters + Include+'&PAGINA='+el.getAttribute("ePag") + ((typeof(_DB)!='undefined')?"&_DB='"+_DB+"'":'')+acc);
}
}else if( el.getAttribute("ePag")>0 ){
S(el).incr("ePag", -1);
top.eCallSrv(window, 'edes.php?E:$seltable.gs&VALUE='+escape(_SelBrowserBuscar)+'&DESTINO='+NmField+'&ALTO='+el.getAttribute("eHeight")+'&COLS='+el.getAttribute("eViewCols")+eParameters + Include+'&PAGINA='+el.getAttribute("ePag")+'&ATRAS=1' + ((typeof(_DB)!='undefined')?"&_DB='"+_DB+"'":'')+acc);
}
S.eventClear(window);
}
function _SelBrowserCopy(Destino, Contenido){
var xy = eXY(DGI(Destino)),
y  = xy[1] + DGI(Destino).offsetHeight - 1;
S(SelBROWSER).css({
left:	 xy[0]+1
,top:	 y+"px"
,zIndex: 50000
...
try{
if( window.frameElement.FSCREEN==undefined || window.frameElement.FSCREEN==false ){
SelBROWSER.style.left = px(SelBROWSER.style.left-2);
}
}catch(e){}
}
S.selectNone(window);
...
if( S.windowIs(window) && parseInt(SelBROWSER.style.top)+parseInt(SelBROWSER.style.height)>document.body.clientHeight ){
S(window).windowResize(document.body.clientWidth-10, parseInt(SelBROWSER.style.height)+y+5, true);
}
}
...
var Dim = obj.getAttribute("eAssign").split(','),
MaxCol = Math.min(Dim.length, SelBROWSER.children[0].rows[n].cells.length), i, value;
for(i=0; i<MaxCol; i++) if( Dim[i]!='' ){
value = (vacio) ? "" : SelBROWSER.children[0].rows[n].cells[i+1].textContent;
ePF(Dim[i], value);
}
}
}
...
eParameters = _SelBrowserParameter(obj),
acc = "&_CC="+S(obj).attr("e-cc");
acc += "&eID="+S(obj).attr("eID");
top.eCallSrv(window, 'edes.php?E:$seltable.gs&VALUE='+escape(obj.value)+'&DESTINO='+NmField+'&ALTO='+obj.getAttribute("eHeight")+'&COLS='+obj.getAttribute("eViewCols")+eParameters + Include + ((typeof(_DB)!='undefined')?"&_DB='"+_DB+"'":'')+acc );
...
obj.setAttribute("cdValue", Dim[0]);
}
break;
case 8:
case 46:
