...
echo '<span style="width:100%;height:24px;vertical-align:baseline;padding-top:4;cursor:var(--cPointer);">'.$DimSolapa[$n]['T'];
if( eSubstrCount('~',$_SESSION["_D_"])==1 || file_exists('../_d_/usr/edhelp.'.$_User) ){
...
while( !feof($fd) ){
$buffer = fgets($fd,512);
if( mb_ord($buffer[0]) == 10 ) continue;
if( mb_substr(trim($buffer),0,1)=='.' || mb_substr($buffer,0,6)=='[help]' || mb_substr($buffer,0,6)=='[edit]' ){
if( mb_substr($buffer,0,6)=='[edit]' ){
...
$tmp = ltrim($buffer);
if( mb_strlen($buffer) < 3 ) continue;
$nivel = mb_strpos( $buffer, $tmp[0] );
$n++;
list( $buffer, $nAyuda ) = explode(mb_chr(126),$buffer);
...
if( mb_substr($Arbol[$n][1],-5) == ':this' ) $Arbol[$n][1] = mb_substr($Arbol[$n][1],0,-4).$Arbol[$n][3];
if( mb_substr($Arbol[$n][1],-7) == ':parent' ){
$i = $n-1;
while( $i>0 ){
if( $Arbol[$n][2] > $Arbol[$i][2] ){
$Arbol[$n][1] = mb_substr($Arbol[$n][1],0,-6).$Arbol[$i][3];
break;
}
$i--;
}
...
if( $buffer[0]=='{' ){
$sOp = $buffer;
$graf = mb_substr( $sOp,1, mb_strpos( $sOp,'}' )-1 );
$buffer = trim(mb_substr( $sOp,mb_strpos( $sOp,'}')+1 ));
}
$tOp++;
...
fclose($fd);
$buffer = explode( CHR10, gzuncompress( $Todo ) );
if( $buffer[0] == mb_substr( $NomFile,mb_strrpos($NomFile,'/')+1 ) ){
for( $i=1; $i<count($buffer); $i++ ){
if( mb_ord($buffer[$i][0]) != 10 ){
$tmp = ltrim($buffer[$i]);
if( $tmp != '' ){
$nivel = mb_strpos( $buffer[$i], $tmp[0] );
$n++;
list( $Arbol[$n][0], $Arbol[$n][1] ) = explode('|',$buffer[$i]);
$Arbol[$n][0] = trim( $Arbol[$n][0] );
$Arbol[$n][1] = trim( $Arbol[$n][1] );
$Arbol[$n][2] = $nivel;
list( $tmp1, $tmp2 ) = explode(mb_chr(126),$Arbol[$n][1]);
$Arbol[$n][1] = trim($tmp1);
$Arbol[$n][3] = trim($tmp2);
}
}
...
echo '</TABLE>';
$txt = '';
foreach($_DimGraf as $clave=>$val){
if( eSubstrCount( $val, '.' ) == 0 ) $val .= '.gif';
$txt .= "{$val}|";
}
?>
<SCRIPT type="text/javascript">
