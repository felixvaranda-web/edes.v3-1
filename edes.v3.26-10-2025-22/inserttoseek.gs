...
<?PHP
$url1 = mb_strtoupper($_GET['_url']);
$url1 = $url1[0]."M".mb_substr($url1,2);
$url2 = "";
switch( $url1[0] ){
case 'G':
$url2 = "@M".mb_substr($url1,2);
break;
case '@':
$url2 = "GM".mb_substr($url1,2);
break;
case 'F':
$url2 = "#M".mb_substr($url1,2);
break;
case '#':
$url2 = "FM".mb_substr($url1,2);
break;
}
$puedeModificar = false;
$dim = file('../_tmp/php/menu.'.S::$_User);
for($n=0; $n<count($dim); $n++){
$url = mb_strtoupper(trim($dim[$n]));
...
if( p[n].tagName=='INPUT' && p[n].className!='READONLY' ){
p[n].maxLength = p[n].maxLength*3+6;
for(i=0; i<_cForm.length; i++){
_cForm[i].Condicion = "";
_DefCampo[_cForm[i].Nombre].Condicion = "";
}
}
...
if( typeof(_CLEARTOCOUNT)!="undefined" ){
var dimClear = _CLEARTOCOUNT.split(",");
for(n=0; n<dimClear.length; n++){
try{
ePF(dimClear[n], "");
}catch(e){}
}
