...
$t=trim($v);
if($t[0]=='.')continue;
$indent=mb_strlen(rtrim($v))-mb_strlen(trim($v));
if(mb_strpos($v,'{')){
$t=explode('{',$v);
$v=$t[0];
$f=mb_substr($t[1],0,mb_strpos($t[1],'}') );
}
$f=$H.mb_strtolower(trim($v)).'.htm';
$f=str_replace(' ','_',$f);
$f=str_replace($buscar,$reemplazar,$f);
if(mb_strpos($v,'+')){
$t=explode('+',$v);
$f=str_replace('+','_',$v);
$f=$H.mb_strtolower(trim($f)).'.htm';
$v=$t[0];
}
$i=count($filesa);
