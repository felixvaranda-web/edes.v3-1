...
$dimTodo = array();
$dim = file(DIREDES."h/i/help2.ind");
for($na=0; $na<count($dim); $na++){
$buscar = trim($dim[$na]);
if( $buscar=="" || eSubstrCount($buscar, " ")>0 ) continue;
if( $buscar[0]=="e" || $buscar[0]=="q" || mb_substr($buscar,0,3)=="xls" || mb_substr($buscar,0,3)=="xml" ){
$file = "";
if( eSubstrCount($buscar, "{")>0 ){
$file = eMid($buscar, "{", "}");
}else if( eSubstrCount($buscar, "+")>0 ){
list($file, $sufi) = explode("+", $buscar);
$file = $file.'_'.$sufi;
}else{
$file = $buscar;
}
$file = mb_strtolower($file).'.htm';
$pos = false;
$txt2 = file_get_contents($dir.$file);
if( mb_strlen($txt2)==0 ) eTron("Falta la ayuda de: ".$buscar);
