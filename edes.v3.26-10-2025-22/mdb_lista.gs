...
if( mb_strtoupper($_PDFCOL[$n])=='XLS'  ) $Visible[$n] = false;
...
$Dim = file(DIREDES.'t/lng/access.rw');
...
$dFile = mb_substr($oFile,0,-3).'unl';
$eFile = '/_tmp/pdf/'.$NomFile.'.unl';
while( eSubstrCount('  ',$_SQL_)>0 ) $_SQL_ = str_replace( '  ', ' ', $_SQL_ );
list( ,$NomTabla ) = explode( ' from ', $_SQL_ );
list( $NomTabla ) = explode( ' ', $NomTabla );
if( trim($NomTabla)=='' ) $NomTabla = 'tabla';
if( $PalabraReservada[mb_strtoupper($NomTabla)] ) $NomTabla = '_'.$NomTabla;
...
fwrite( $fp, 'replace=~::{&#126;}'.$Enter );
if( $_DEBUGAccess ) fwrite( $fp, 'debug=1'.$Enter );
...
if( eSubstrCount( $NomCampo, ')' ) > 0 ) list( $NomCampo ) = explode(')',$NomCampo);
...
$NomCampo = str_replace(' ','_',$NomCampo);
$NomCampo = trim($NomCampo);
if( $PalabraReservada[mb_strtoupper($NomCampo)] ) $NomCampo = '_'.$NomCampo;
...
if( mb_strtoupper($_Form[$n][3])=='C' ){
$Create .= 'char(1)';
}else if( mb_strtoupper($_Form[$n][3][0])=='S' ){
...
$valor = str_replace('&#92;' ,'\\',$valor);
$valor = str_replace('&nbsp;',' ' ,$valor);
if( (mb_strlen($valor)==10 || mb_strlen($valor)==19) && isZero($valor) ) $valor = "";
...
$valor = mb_substr(strip_tags($valor), 0, $_Form[$n][4]);
}
