...
if( $_LogUser[0]=='"' || $_LogUser[0]=="'" ) $_LogUser = mb_substr($_LogUser,1);
if( mb_substr($_LogUser,-1)=='"' || mb_substr($_LogUser,-1)=="'" ) $_LogUser = mb_substr($_LogUser,0,-1);
...
if( SS::query( "select {$Campos} from {$Tabla} where {$Condicion}" ) != -1 ){
$r=SS::get("num");
$Fecha = trim($r[0]);
if( $_Rastrear ){
...
if( $_Rastrear ) echo 'SQL: '."select user_name, user_surname from {$_ENV['SYSDB']}gs_user where cd_gs_user='{$r[1]}'".'<br>';
SS::query( "select user_name, user_surname from {$_ENV['SYSDB']}gs_user where cd_gs_user='{$r[1]}'" );
$r=SS::get("num");
$r[0] = trim($r[0]);
if( $r[0]!='' ){
...
if( $_Rastrear ) echo 'SQL: '."select {$Campos} from {$Tabla} where {$Condicion}".'<br>';
if( SS::query( "select {$Campos} from {$Tabla} where {$Condicion}", [], 1) != -1 ){
$r=SS::get("num", 1);
$nc = 0;
...
SS::query( "select user_name, user_surname from {$_ENV['SYSDB']}gs_user where cd_gs_user='".$r[$nc+1]."'" );
$r2=SS::get("num");
$r2[0] = trim($r2[0]);
if( $r2[0]!='' ){
...
$PorTipo = array();
if( $_Rastrear ) echo 'SQL: '."select * from {$_ENV['SYSDB']}gs_log where tabla='{$Tabla}' and clave='{$IndiceLog}' order by 1".'<br>';
SS::query("select * from {$_ENV['SYSDB']}gs_log where tabla='{$Tabla}' and clave='{$IndiceLog}' order by 1");
while( $rl = SS::get() ){
$Fecha = mb_substr($rl['cdi'],0,10);
SS::query("select user_name, user_surname from {$_ENV['SYSDB']}gs_user where cd_gs_user='{$rl['cd_gs_user']}'", [], 1);
$r = SS::get("num", 1);
$r[0] = trim($r[0]);
if( $r[0]!='' ){
$NomUsuario = $r[0].' '.trim($r[1]);
...
if( mb_strtoupper(trim($xTipo))<>'HIDDEN' ) exit;
$DimCampos = explode( ',', $Campos );
$DimLabel = explode( ',', $DimLabel );
