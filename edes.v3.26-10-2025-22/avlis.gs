...
<?PHP
if( SS::count("{$_ENV['SYSDB']}gs_exp_file", "cd_gs_user={$_User} and estado='T'" ) == 0 ){
?>
...
if( $MasInfo ){
?>
<BR><TABLE cellspacing=0 cellpadding=0 style='background:transparent'><tr><td id=TITULO align=center nowrap style='cursor:var(--cAuto);background:transparent;'>LISTADO DE EXTRACCIONES</td></tr></TABLE>
<?PHP
}
...
if( $MasInfo ){
SS::query("select * from {$_ENV['SYSDB']}gs_exp_file where cd_gs_user={$_User} and cdi>='{$HastaCDI}' order by cdi DESC");
}else{
SS::query("select * from {$_ENV['SYSDB']}gs_exp_file where cd_gs_user={$_User} and estado='T' and cdi>='{$HastaCDI}' order by cdi DESC");
}
while( $row = SS::get() ){
...
'&U=<?= $_User ?>';
DESCARGA.location.href = top.S.urlAdd(Dato);
