<!DOCTYPE HTML>
<HTML><HEAD><META http-equiv="Content-Type" content="text/html; charset=UTF-8"><TITLE> INSTALACIONES </TITLE>
...
function Instalar(){
	...
	switch( Obj.Producto*1 ){
		...
		case 3:		//FLASH
			...
			break;
		case 4:		//Añadir a favoritos
			<?PHP  include( '../_datos/config/desktop.ini' ); ?>
			window.external.AddFavorite( _url, '<?= eFileGetVar("System.ApplicationName"); ?>' );
			return;
		case 5:		//Organizar favoritos
			window.external.ShowBrowserUI("OrganizeFavorites",null);
			return;
		case 6:		//Página de inicio
			INICIO.fireEvent('onclick');
			return;
		case 7:		//Propiedades IE
			window.external.ShowBrowserUI("PrivacySettings",null);
			return;
		case 8:		//Internet Explorer
			Url = 'http://www.microsoft.com/downloads/';
			break;
		default:
			top.eAlert( top._xMESSAGE, 'Producto no definido.', 'A', 'W' );			//alert('Producto no definido');
			return;
	}
