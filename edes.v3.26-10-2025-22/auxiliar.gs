...
global $_NFunc;
$_NFunc++;
if( mb_substr($Tabla,0,3)=='cd_' ) $Tabla = mb_substr($Tabla,3);
$Campos = 'cd_'.$Tabla.', nm_'.$Tabla;
$Orden = 'nm_'.$Tabla;
SS::query("select {$Campos} from {$Tabla} where {$where} order by {$Orden}");
...
if( trim(mb_strtoupper($Vacio)) != 'NOEMPTY' ){
echo '<tr>';
echo '<td><td>&nbsp;';
}
...
<SCRIPT type="text/javascript">
DB<?= $Tabla; ?>.DataURL = "edes.php?T:&T=<?= $Tabla; ?>&F="+escape("<?= $where; ?>")+'&E=<?= trim(mb_strtoupper($Vacio)); ?>';
DB<?= $Tabla; ?>.Reset();
<?PHP  if( $Desde!='' ){ ?>
<?PHP  } ?>
</SCRIPT>
<?PHP
echo "<table CLASS='AUXI' id='{$Tabla}' Desde='".str_replace("'",'',$Desde)."' Hasta='' ancho=0 datasrc='#DB{$Tabla}' DataPagesize=".SETUP::$List['MaxVisibleRows']." style='display:none' cellspacing=1px cellpadding=1px>";
echo '<COL id=o><COL>';
echo '<thead>';
echo '<TR style="position:relative; top:0px">';
echo '<th><th>';
...
if( SS::isDriver("informix") ) $Campos = $Campos.',orden';
SS::query("select {$Campos} from {$Tabla} where {$where} order by orden");
echo "<table CLASS='AUXI' id='{$Tabla}' style='display:none' cellspacing=1px cellpadding=1px>";
echo '<COL id=o>';
echo '<COL>';
...
$tCampos = 1;
$Tabla = mb_substr( strmb_chr($nNomCampo,'_'), 1 );
...
if( trim(mb_strtoupper($Vacio)) != 'NOEMPTY' ) echo '<TR><TD><TD>&nbsp;';
...
SS::query("select {$Campos} from {$Tabla} where {$condicion} order by {$Ordenacion}");
while( $row = SS::get("num") ){
echo '<TR><TD>'.trim($row[0]).'<TD>'.trim($row[1]);
...
if( mb_substr($_FileLoading,-4)=='.swf' ){
