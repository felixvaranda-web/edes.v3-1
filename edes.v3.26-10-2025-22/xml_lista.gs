...
if( $_SESSION["xml_private"]!=$_ENV['ON'] && $_SESSION["xml_public"]!=$_ENV['ON'] ) eEnd();
eInclude("xml");
...
if( mb_strtoupper($_PDFCOL[$n])=='XLS' ) $Visible[$n] = true;
$_ALIGN[$n] = trim(str_replace('id=','',$_ALIGN[$n]));
...
$NomFile = '/_tmp/pdf/lst_'.$_User.'.xml';
...
$NomFile = '/_tmp/pdf/lst_'.S::$_User.'.xml';
_DownloadEnd(eScript($NomFile), eScript($_DownloadPath).$_CdGsExpFile.'.zip');
eEnd();
}
if( $_ToXML===true ){
eInit();
echo "<a href='edes.php?D:{$NomFile}' style='display:none'>Descargar XML</a>";
...
global $_ROWSOP, $_exeROWSOPCALC;
$TReg = 0;
$File = xmlCreate($NomFile);
$nl = 0;
if( $_TITLE!="" ){
fputs($File, "<Row>\n");
$tmp = explode('<BR>', strip_tags($_TITLE));
for($i=0; $i<count($tmp); $i++){
xmlText($File, trim($tmp[$i]));
$nl++;
}
fputs($File, "</Row>\n");
$nl++;
...
_xml_pintaCondiciones( $_DimCondicion, $File );
...
$txt = strip_tags($_DimTHText[$i][$n]);
eTron("1:".$txt);
if( isset($_HEADER_ORIGINAL) && $_HEADER_ORIGINAL ){
$txt = str_replace('<BR>', ' ', $txt);
}else{
$txt = str_replace('<BR>', ' ', eStrUpper($txt));
}
eTron("2:".$txt);
...
for( $n=0; $n<count($Titulos); $n++ ){
if( !$Visible[$n] ) continue;
xmlText($File, strip_tags($Titulos[$n]));
$nc++;
}
$nl++;
fputs($File, "</Row>\n");
...
if( $nl > $_XLSLIMIT ){
fputs($File, "<Row>\n");
xmlText( $File, 'Se ha llegado al límite de la extracción '.($nl-1).' de '.$_TReg );
fputs($File, "</Row>\n");
break;
}
$nc = 0;
...
$Celda = str_replace('&#43;','+',$Celda);
$Celda = str_replace('&#92;','\\',$Celda);
$Celda = preg_replace( '/\{#\}ENTER\{#\}/i', " ", $Celda );
}else if( $_Form[$n][2] == 'o' ){
if( $Celda!='' ){
...
break;
default:
xmlText( $File, strip_tags($Celda) );
}
$nc++;
}
if( isset($_ROWSOP) && count($_ROWSOP)>0 ){
if( isset($_exeROWSOPCALC) ) $Celda = eval($_exeROWSOPCALC);
xmlNumber($File, strip_tags($Celda));
}
if( $TReg==0 ){
}
fputs($File, "</Row>\n");
...
global $_SubMode, $_FileDF;
fputs($File, "<hiddenData>".setFileWatermark(null, $_SubMode, $_FileDF)."</hiddenData>\n");
xmlClose( $File );
return $TReg;
}
...
function _xml_pintaCondiciones( $_DimCondicion, $File ){
if( count($_DimCondicion) > 0 ){
