...
if( $_SESSION["csv_private"]!=$_ENV['ON'] && $_SESSION["csv_public"]!=$_ENV['ON'] ) eEnd();
...
if( mb_strtoupper($_PDFCOL[$n])=='CSV' ) $Visible[$n] = true;
$_ALIGN[$n] = trim(str_replace('id=','',$_ALIGN[$n]));
...
$NomFile = '/_tmp/zip/lst_'.$_User.'.csv';
$_fichero = 'lst_'.$_User;
$NomFile = eScript($NomFile);
if( file_exists($NomFile) ) unlink($NomFile);
$oNomFile = mb_substr($NomFile,0,-3);
if( file_exists($oNomFile.'txt') ) @unlink($oNomFile.'txt');
...
$_TReg = DestinoCSV($NomFile, $_Form, $Visible);
if( $_BrowseInTemporary!="" ){
fclose($_FileDownload);
}
$NomFile = str_replace('..','',$NomFile);
eZipFile("{$Dir}{$_fichero}.zip", ["{$Dir}{$_fichero}.csv", "{$Dir}{$_fichero}.def"]);
@unlink($Dir.$_fichero.'.csv');
@unlink($Dir.$_fichero.'.def');
$NomFile = str_replace('.csv', '.zip', $NomFile);
setFileWatermark($NomFile, $_SubMode, $_FileDF);
if( $_BackgroundReport ){
$NomFile = '/_tmp/zip/lst_'.S::$_User.'.zip';
_DownloadEnd(eScript($NomFile), eScript($_DownloadPath).$_CdGsExpFile.'.zip');
eEnd();
}
if( $_ToCSV===true ){
eInit();
echo "<a href='edes.php?D:{$NomFile}' style='display:none'>Descargar CSV</a>";
...
eEnd();
function DestinoCSV( $NomFile, $_Form, $Visible ){
...
global $_CSVWITHHEADER;
if( !isset($_CSVWITHHEADER) ){
$_CSVWITHHEADER = false;
}
$TReg = 0;
$dimHeader = [];
$FileTH = fopen( str_replace('.csv','.def',$NomFile), 'w' );
...
$_Form[$n][0] = str_replace('<BR>',' ',$_Form[$n][0]);
fwrite($FileTH, $_Form[$n][0].' ('.$_Form[$n][1].')'."\n");
array_push($dimHeader, $_Form[$n][0]);
}
$DimCelda[] = $Celda;
}
if( isset($_ROWSOP) && count($_ROWSOP)>0 ){
...
if( $TReg==0 && $_CSVWITHHEADER ){
fputcsv($File, $dimHeader);
}
fputcsv($File, $DimCelda);
$TReg++;
}
fclose($FileTH);
fclose($File);
