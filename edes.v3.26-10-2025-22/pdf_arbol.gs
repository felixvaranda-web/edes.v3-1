<?PHP
include_once(DIREDES.'lng/manager_op.ini');
...
if( $_GET['TipoArbol']=='U' ){
if( $_GET['USER'] > 0 ){;
SS::query("select * from {$_ENV['SYSDB']}gs_user where cd_gs_user='{$_GET['USER']}'");
$row = SS::get();
...
while( $_DimTree[] = SS::get() ){}
}else if( $_GET['TipoArbol']=='A' && $_GET['nArbol'] > 0 ){
$nArbol = $_GET['nArbol'];
SS::query("select nm_gs_tree from {$_ENV['SYSDB']}gs_tree where cd_gs_tree={$nArbol}");
list( $NomArbol ) = SS::get("num");
$_TITLE = 'ARBOL: '.$NomArbol;
SS::query("select * from {$_ENV['SYSDB']}gs_op where cd_gs_op in (select cd_gs_op from {$_ENV['SYSDB']}gs_tree_op where cd_gs_tree={$nArbol}) order by seq");
while( $_DimTree[] = SS::get() ){}
}else{
exit;
}
...
PDF_show_xy( $p, $_ModeOp[$row['mode']], (double)($PDF_xMargen), (double)$y );
$incr = $row['indent']*15+15;
if( $row['type']=='L' ){
if( $row['caption'][0]=='-' ) $row['caption'] = trim(mb_substr($row['caption'],1));
if( trim($row['caption'])!='' ){
...
for($n=1; $n<=$Fin; $n++){
$file = "../_tmp/pdf/lst_{$_User}_file_{$n},{$Fin}.pdf";
if( file_exists($file) ){
@unlink($file);
}
}
...
eInit();
$url = $oNomFile."&_Source={$OriFichero}&_SubMode={$_SubModo}";
eMessage('~PDF', 'HS', '', 'try{_WOPENER.eHideBusy();}catch(e){};location.href = "'."edes.php?{$ConRemoto}D:{$url}".'";');
}else{
eInit();
eJS('var Func=window.frameElement.eEXE; window.frameElement.eEXE=null; Func();');
}
}else{
if( $_CACHEFILESRV!='' ) copy( eScript($oNomFile), eScript($_CACHEFILESRV) );
if( function_exists('uFileSupport') ) uFileSupport($oNomFile);
$url = $oNomFile."&_Source={$OriFichero}&_TReg={$_NumReg}&_SubMode={$_SubModo}&_Doc='{$_TITLETOEXTRACT}'";
eMessage('~PDF','HS','','try{_WOPENER.eHideBusy();}catch(e){};location.href = "edes.php?'.$ConRemoto.'D:'.$url.'";');
}
}
eEnd();
...
PDF_show_xy( $p, $PDF_TxtFecha, $HojaAncho-$MargenHoja-(mb_strlen($PDF_TxtFecha)*$FontPuntos), $y );
PDF_restore($p);
}
...
PDF_restore($p);
$sx += mb_strlen($PDF_TxtTitulo)*$FontPuntos+5;
$GLOBALS['_yUltimaLinea'] = $y;
