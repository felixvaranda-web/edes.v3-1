...
echo 'Arg: '.$query[0].'<br><br>';
echo 'Arg: '.urldecode($query[0]).'<br><br>';
}
$_DB = $_GET['_DB'];
if( $_DB!='' ){
if( $_DB[0]=='>' ) $_DB = trim(mb_substr($_DB,1));
if( eSubstrCount(str_replace('\\','/',$_DB),'/')==0 ) $_DB = '/_datos/config/'.$_DB;
if( eSubstrCount($_DB,'.')==0 ) $_DB .= '.ini';
if( $_DB[0]=='~' ){
...
if( eSubstrCount( $Dim[2], CHR92 ) > 0 ){
list( $uMensaje, $Linea2 ) = explode( CHR92, $Dim[2] );
}else{
...
if( $Comprobar ){
if( DB::count($Dim[3], $txt) > 0 ){
if( eSubstrCount( $Dim[1], '.' ) == 1 ){
DB::query("select * from {$Dim[3]} where {$txt}");
$_vF = DB::get();
eInit();
...
<?PHP
if( mb_substr($Dim[4],0,7)=='_INPUT_'){
echo 'try{';
echo 'WOPENER.DGI("'.mb_substr($Dim[4],7).'").value="";';
echo '}catch(e){}';
}
...
DB::query("select * from {$Dim[3]}", $txt );
$row = DB::get();
...
if( eSubstrCount($xLabel,"'")==2 ) list(,$xLabel) = explode("'",$xLabel);
$xLabel = eUcFirst($xLabel);
}
if( $_Form[$n][3]=='S' ){
if( mb_substr($nomCampo,0,3)=="cd_" ){
$SubTabla = trim(mb_substr( $_Form[$n][1], 3 ));
SS::query("select nm_{$SubTabla} from {$SubTabla} where cd_{$SubTabla}='{$row[$_Form[$n][1]]}'", [], 1);
}else if( eSubstrCount($_Form[$n][1], '{')==1 ){
$qCampo = str_replace('&#124;','|',$_Form[$n][1]);
$tmp = str_replace('{', ',', $qCampo);
$tmp = str_replace('}', '', $tmp);
$tmp = explode(',', $tmp);
SS::query("select {$tmp[3]} from {$tmp[1]} where {$tmp[2]}='{$row[$nomCampo]}'", '', [], 1);
}else if( eSubstrCount($_Form[$n][1],':')>0 ){
list($NomCampo, $nNomCampo) = explode(':', $_Form[$n][1]);
$SubTabla = mb_substr($nNomCampo, 3);
if( mb_strpos($_LanguageTables, ",{$SubTabla},")===false ){
...
if( mb_substr($xCampos,-1)=="." ) $xCampos = mb_substr($xCampos,0,-1);
continue;
}
$row2 = SS::get("num", 1);
...
global $_User, $_Node, $_Tree, $_ePermission;
$OriFichero = $FicheroD;
$cModo = 'a'; $Option = 'a';
$DFichero = array();
$Opcion = 'a';
...
if( eSubstrCount( ",{$v2[0]},", $Opcion ) > 0 || $v2[0]=='*' ){
...
if( $Ok ){
list( $tmp[0], $MsgList, $MsgError ) = explode( CHR92, $tmp[0] );
