...
<?PHP
echo '</HEAD><BODY>';
echo "<FORM accept-charset='utf-8' eType='Directo' NAME='FRM1' METHOD='POST' ACTION='{$DirReceptor}'>".$__Enter;
echo "EQUIPO: <B>".SETUP::$System['ApplicationName']."</B><BR><BR>";
...
fclose($fd);
echo '|'. trim(mb_substr($NomFile,2)).str_replace(' ','','#'.$Long.'#'.crc32($Contenido));
}
}
}
closedir( $di );
...
<?PHP
echo '</HEAD><BODY>';
echo "<FORM accept-charset='utf-8' eType='Directo' NAME='FRM1' METHOD='POST' ACTION=''>".$__Enter;
echo "<center><br>EQUIPO: <b>".SETUP::$System['ApplicationName']."</b><BR><BR>";
$Dim = array();
$Ok = 0;
if( $EsEdes ){
$DirUrl = file($FileConfig);
