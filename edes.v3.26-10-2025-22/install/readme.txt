...
Nota Inicial. Los antivirus pueden producir un mal funcionamiento por la captura de los ficheros javascript comprimidos ya que interpretan que puede ser codigo malicioso.
...
PASOS A SEGUIR CON UN MODELO COMPATIBLE UTILIZANDO LA DISTRIBUCION XAMPP:
	...
	- Para instalar la licencia hay que añadir la siguiente linea
		zend_optimizer.license_path = "c:\xampp\htdocs\edes.v3\license_edes.zl" ; El fichero de licencia se encuentra dentro del directorio raiz de edes.
...
3- INSTALAR E-DES.
	...
	- Descomprimir el fichero edes.zip en un directorio dentro de "c:\web\apache\htdocs" se creara un directorio "c:\xampp\apache\htdocs\edes.v3".
	- Las Intranet que tu desarrolles se situaran al mismo nivel que EDES. 
	- En el caso de instalar una Intranet en una máquina de producción se debe crear un directorio "c:\web\apache\htdocs\aplicacion\http" y configurar el http.conf del servidor apache con "DocumentRoot "c:\web\apache\htdocs\aplicacion\http" y el <Directory "c:\web\apache\htdocs">. 
...
4- ARRANCAR "gsCreate" PARA CONFIGURAR Y CREAR UNA INTRANET NUEVA.
	...
	- Para ejecutar "gsCreate" hay que entrar en la aplicación "edes.php" llamando a http://localhost//edes.v3/ttp/edes.php?gscreate
	...
	- Al ejecutar  http://localhost/edes.v3/http/edes.php?gscreate por primera vez puede mostrarte una pantalla con las indicaciones precisas para que configures tu navegador de forma automatica. Tendras que entrar nuevamente para que los cambios se produzcan. 
...
5- DEFINIR SQL.
	...
	- Nombre de campos "cd_" / "nm_".  Esto permite facilidad de relación en tablas auxiliares que con el nombre el motor E-DES sabe encontrarlas.

	- Si antes del label ponemos una "," el campo se situará a la derecha del anterior.

		#Tab: Expedientes
		#Forder:
		CREATE TABLE prov (               # Nombre carpeta: Descripción
			campo01 char(2),          # Label campo: Descripción si hace falta
			campo02 char(2),          #,Label campo: Descripción si hace falta
			...
			PRIMARY KEY (campo01),
			...
		);
...
Nota: El manual se encuentra en el directorio "edes.v3\t\manuales\edes.chm".


:: Instalación mediante ficheros de configuración
	1- En el directorio "/edes.v3/install"
		1- 
	http://localhost:8080/edes.v3/http/install.php?[PathApplication]&[KeyCode]
