...
file_put_contents('../_tmp/php/'.str_replace('/','_',$OriFichero).'.'.S::$_User.'.req', $txt);
}
function eRequestRun(){
global $OriFichero;
eInit();
$Dim = file('../_tmp/php/'.str_replace('/','_',$OriFichero).'.'.S::$_User.'.req');
echo "<!DOCTYPE HTML>";
echo '<HTML><HEAD></HEAD><BODY>';
if( trim($Dim[0])=='POST' ){
echo '<FORM accept-charset="utf-8" method="post" action="'.trim($Dim[1]).'" style="display:none">';
