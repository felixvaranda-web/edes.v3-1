function eToneSelect(func, cp, aColor){
if( S("#_SELECTCOLOR") ) S("#_SELECTCOLOR").nodeRemove();
var tabla = S("<table id='_SELECTCOLOR' class='TOOLBAR' cellspacing=0 cellpadding=0><tr><th class='TITULO'>Seleccionar color</th><th class='TITULO' width=1px><i class='ICONWINDOW' op='C' onclick='S(S.toTag(S.event(window),\"TABLE\")).none();S.eventClear(event);'>5</i></th></tr><tr><td colspan=2><canvas></canvas><br><canvas></canvas></td></tr></table>").obj,
obj = tabla.rows[1].cells[0].children[0], contexto,
obj2 = tabla.rows[1].cells[0].children[2], contexto2,
ancho = alto = 15,
iColor = "#ffffff",
fColor = "#000000", xyColor=[];
S(tabla).nodeEnd();
S(tabla.rows[0].cells[0]).tooltip("<table><tr><td>Click<td>Selecciona color<tr><td>DblClick<td>Selecciona color y cierra ventana<tr><td>CtrlKey<td>Color inicial<tr><td>AltKey<td>Color final</table>");
if(obj && obj.getContext){
S(obj).css({width:20*ancho, height:10*alto, cursor:"crosshair"});
obj.onclick = function(){
var ev = window.event,
x = ev.offsetX, y = ev.offsetY,
p = obj.getContext('2d').getImageData(x, y, 1, 1).data,
hex = S.rgb2hex(p);
x = Math.floor(x/ancho)*ancho;
y = Math.floor(y/alto)*alto;
marcarColor(x,y,hex);
if( !event.ctrlKey && !event.altKey ) (func)(hex, cp);
...
hex = S.rgb2hex(p);
(func)(hex, cp);
S(S.toTag(S.event(window),'TABLE')).none();
S.eventClear(event);
}
...
i,j,p=0;
if( aColor!=null ) aColor = aColor.toLowerCase();
for(i=0; i<20; i++){
for(j=0; j<10; j++){
if( aColor!=null && aColor=="#"+color[p] ){
xyColor = [i*ancho, j*alto, "#"+color[p]];
}
...
if( xyColor.length>0 ){
marcarColor(xyColor[0], xyColor[1], xyColor[2]);
}
}
...
var ev = event,
x = ev.offsetX, y = ev.offsetY,
p = obj2.getContext('2d').getImageData(x,y, 1,1).data,
hex = S.rgb2hex(p);
contexto2.fillStyle = top.eColorContrastBW(hex);
contexto2.fillRect(x,y, 1,50);
(func)(hex, cp);
}
...
if( aColor==null || aColor=="" ) aColor = "#808080";
tonos(aColor);
}
...
function marcarColor(x,y,hex){
contexto.fillStyle = top.eColorContrastBW(hex);
contexto.fillRect(x-1, y-1, ancho+2, alto+2);
contexto.fillStyle = hex;
