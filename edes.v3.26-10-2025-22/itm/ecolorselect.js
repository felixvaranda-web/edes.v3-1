function eColorSelect(func, cp, aColor){
if( S("#_SELECTCOLOR") ) S("#_SELECTCOLOR").nodeRemove();
var tabla = S("<table id='_SELECTCOLOR' class='TOOLBAR'><tr><th class='TITULO'>Seleccionar color</th><th class='TITULO' width=1px><i class='ICONWINDOW' op='C' onclick='S(S.toTag(S.event(window),\"TABLE\")).none();S.eventClear(event);'>5</i></th></tr><tr><td colspan=2><canvas></td></tr></table>").obj,
obj = tabla.rows[1].cells[0].children[0], contexto,
ancho = alto = 15, xyColor=[];
S(tabla).nodeEnd();
S(S.event(window)).around(tabla, {type:"11,1,12,7,5,6,14"});
S(tabla.rows[0].cells[0]).tooltip("<table><tr><td>Click<td>Selecciona color<tr><td>DblClick<td>Selecciona color y cierra ventana</table>");
