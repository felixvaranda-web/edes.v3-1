...
$status = 0;
for ($i = 0; $i < $len; $i++) {
$status |= (mb_ord($signature[$i]) ^ mb_ord($hash[$i]));
}
...
$max_int_length = mb_strlen((string) PHP_INT_MAX) - 1;
...
return base64_decode(eStrtr($input, '-_', '+/'));
}
public static function urlsafeB64Encode($input){
return str_replace('=', '', eStrtr(base64_encode($input), '+/', '-_'));
}
