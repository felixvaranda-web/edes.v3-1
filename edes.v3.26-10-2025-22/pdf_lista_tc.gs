...
if( $_SESSION["pdf_private"]==$_ENV['OFF'] && $_SESSION["pdf_public"]==$_ENV['OFF'] ) eEnd();
set_time_limit(60*5);
include("../../lib/tcpdf/tcpdf.inc");
if( !isset($_ToPDF) ) $_ToPDF = false;
if( !isset($_BackgroundReport) ) $_BackgroundReport = false;
if( !empty($_THCOLSPAN) ){
for($n=0; $n<count($_THCOLSPAN); $n++){
$_THCOLSPAN[$n] = eClearAccent($_THCOLSPAN[$n]);
}
}
$_TITLE = eClearAccent($_TITLE);
if( !empty($_POST['_ReportAll'])==$_ENV['ON'] && !function_exists('gsActivity') ){
$_POST['_GRAPHS'] = '*';
$_GRAPH = '*';
if( !function_exists('eGraph') ) include(DIREDES.'graph.inc');
$TituloGraph = '';
...
if( !empty($_GRAPH[1]) ) $TituloFila = $_GRAPH[1];
if( !empty($_GRAPH[2]) ) $TituloCol = $_GRAPH[2];
...
if( !isset($_THCOLSPAN[0]) && isset($_ROWSOP) && count($_ROWSOP)==0 ){
$_ROWSOP = array();
for($n=0; $n<count($_Form)-1; $n++) $_ROWSOP[$n] = 'C';
$_ROWSOP[count($_Form)-1] = '+';
}
if( !empty($_GRAPH[3]) ) $TituloCol2 = $_GRAPH[3];
if( eSubstrCount($TipoGraph,'R')>0 ) $xGraphR = eGraph( 'R', $usuCursor, $_Form, $_ROWSOP, $TituloGraph, $TituloCol2, $TituloFila, '', $DimVar );
if( !empty($_GRAPH[4]) ) $TituloCol = $_GRAPH[4];
$TituloLeyenda = ''; if( !empty($_GRAPH[5]) ) $TituloLeyenda = trim($_GRAPH[5]);
...
$_POST['_GRAPHS'] = '';
if( !empty($xGraphP[2]) ) $_POST['_GRAPHS'] .= ','.$xGraphP[2];
if( !empty($xGraphR[2]) ) $_POST['_GRAPHS'] .= ','.$xGraphR[2];
if( !empty($xGraphC[2]) ) $_POST['_GRAPHS'] .= ','.$xGraphC[2];
if( !empty($xGraphG[2]) ) $_POST['_GRAPHS'] .= ','.$xGraphG[2];
if( $_POST['_GRAPHS'][0]==',' ) $_POST['_GRAPHS'] = mb_substr($_POST['_GRAPHS'],1);
}
$_DimCondicion = array();
$sPDF_Ordenacion = '';
...
if( isset($PDF_SQL) && empty($PDF_SQL) ) $PDF_SQL = ' ';
...
$_TITLE = str_replace('#BR#', '<BR>', $_TITLE);
while( eSubstrCount($_TITLE, '  ')>0 ) $_TITLE = str_replace('  ', ' ', $_TITLE);
$_Summary = (isset($_POST["_Summary"]) && $_POST["_Summary"]==$_ENV['ON']);
...
if( empty($PDF_InfoTitulo)			) $PDF_InfoTitulo		 = $_TITLE;
if( !isset($PDF_MarginBottom)		) $PDF_MarginBottom		 = 0;
if( !isset($PDF_LastAfterMarginBottom) ) $PDF_LastAfterMarginBottom = 0;
...
$PDF_Ordenacion = str_replace('ñ','Ñ',mb_strtoupper($PDF_Ordenacion));
...
for( $c=0; $c<$t; $c++ ) $DimMaxAncho[$tmp[$c]] = max($DimMaxAncho[$tmp[$c]], mb_strlen(trim($usuCursor[$n][$tmp[$c]])));
}
for( $n=0; $n<count($usuCursor); $n++ ){
...
if( !empty($PDF_Condition) ) $_DimCondicion = array($PDF_Condition);
}
}
$PDF_TxtCondicion = $__Lng[count($_DimCondicion)<=1 ? 78:43];
if( !empty($_TextOverride['condition']) ){
$PDF_TxtCondicion = $_TextOverride['condition'];
if( $PDF_TxtCondicion=='-' ) $PDF_TxtCondicion = '';
}
$Long = max(mb_strlen(trim($PDF_TxtTitulo)), mb_strlen(trim($PDF_TxtOrdenacion)), mb_strlen(trim($PDF_TxtCondicion)));
if( !empty($PDF_TxtTitulo)		) $PDF_TxtTitulo		= str_pad($PDF_TxtTitulo	, $Long, " ", STR_PAD_LEFT).' ';
if( !empty($PDF_TxtOrdenacion)	) $PDF_TxtOrdenacion	= str_pad($PDF_TxtOrdenacion, $Long, " ", STR_PAD_LEFT).' ';
if( !empty($PDF_TxtCondicion)	) $PDF_TxtCondicion		= str_pad($PDF_TxtCondicion	, $Long, " ", STR_PAD_LEFT).' ';
if( isset($_PDFTH) ){
for($n=1; $n<=9; $n++){
...
if( !empty($_NotInTemporary) ) $_DimCondicion = array();
...
$PDF_TxtCondicion = str_replace('&#43;', '+', $PDF_TxtCondicion);
$PDF_TxtCondicion = str_replace('&#39;', "'", $PDF_TxtCondicion);
$LenMax = max(mb_strlen($PDF_TxtTitulo), mb_strlen($PDF_TxtOrdenacion), mb_strlen($PDF_TxtCondicion));
...
$_Color = array();
ColoresEnPDF();
if( isset($_COLSNOREPEATFILL) && $_COLSNOREPEATFILL ){
$_Color['TD']['COLORFILL'] = eColorTone($_Color['TD']['COLOR'], "","", -75);
}
...
$_ChartMax  = null;
$_ChartLong = null;
if( count($_CHARTCOL)>0 && isset($usuCursor) && $PDF_ChartCol ){
if( empty($_CHARTCOL[2]) ){
$_CHARTCOL[2] = "Gráfica";
}
$n = $_CHARTCOL[0]+1;
...
if( isset($_VIEW) && !empty($conView) ){
if( eSubstrCount($_Form[$n][6], $conView)==0  ){
$_Form[$n][6] = "*";
$_PDFCOL[$n] = 0;
}
}
if( mb_substr($_Form[$n][0],0,6)=='&nbsp;' ) $_Form[$n][0] = mb_substr($_Form[$n][0],6);
if( isset($_PDFBREAKPAGE) ){
...
$Visible[$n] = (eSubstrCount($_Form[$n][6],'*')==0 && eSubstrCount($_Form[$n][6],'L')==0);
if( !$Visible[$n] && $_PDFCOL[$n]>0 ) $Visible[$n] = true;
if( isset($_PDFCOL[$n]) && $_PDFCOL[$n]=='0' ) $Visible[$n] = false;
if( !empty($_FORMAT[$n]) ){
if( mb_strtoupper($_PDFCOL[$n])=='XLS'  ) $_PDFCOL[$n] = '0';
if( mb_strtoupper($_FORMAT[$n])=='IMG'  ) $_PDFCOL[$n] = '0';
if( mb_strtoupper($_FORMAT[$n])=='ICON' ) $_PDFCOL[$n] = '0';
}
if( !empty($_PDFCOL[$n]) && $_PDFCOL[$n]==='0' ) $Visible[$n] = false;
$_ALIGN[$n] = trim(str_replace('id=','',$_ALIGN[$n]));
if( !empty($_COLSOP[$n]) && $_COLSOP[$n]=='S' ){
$Visible[$n] = false;
$ConSubTotales = true;
$_MasColPrimera += 2;
}
if( $_Form[$n][4]==0 || $_Form[$n][3]=="H" ) $Visible[$n] = false;
if( !empty($_COLSOP[$n]) && eSubstrCount('+C#', $_COLSOP[$n])>0 ){
$_InfSinTotales = false;
}
if( !empty($_PDFCOLSSHADE[$n]) && $_PDFCOLSSHADE[$n][0]!='#' ){
...
}else if( !empty($_PDFCOLSSHADE[$n][0]) && $_PDFCOLSSHADE[$n][0]=='#' && !$PDF_Colors ){
$_PDFCOLSSHADE[$n] = RGBToGray($_PDFCOLSSHADE[$n]);
}
if( empty($_PDFCOLBORDER[$n]) ){
$_PDFCOLBORDER[$n] = 0.1;
$PDF_ColStyle[$n] = '';
}else{
...
if( empty($_LISTCHECKBOX['P']) ){
$_SelVirtual[$_Form[$n][1]][$_ENV['ON']]  = $_CheckBox['P']['ON'];
$_SelVirtual[$_Form[$n][1]][$_ENV['OFF']] = $_CheckBox['P']['OFF'];
}else{
$_SelVirtual[$_Form[$n][1]][$_ENV['ON']]  = strip_tags($_LISTCHECKBOX['P'][0]);
$_SelVirtual[$_Form[$n][1]][$_ENV['OFF']] = strip_tags($_LISTCHECKBOX['P'][1]);
}
}
}
$_PDFWRAPCOL = array();
...
if( eSubstrCount($PDF_InfoTitulo, '<')>0 ){
...
$ConTOTALSROWS = ($_TOTALSROWS && isset($_COLSOP) && count($_COLSOP)==0);
...
$p = new TCPDF((($PDF_Horientacion=='V')?'P':'L'), 'pt', 'A4', true, 'UTF-8', false);
$optlist = '';
if( !empty($_POST['_doc_password_']) ) $optlist = 'userpassword='.$_POST['_doc_password_'].' ';
if( !empty($_eDesTitle) ){
$p->SetCreator(eClearAccent($_eDesTitle));
}
if( !empty(SETUP::$System['ApplicationName']) ){
$p->SetAuthor(SETUP::$System['ApplicationName']);
}
if( !empty($PDF_InfoTitulo) ){
if( $PDF_InfoTitulo[0]=='=' ){
$PDF_InfoTitulo = trim(mb_substr($PDF_InfoTitulo, 1));
}
$p->SetTitle($PDF_InfoTitulo);
}
$p->SetKeywords('eDes');
if( isset($l) && gettype($l)=="array" ){
$p->setLanguageArray($l);
$p->setFontSubsetting(true);
}
...
$PDF_Separacion = (float)$PDF_Separacion;
...
$_PDFCOL[(int)$_CHARTCOL[0]+1] = (int)($_CHARTCOL[1]/$PDF_FontSize);
}
$Calcular = false;
}
if( $y>=($HojaAlto-10) ){
$snPag = $nPag;
if( $nPag==1 && !empty($_PDFINCLUDE['S']) ){
...
if( !empty($_ADDOPTION[$_Form[$n][1]]) ) $Celda = $_SelVirtual[$_Form[$n][1]][$Celda];
$Celda = trim(strip_tags($Celda));
if( !empty($_COLSOPPREFIX[$n]) ) $Celda = $_COLSOPPREFIX[$n].' '.$Celda;
...
break;
default:
ePDF_ShowXY($p, mb_substr($Celda,0,$AnchoTotal), (double)($PDF_xMargen+$incr), (double)$y );
}
}else{
...
$_NumReg++;
$SaltoObligado = false;
if( $y<$PDF_LastAfterMarginBottom ){
if( (($TotalReg-$_NumReg)*$AltoLineas) < ($y+$UltimaLinea ) ){
$SaltoObligado = true;
}
}
$SePintoPie = false;
...
ePDF_ShowXY($p, $PDF_TxtLimite, ($HojaAncho-(mb_strlen($PDF_TxtLimite)*$FontPuntos))/2, $y-$AltoLineas);
break;
}
for($n=0; $n<$totalRows; $n++){
if( isset($row[$n]) ){
$_OldValGrupo[$n] = $row[$n];
}
}
}
if( !empty($_BrowseInTemporary) ){
...
if( mb_substr($_FOOTTITLE,0,5)=='echo ' ){
...
break;
case 'C':
$x = ($HojaAncho-(mb_strlen($txt)*$FontPuntos)) / 2;
break;
case 'D':
$x = $HojaAncho-$MargenHoja-(mb_strlen($txt)*$FontPuntos);
break;
default:
$x = -1;
}
...
$dim = explode("/", eGetCWD());
array_pop($dim);
$filePDF = implode("/", $dim)."/_tmp/pdf/lst_{$_User}.pdf";
$p->Output($filePDF, "F");
if( !empty($_PDFINCLUDE['EF']) ){
}
setFileWatermark($oNomFile, $_SubMode, $_FileDF);
...
$_TReg = $_NumReg;
$Extension = "PDF";
include_once(DIREDES.'downloadfile.inc');
eEnd();
function FootTitle(){
...
if( !empty($_PDFINCLUDE['LB']) ){
PDFIncludeLB($p, $HojaAncho, $HojaAlto, $nPag, $GLOBALS['_HeaderLine'], explode('|',$GLOBALS['uBreakPage']), $GLOBALS['AltoLineas']);
}
if( !empty($_TxtFOOTTITLE) ){
$y -= ($ascender/2);
...
if( !empty($_PDFINCLUDE['LA']) ){
PDFIncludeLA($p, $HojaAncho, $HojaAlto, $nPag, $GLOBALS['_HeaderLine'], explode('|',$GLOBALS['uBreakPage']), $GLOBALS['AltoLineas']);
}
return $FootNewPag;
}
function NewFila(){
global $usuCursor, $Indice, $_FileDownload;
if( !empty($_FileDownload) ){
...
$Alto = 0;
if( !empty($PDF_Imagen) ){
$Extension = str_replace('jpg','jpeg', mb_substr($PDF_Imagen, mb_strpos($PDF_Imagen,'.')+1));
if( WINDOW_OS ){
$PDF_Imagen = str_replace('/http/edes.php', '', $_SERVER['SCRIPT_FILENAME']).$PDF_Imagen;
}else{
$PDF_Imagen = eScript($PDF_Imagen);
...
$y = $HojaAlto - $PDF_yMargen;
if( !empty($PDF_TxtCabecera) ){
if( $PDF_Colors ) eSetTextColor($p, $_Color['COMPANY']['COLOR'], 'text');
else eSetTextGris(0.75);
ePDF_ShowXY($p, $PDF_TxtCabecera, $Ancho+$MargenHoja, $y);
}
if( $PDF_Fecha ){
while( eSubstrCount($PDF_TxtFecha, '#')>1 ){
$tmp = trim(mb_substr($PDF_TxtFecha, mb_strpos($PDF_TxtFecha, '#')+1));
$tmp = trim(mb_substr($tmp, 0, mb_strpos($tmp, '#')));
if( $tmp=='F' || $tmp == 'D'){
...
ePDF_ShowXY($p, $PDF_TxtFecha, $HojaAncho-$MargenHoja-(mb_strlen($PDF_TxtFecha)*$FontPuntos), $y);
if( $PDF_CdScript ){
eSetTextColor( $p, $_Color['CDSCRIPT']['COLOR'], 'text' );
$txt = _CodigoScript();
ePDF_ShowXY( $p, $txt, $HojaAncho-$MargenHoja-(mb_strlen($txt)*$FontPuntos), $y-$AltoLineas+($AltoLineas/4) );
}
}
if( !empty($_DocLabelUserTo) || !empty($_DocLabelUserFrom) ){
if( !empty($_DocLabelUserTo) && !empty($_DocLabelUserFrom) ){
$_DocLabelUserTo = ePDF_ReplaceChr($_DocLabelUserTo);
$_DocLabelUserFrom = ePDF_ReplaceChr($_DocLabelUserFrom);
$Long = max(mb_strlen($_DocLabelUserTo),mb_strlen($_DocLabelUserFrom));
...
if( empty($_POST['_doc_to_']) ){
ePDF_ShowXY( $p, ePDF_ReplaceChr($_POST['_doc_from_']), 10, $PDF_y2Margen );
}else{
if( !empty($_DocLabelUserFrom) ){
ePDF_ShowXY( $p, ePDF_ReplaceChr($_DocLabelUserFrom.': '.$GLOBALS['_UsuarioDelPDF']), 10, $PDF_y2Margen+((!empty($_DocLabelUserTo)) ? $AltoLineas : 0 ) );
}
if( !empty($_DocLabelUserTo) ){
ePDF_ShowXY( $p, ePDF_ReplaceChr($_DocLabelUserTo.': '.$_POST['_doc_to_']), 10, $PDF_y2Margen );
}
}
}
...
if( !empty($PDF_Titulo) && !empty($PDF_TxtTitulo) ){
...
$sx += mb_strlen($PDF_TxtTitulo)*$FontPuntos;
$GLOBALS['_yUltimaLinea'] = $y;
...
$sx += mb_strlen($PDF_TxtOrdenacion)*$FontPuntos;
if( $PDF_Colors ) eSetTextColor( $p, $_Color['CIRCLE']['COLOR'], 'fill' );
$p->Circle($sx+$cx, getY($y+$FontPuntos/2)+$cy, 1.5, 0, 360, 'F');
...
if( ($sx+(1*$FontPuntos)+mb_strlen($_DimCondicion[$n])*$FontPuntos)>($HojaAncho-$MargenHoja-$PDF_ImgMarginRight) ){
...
if( !empty($_PDFINCLUDE['FH']) ){
PDFIncludeFH($p, $HojaAncho, $HojaAlto, $nPag, $GLOBALS['_HeaderLine'], explode('|',$GLOBALS['uBreakPage']), $GLOBALS['AltoLineas']);
}
}
if( (count($_THCOLSPAN)>0 && $nPag==-1) || (count($_THCOLSPAN)==0 && $nPag!=-1) ){
if( !empty($_PDFINCLUDE['EH']) ){
PDFIncludeEH($p, $HojaAncho, $HojaAlto, $nPag, $GLOBALS['_HeaderLine'], explode('|',$GLOBALS['uBreakPage']), $GLOBALS['AltoLineas']);
}
}
$_RowSpan = array();
$MinY = $y;
...
if( !empty($_CHECKLIST[ $_Form[$n][1] ][2]) ){
$tmp = explode(',',$Celda);
$Celda = '';
...
if( !empty($_RADIOLIST[ $_Form[$n][1] ][2]) ){
if( empty($_RADIOLIST[ $_Form[$n][1] ][2]) ){
...
}else if( $_SELECTMULTIPLE[ $_Form[$n][1] ] > 0 && !empty($Celda) ){
...
$Celda = call_user_func(mb_substr($Formato,9,-3), $n, $Celda);
}else{
...
$Celda = str_replace('&quot;', '"', $Celda);
$Celda = str_replace('&nbsp;', ' ', $Celda);
if( ($_PDFWRAP[0]>1 && mb_strlen($Celda)>$_AnchoCol[$n] && $_PDFWRAPCOL[$n]) || $_Form[$n][3]=='H' || $_SELECTMULTIPLE[ $_Form[$n][1] ]>0 ){
if( !$_PDFWRAP[1] ){
$Celda = str_replace("\n",' ',$Celda);
...
for($n=0; $n<count($row); $n++){
if( !empty($_COLSOP[$n]) && $_COLSOP[$n]=='L' ){
...
$_SummaryGroupTitle[$n] = str_repeat(' ',mb_strlen($sCelda)-mb_strlen(trim($sCelda))).$_Form[$n][0].': '.trim($sCelda);
if( !$_SummaryNoHeaders ){
if( $ConSubTotales && $PDF_Colors ){
eSetTextColor( $p, $_Color['GROUPHEADER_'.($n+1)]['COLOR'], 'fill' );
SetFuente('GROUPHEADER_'.($n+1));
}
ePDF_ShowXY($p, mb_substr($sCelda,0,$AnchoTotal), (double)($PDF_xMargen+$incr), (double)$y);
SetFuente('NORMAL');
for($i=0; $i<count($row); $i++) $_RowSpan[$i+1][] = $y;
$y -= ($AltoLineas*1);
}
...
if( (isset($PDF_ShadowRows[$_NumReg]) && $PDF_ShadowRows[$_NumReg]==true)
||
(isset($_PDFRowShadow) && $_PDFRowShadow==true)
){
eSetFillColor(0.90);
eRect($PDF_xMargen, getY($y-($ascender/2),($AltoLineas*1)), $MargenDerecho, ($AltoLineas*1), 'DF');
}
...
if( $PDF_Colors ){
if( (
($_GREENBAR && ($_NumReg % 2)!=0)
||
(
(isset($PDF_ShadowRows[$_NumReg]) && $PDF_ShadowRows[$_NumReg]==true)
||
(isset($_PDFRowShadow) && $_PDFRowShadow==true)
)
)
){
if( !empty($PDF_ColorRows[$_NumReg]) ){
...
if( isset($_ROWSOP) && count($_ROWSOP)>0 ){
if(      $_ROWSOP[$n]=='+' ) $_OpLin += floatval($row[$n]);
else if( $_ROWSOP[$n]=='-' ) $_OpLin -= floatval($row[$n]);
else if( $_ROWSOP[$n]=='*' ) $_OpLin *= floatval($row[$n]);
}
if( isset($Visible[$n]) && !$Visible[$n] ) continue;
$Celda = $row[$n];
if( !empty($_COLSOP[$n]) ){
switch( $_COLSOP[$n] ){
case '+':
case 'M':
$_OpCol[$n] += floatval($Celda);
...
if( !empty($_ADDOPTION[$_Form[$n][1]]) ){
if( isset($_SelVirtual[$_Form[$n][1]]) ){
if( isset($_SelVirtual[$_Form[$n][1]][$Celda]) ){
$Celda = $_SelVirtual[$_Form[$n][1]][$Celda];
}else{
$Celda = "";
}
...
$Celda = trim($Celda);
if( $_Form[$n][2]=="+," || $_Form[$n][2]=="-," ){
$Celda = (float)$Celda;
}else if( $_Form[$n][2]=="+" || $_Form[$n][2]=="-" ){
$Celda = (int)$Celda;
}
if( $_Form[$n][3]=='H' ){
$Celda = strip_tags( str_replace('<BR>',"\n",$Celda) );
...
}else if( isset($_SELECTMULTIPLE[$_Form[$n][1]]) && $_SELECTMULTIPLE[$_Form[$n][1]]>0 && !empty($Celda) ){
if( $_SelVirtualType[$_Form[$n][1]]!='T' ){
$tmp = explode(',',$Celda);
$Celda = '';
for($i=1; $i<count($tmp)-1; $i++){
...
if( !empty($_NOZERO[$n]) && $_NOZERO[$n]=='S' ){
if( isZero($Celda) ) $Celda = '';
}
}
}
...
if( isset($_PDFWRAP[0]) && $_PDFWRAP[0]>1 ){
}
if( mb_strlen($Celda)>$_AnchoCol[$n] ){
}
if( $_PDFWRAPCOL[$n] ){
}
if( isset($_PDFWRAP[0]) && $_PDFWRAP[0]>1 && mb_strlen($Celda)>$_AnchoCol[$n] && $_PDFWRAPCOL[$n] ){
}
if( $_Form[$n][3]=='H' ){
}
if( isset($_SELECTMULTIPLE[$_Form[$n][1]]) && $_SELECTMULTIPLE[$_Form[$n][1]]>0 ){
}
if(
(isset($_PDFWRAP[0]) && $_PDFWRAP[0]>1 && mb_strlen($Celda)>$_AnchoCol[$n] && $_PDFWRAPCOL[$n])
|| ($_Form[$n][3]=='H' || $_Form[$n][3]=='A')
|| (isset($_SELECTMULTIPLE[$_Form[$n][1]]) && $_SELECTMULTIPLE[$_Form[$n][1]]>0)
){
if( !$_PDFWRAP[1] ){
$Celda = str_replace("\n",' ',$Celda);
$Celda = str_replace("\r",' ',$Celda);
...
for($i=0; $i<count($NewTxt); $i++){
$NewTxt[$i] = trim($NewTxt[$i]);
if( !empty($NewTxt[$i]) ) $MultiLinea[$n][] = str_repeat(' ',$indent).trim($NewTxt[$i]);
}
$Celda = $MultiLinea[$n][0];
if( isset($MultiLinea[$n]) && gettype($MultiLinea[$n])=="array" ){
if( $ConPDFWrap<count($MultiLinea[$n]) ){
$ConPDFWrap = count($MultiLinea[$n]);
}
if( $_PDFWRAP[0]<count($MultiLinea[$n]) ){
$MultiLinea[$n][$_PDFWRAP[0]-1] = mb_substr($MultiLinea[$n][$_PDFWRAP[0]-1], 0, $_AnchoCol[$n]-3).'...';
}
...
$oCelda = $Celda;
$Celda = mb_substr($Celda, 0, $_AnchoCol[$n]);
if( !empty($_ALIGN[$n]) ){
if( $_ALIGN[$n]=='d' ){
if( mb_strlen($oCelda."")>$_AnchoCol[$n] ){
$Celda = str_repeat("*",$_AnchoCol[$n]);
}else{
$Celda = mb_substr(str_repeat(' ', $_AnchoCol[$n]).$Celda, -($_AnchoCol[$n]));
}
}elseif( $_ALIGN[$n]=='c' ){
$Celda = str_repeat(' ', ($_AnchoCol[$n]-mb_strlen(str_replace('&lt;','<',strip_tags($Celda))))/2).$Celda;
}
}
switch( gettype($_PDFBackgroundColor) ){
case 'NULL':
if( isset($_PDFCOLSSHADE[$n]) ){
$BackgroundColor = $_PDFCOLSSHADE[$n];
}
break;
case 'array':
if( isset($_PDFBackgroundColor[$n]) ){
$BackgroundColor = $_PDFBackgroundColor[$n];
}
break;
case 'string':
if( isset($_PDFBackgroundColor) ){
...
$enGris = false;
if( isset($_COLSNOREPEAT[$n]) && $_COLSNOREPEAT[$n] ){
if( $_COLSBAK[$n]===$Celda ){
...
case 'array':
if( !empty($_PDFColor[$n]) ) eSetTextColor($p, $_PDFColor[$n], 'text');
break;
case 'string':
if( !empty($_PDFColor) ) eSetTextColor($p, $_PDFColor, 'text');
break;
}
if( !$_Summary ){
...
if( isset($_CHARTCOL[0]) && $_CHARTCOL[0]>0 ){
...
if( isset($PDF_LineRows[$_NumReg]) && $PDF_LineRows[$_NumReg]==true ){
...
$Celda = mb_substr($MultiLinea[$n][$i], 0, $_AnchoCol[$n]);
...
$n = dechex(255-$n*255);
if( mb_strlen($n."")==1 ) $n = "0".$n;
return "#".$n.$n.$n;
}
function Pie(){
global $p, $PDF_xMargen, $MargenHoja, $ascender, $nPag, $HojaAncho, $HojaAlto, $PDF_Colors, $_Color, $_FORMATTOTALSCS;
global $_AnchoPt, $FontPuntos, $InicioY, $y, $AltoLineas, $PDF_y2Margen, $_RowSpan, $_PDFINCLUDE, $PDF_OffsetPie;
global $PDF_AnchoSombra, $PDF_OffsetSombra, $PDF_NumPagina, $PDF_TxtPagina, $_AltoTH, $_PDFCOLBORDER, $_SePintoPie, $_PDFSHEET;
if( !empty($_PDFINCLUDE['EB']) || ($nPag==1 && !empty($_PDFINCLUDE['FB'])) ){
eResetColor($p);
}
$_SePintoPie = true;
if( $nPag==1 && !empty($_PDFINCLUDE['FB']) ){
PDFIncludeFB($p, $HojaAncho, $HojaAlto, $nPag, $GLOBALS['_HeaderLine'], explode('|',$GLOBALS['uBreakPage']), $GLOBALS['AltoLineas']);
}
if( !empty($_PDFINCLUDE['EB']) ){
PDFIncludeEB($p, $HojaAncho, $HojaAlto, $nPag, $GLOBALS['_HeaderLine'], explode('|',$GLOBALS['uBreakPage']), $GLOBALS['AltoLineas']);
}
...
if( $_AnchoPt[$n]>0 && $_PDFCOLBORDER[$n+1]!=0 ){
$p->SetLineWidth($_PDFCOLBORDER[$n+1]);
if( (isset($_RowSpan[$n]) && gettype($_RowSpan[$n])=="array" && count($_RowSpan[$n])==0) || $n+1==$tLineas ){
...
$sIni = $InicioY+$Des;
$sFin = $y+$Des;
if( isset($_RowSpan[$n]) && gettype($_RowSpan[$n])=="array" ){
...
if( !empty($_PDFINCLUDE['EA']) || ($nPag==1 && !empty($_PDFINCLUDE['FA'])) ){
eResetColor($p);
}
if( !empty($_PDFINCLUDE['EA']) ){
PDFIncludeEA($p, $HojaAncho, $HojaAlto, $nPag, $GLOBALS['_HeaderLine'], explode('|',$GLOBALS['uBreakPage']), $GLOBALS['AltoLineas']);
}
if( $nPag==1 && !empty($_PDFINCLUDE['FA']) ){
PDFIncludeFA($p, $HojaAncho, $HojaAlto, $nPag, $GLOBALS['_HeaderLine'], explode('|',$GLOBALS['uBreakPage']), $GLOBALS['AltoLineas']);
}
$y -= $PDF_OffsetPie;
return;
}
...
if( isset($_COLSOP) && count($_COLSOP)==0 ) return;
...
if( mb_substr($Formato,-3)=='();' ){
$Celda = call_user_func(mb_substr($Formato,9,-3), $n, $_OpCol);
}else if( $Celda==="" ){
}else{
@eval($Formato);
}
...
if( $_COLSOP[$n][0]=='"' || $_COLSOP[$n][0]=="'" ) $Celda = mb_substr($_COLSOP[$n],1,-1);
$Saltar = true;
}elseif( $_COLSOP[$n]=='#' ){
}else{
}
...
if( $n==$_TGrupos ){
if( !empty($_TextGrupo[$NivelGrupo]) ){
...
$Celda = str_repeat(' ', ($_AnchoCol[$n]-mb_strlen($Celda))/2).$Celda;
}
}else if( !empty($_ALIGN[$n]) ){
...
if( !empty($_ALIGN[$n]) ){
if( $_ALIGN[$n]=='d' ){
$Celda = mb_substr($Celda, -($_PDFCOL[$n]));
}elseif( $_ALIGN[$n]=='c' ){
}
}else{
$Celda = mb_substr($Celda, 0, $_PDFCOL[$n]);
}
}
if( $sLen>mb_strlen($Celda) ){
if( $PintaGrupo ){
$Celda = mb_substr($Celda, 0, $_nGrupoColSpan);
}else{
$Celda = str_repeat('*', mb_strlen($Celda));
}
}
if( !$SaltarCelda[$n] ){
if( $PDF_Colors ){
...
if( $_SummaryNoHeaders && $ConGrupos && !empty($_SummaryGroupTitle[$NivelGrupo]) && $n==$_TGrupos ){
$Celda = $_SummaryGroupTitle[$NivelGrupo];
$_SummaryGroupTitle[$NivelGrupo] = '';
}
$_vF[$n] = str_replace(array(".",","), array("","."), $Celda);
if( $n==count($_AnchoPt)-1 && isset($_ROWSOP) && count($_ROWSOP)>0 ){
...
$Celda = mb_substr($_FORMATTOTALS[$n],1,-1);
}else{
$Formato = '$Celda = '.str_replace('#', $Celda, $_FORMATTOTALS[$n]).';';
@eval($Formato);
}
...
}else if( $SubTotal && !empty($_TextGrupo[$NivelGrupo]) ){
}else if( $_FORMATTOTALS[$n][0]=="'" || $_FORMATTOTALS[$n][0]=='"' ){
$Celda = mb_substr($_FORMATTOTALS[$n],1,-1);
if( !empty($_FORMATTOTALSALIGN) ){
$totalColSpan = 0;
...
$iv++;
$Celda = $_Form[$n][0];
if( !empty($_PDFTH[$n]) ) $Celda = $_PDFTH[$n];
$Celda = str_replace('&nbsp;', ' ' , $Celda);
$Celda = str_replace('&NBSP;', ' ' , $Celda);
$Celda = str_replace('&#92;' , '\\', $Celda);
$Celda = str_replace('&#43;' , '+' , $Celda);
if( $PDF_UpperCabecera ){
$Celda = mb_strtoupper($Celda);
}else{
if( eSubstrCount($Celda, '<') > 0 ){
...
if( !isset($Decimales) ) $Decimales = 0;
if( (int)$Decimales>0 ) $Ancho += ($Decimales*1)+1;
if( isset($_COLSOP[0]) && $_COLSOP[0]=="S" && $iv==0 ) $Ancho += $_MasColPrimera;
if( !empty($Alto) ) $Ancho = $Decimales;
if( !empty($_ADDOPTION[$_Form[$n][1]]) ){
foreach($_SelVirtual[$_Form[$n][1]] as $valor){
$Ancho = max($Ancho, mb_strlen($valor));
}
}else{
...
if( $Ancho<mb_strlen($Titulos[$n][$i]) ) $Ancho = mb_strlen($Titulos[$n][$i]);
$xTitulos[$n][$i] = $sX;
}
if( count($_AnchoColSpan)>0 ){
$Ancho = max($Ancho, $_AnchoColSpan[$n]);
}
if( isset($_PDFCOL[$n]) && $_PDFCOL[$n]!=0 && $Ancho!=$_PDFCOL[$n] ){
$Ancho = $_PDFCOL[$n];
}
$Ancho = (float)($Ancho);
...
for($i=0; $i<count($Titulos[$n]); $i++){
$xTitulos[$n][$i] = $xTitulos[$n][$i]+((($Ancho-mb_strlen($Titulos[$n][$i]))*$FontPuntos)/2)+(($PDF_Separacion*$FontPuntos)/2);
}
...
$i = count($Titulos[$n]);
$MaxI = max($i,$MaxI);
if( empty($yTitulos[$n]) ) $yTitulos[$n] = -((($MaxFilas-$i)*$AltoLineas)/2)-($ascender/2);
}
$MargenDerecho = $sX;
$AnchoTotal = 0;
...
$PDF_Imagen = $d[3];
$PDF_Imagen = eScript($PDF_Imagen);
if( !empty($d[5]) ) $MaxAncho = $d[5];
if( !empty($d[6]) ) $MaxAlto = $d[6];
...
$sAncho = $Ancho;
$sAlto = $Alto;
if( empty($d[5]) ) $MaxAncho = $Ancho;
if( empty($d[6]) ) $MaxAlto = $Alto;
$zoom = 1;
switch( trim($d[4]) ){
case '#':
break;
...
$Ancho = $Ancho * $zoom;
$Alto = $Alto * $zoom;
if( mb_strtoupper($d[0])=="R" ) $x = $HojaAncho-$MargenHoja-$x-$Ancho;
...
$PDF_Imagen = eScript($PDF_Imagen);
if( !empty($d[5]) ) $MaxAncho = cW($d[5]);
if( !empty($d[6]) ) $MaxAlto = cH($d[6]);
$TxtError = '';
$stmp = explode('.', $PDF_Imagen);
$Extension = $stmp[count($stmp)-1];
$Extension = str_replace('jpg', 'jpeg', $Extension);
...
if( empty($Color) || gettype($Color)=="NULL" ) return;
if( is_numeric($Color) ){
$p->SetColor($Tipo, 255*$Color);
return;
}
list($xr, $xg, $xb) = explode("\n", mb_chunk_split(str_replace('#','',$Color), 2));
...
$nmFile = "pdf.css";
if( isset($_SESSION["tmp"]['VAR_TMP']) ){
if( isset($_SESSION["tmp"]['VAR_TMP']["pdf.css"]) ){
$nmFile = $_SESSION["tmp"]['VAR_TMP']["pdf.css"];
unset($_SESSION["tmp"]['VAR_TMP']["pdf.css"]);
}
}
...
list( $Regla ) = explode('{',mb_strtoupper($Dim[$n]));
$Regla = trim($Regla);
if( $Regla[0] == '#' || $Regla[0] == '.' ) $Regla = mb_substr($Regla,1);
$i = $n;
while( eSubstrCount($Dim[++$i],'}')==0 ){
list( $Dim[$i] ) = explode('/'.'*',$Dim[$i]);
list( $NomColor, $ValColor ) = explode(':',mb_strtoupper($Dim[$i]));
$NomColor = trim($NomColor);
$ValColor = trim($ValColor);
if( mb_substr($ValColor,-1)==';' ) $ValColor =  mb_substr($ValColor,0,-1);
if( !$PDF_Colors ) $ValColor = RGBToGray($ValColor);
$_Color[$Regla][$NomColor] = $ValColor;
}
}
}
if( empty($_Color['CDSCRIPT']['COLOR'])		 ) $_Color['CDSCRIPT']['COLOR'] = '#eeeeee';
if( empty($_Color['SHOWRECIPIENT']['COLOR']) ) $_Color['SHOWRECIPIENT']['COLOR'] = '#cccccc';
if( empty($_Color['FOOTTITLE']['COLOR'])	 ) $_Color['FOOTTITLE']['COLOR'] = '#000e4b';
}
...
global $p;
switch( str_replace('-','',mb_strtoupper($Font)) ){
case 'COURIER':
...
if( $n+$THCol[$n][1]==$NCampos && count($_CHARTCOL)>0 && empty($_CHARTCOL[2]) ) $THCol[$n][1]++;
...
$OldMaxFilas = 0;
for($n=0; $n<count($_Form); $n++){
$OldMaxFilas = max($OldMaxFilas, !empty($OldTitulos[$n])? count($OldTitulos[$n]):1);
$Titulos[$n] = '';
...
$Ini = mb_strpos($PDF_Titulo,'<');
$Fin = mb_strpos($PDF_Titulo,'>');
if( $Ini===false ){
}else{
if( $Fin===false ){
}else{
if( $Ini < $Fin && mb_substr($PDF_Titulo,$Ini+1,1) > ' ' ) $PDF_Titulo = mb_substr($PDF_Titulo,0,$Ini).mb_substr($PDF_Titulo,$Fin+1);
}
}
...
function eColorTone($r, $g="", $b="", $t=0){
if( empty($r) ) return '#000000';
if( empty($b) ){
...
if( mb_strtoupper($AnchoAlto)=='IMAGEWIDTH' ){
...
$tmp = explode('.',$Imagen);
$p->Image( $Imagen, $xImg, getY($yImg, $sh*$Zoom), $sw*$Zoom, $sh*$Zoom, mb_strtoupper($tmp[count($tmp)-1]) );
}
...
$ancho = mm($ancho);
$alto = mm($alto);
if( !empty($cnf["color"]) ) $GLOBALS["__color_text"] = $cnf["color"];
if( !empty($cnf["size"]) ) $GLOBALS["__size"] = $cnf["size"];
if( !empty($cnf["font"]) ) $GLOBALS["__font"] = $cnf["font"];
if( !empty($cnf["textAlign"]) ) $cnf["textAlign"] = trim(mb_strtoupper($cnf["textAlign"]))[0];
else $cnf["textAlign"] = "";
pFont($GLOBALS["__font"], $GLOBALS["__size"]);
pColor($GLOBALS["__color_text"], "text");
if( !empty($cnf["borderColor"]) ) $GLOBALS["__color_draw"] = $cnf["borderColor"];
pColor($GLOBALS["__color_draw"], "draw");
if( !empty($cnf["backgroundColor"]) ) $GLOBALS["__color_fill"] = $cnf["backgroundColor"];
pColor($GLOBALS["__color_fill"], "fill");
if( !empty($cnf["width"]) ) $GLOBALS["__width"] = $cnf["width"];
...
if( !empty($cnf["rotate"]) ) $p->StopTransform();
...
$x2 = mm($x2);
$y2 = mm($y2);
if( !empty($cnf["color"]) ) $GLOBALS["__color_draw"] = $cnf["color"];
if( !empty($cnf["width"]) ) $GLOBALS["__width"] = $cnf["width"];
pColor($GLOBALS["__color_draw"], "draw");
$p->SetLineStyle(array('width'=>$GLOBALS["__width"]));
$p->Line($x, $y, $x2, $y2);
...
if( !empty($cnf["borderColor"])     ) $GLOBALS["__borderColor"] = $cnf["borderColor"];
if( !empty($cnf["backgroundColor"]) ) $GLOBALS["__backgroundColor"] = $cnf["backgroundColor"];
...
$Extension = str_replace('jpg','jpeg', mb_substr($file, mb_strpos($file,'.')+1));
if( WINDOW_OS ){
