...
global $_eAddEvento, $_AlertsTables, $_User;
$_eAddEvento = array();
...
SS::query("select cd_{$TABLA}, new_hour_ev, dt_date_ev, hour_ev, nm_{$TABLA} from {$TABLA}
where ( status_ev='P' or status_ev='p' ) and cd_gs_user='{$_User}' and dt_new_date_ev<='".date('Y-m-d')."'"
);
while( $row = SS::get() ){
eAddEvent(
$row["cd_{$TABLA}"],
...
SS::query("select u.cd_{$TABLA}_user, u.new_hour_ev, e.dt_date_ev, e.hour_ev, e.nm_{$TABLA} from {$TABLA}_user u, {$TABLA} e
where ( u.status_ev='P' or u.status_ev='p' ) and u.cd_gs_user='{$_User}' and u.dt_new_date_ev<='".date('Y-m-d')."' and u.cd_{$TABLA}=e.cd_{$TABLA}"
);
while( $row = SS::get() ){
eAddEvent(
$row["cd_{$TABLA}_user"],
