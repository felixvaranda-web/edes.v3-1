...
die('GRABADO');
$txt = $_POST['_TAREAS'];
$Ini = mb_stripos( $txt, '<TBODY>' )+7;
$Fin = mb_stripos( $txt, '</TBODY>' );
$txt = mb_substr( $txt, $Ini, $Fin-$Ini );
$Dim = explode( '<TR>', $txt );
for( $n=2; $n<count($Dim); $n++ ){
$txt = mb_substr(trim($Dim[$n]),7,-5-11);
list( $tipo, $txt ) = explode( '<DIV ', $txt );
$tipo = (mb_substr(trim($tipo),0,-1)=='EDcs')?'C':'T';
$i = mb_strpos( $txt, '>' );
$indice = mb_substr( $txt, 0,$i );
$datos = mb_substr( $txt, $i+1 );
...
<FORM accept-charset="utf-8" METHOD=POST ACTION="edes.php?E:$glist.gs&F=<?=$_GET['F']?>" style="display:none">
