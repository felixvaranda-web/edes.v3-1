...
if( mb_substr($_DownloadPath,-1)!='/' && mb_substr($_DownloadPath,-1)!='\\' ) $_DownloadPath .= '/';
...
$_DirRoot = eGetCWD();
if( mb_substr($_DirRoot,-1) != '/' ) $_DirRoot .= '/';
$sg = gettimeofday();
$_SecondINI = (int)$sg["sec"];
$def__online = str_replace(' ','',trim(mb_strtoupper($def__online)));
$_VirtualVALUE = array();
$_VirtualFIELD = array();
$_EjecutaIF_EVAL = array();
$_EjecutarIF_FIELD = array();
$_cd_gs_formato = $_POST['def__cd_gs_formato'];
if( eSubstrCount($def__online, '2')>0 ) $def__online = '2';
...
$_UnloadWrapWidth = eFileGetVar("System.UnloadWrapWidth");
...
global $_DEBUG, $cd_gs_destino, $_User, $cd_gs_formato, $cd_gs_entidad, $_BackgroundShow, $dimAling;
...
if( mb_substr( $DimNomVar[$n],0,5 ) == 'def__' ){
$NomVar = mb_substr( $DimNomVar[$n], 5 );
${$NomVar} = $DimValor[$n];
continue;
}
...
if( mb_strlen($CamposDeCondi) > 0 ) $CamposDeCondi .= ',';
$CamposDeCondi .= "'{$DimNomVar[$n]}'";
$ValCount[ $DimNomVar[$n] ] = $DimValor[$n];
}
}
SS::query("select * from {$_ENV['SYSDB']}gs_formato where cd_gs_formato='{$cd_gs_formato}' and cd_gs_user='{$_User}'");
$Formato = SS::get();
...
SS::query( "select * from {$_ENV['SYSDB']}gs_entidad where cd_gs_entidad='{$cd_gs_entidad}'" );
$Entidad = SS::get();
...
$Tabla[] = $gs_tabla;
$Alias[$gs_tabla] = mb_chr($NumAlias++);
...
eTrace( "SQL...........: select tabla,campo,relacion,etiqueta from {$_ENV['SYSDB']}gs_campo where cd_gs_entidad='{$cd_gs_entidad}' and campo in ( $CamposDeCondi )" );
eTrace('');
}
$Busca = '';
if( $CamposDeCondi != '' ){
$TCamposExist = SS::count("{$_ENV['SYSDB']}gs_campo", "cd_gs_entidad='{$cd_gs_entidad}' and campo in ( $CamposDeCondi )" );
SS::query( "select tabla,campo,relacion,etiqueta from {$_ENV['SYSDB']}gs_campo where cd_gs_entidad='{$cd_gs_entidad}' and campo in ( $CamposDeCondi )" );
if( $_Debug2Plano ) eTrace('[ CAMPOS DE LA CONDICION ]');
while( $DefCampo = SS::get() ){
...
$Alias[$DefCampo['tabla']] = mb_chr($NumAlias++);
...
if( $_POST['_SqlToTron_']==$_ENV['ON'] ) eTron('>>> Faltan campos de condición en "gs_campo" -> '."cd_gs_entidad='{$cd_gs_entidad}' and campo in ( $CamposDeCondi )" );
for( $n=0; $n<count($Dim); $n++ ){
if( $DimAliasCampo[ trim($Dim[$n]) ] == '' ) {
$DimAliasCampo[ trim($Dim[$n]) ] = 'A';
if( $_POST['_SqlToTron_']==$_ENV['ON'] ) eTron('  > Faltan campos de condición: Se asigna "A" al campo '.trim($Dim[$n]) );
}
}
}
...
if( mb_substr( $xCampo, 0, 5 ) == 'def__' ) continue;
...
if( mb_substr( $DimNomVar[$n], 0, 1 ) == '_' ) continue;
if( mb_substr( $DimNomVar[$n], 0, 5 ) == 'def__' ) continue;
...
if( SS::count( "{$_ENV['SYSDB']}gs_formato", "cd_gs_formato='{$cd_gs_formato}' and cd_gs_user='{$_User}'" ) == 0 ){
...
SS::query( "select * from {$_ENV['SYSDB']}gs_campo where cd_gs_campo={$DimCampos[$n]}" );
$DimCampo = SS::get();
...
$Alias[$DimCampo['tabla']] = mb_chr($NumAlias++);
...
$RastroCampo = false;
if( $DimCampo['tipo']!='' && eSubstrCount( $DimCampo['tipo'], '#') > 0 ){
...
if( eSubstrCount( $DimCampo['tipo'], '(') == 1 && eSubstrCount( $DimCampo['tipo'], ')') == 1 ){
...
if( eSubstrCount( $DimCampo['tipo'], '#') > 0 ){
...
if( mb_substr( trim($DimCampo['campo']), 0, 1 ) != '(' ){
...
$Estructura[] = array( trim(mb_substr( $ListaCampos, mb_strlen($sListaCampos) )), $DimCampo['ancho'], $DimCampo['decimales'], $DimCampo['alineacion'], $DimCampo['campo'] );
if( $_Debug2Plano ){
eTrace( '>>> '.$DimCampo['campo'].' - '.$DimCampo['ancho'].' - '.$DimCampo['decimales'].' - '.$DimCampo['alineacion'] );
}
$nCol = $n+1;
switch( mb_strtoupper($DimCampo['alineacion']) ){
case 'I':
$ListaAlineacion[$n] = 'i';
break;
case 'C':
$ListaAlineacion[$n] = 'c';
array_push($dimAling, "th:nth-child({$nCol}), td:nth-child({$nCol}){text-align:center;}");
break;
case 'D':
$ListaAlineacion[$n] = 'd';
array_push($dimAling, "th:nth-child({$nCol}), td:nth-child({$nCol}){text-align:right;}");
break;
case 'L':
$ListaAlineacion[$n] = 'L';
break;
...
for( $i=mb_strlen($ListaRelacion[$n])-1; $i>=0; $i-- ){
$c = mb_substr($ListaRelacion[$n],$i,1 );
if( $c == '.' ){
$final = $i;
$inicio = -1;
$OnOff = true;
$DimPosTabla[++$NumTabla] = array( $inicio, $final );
}else if( eSubstrCount( " ('=", $c ) > 0 && $OnOff ){
$inicio = $i;
$OnOff = false;
...
$sTabla = mb_substr( $ListaRelacion[$n], $inicio, $final );
if( $_Debug2Plano ) eTrace( '['.$sTabla.'] - '.mb_strlen(mb_substr( $ListaRelacion[$n], $inicio, $final )) );
if( $_Debug2Plano ) eTrace( 'ListaRelacion.i['.$ListaRelacion[$n].']' );
$ListaRelacion[$n] = mb_substr( $ListaRelacion[$n],0,$inicio ).
$Alias[$sTabla].'.'.
mb_substr( $ListaRelacion[$n], $DimPosTabla[$i][1]+1 );
...
$nombre = mb_substr( $GLOBALS['TempFile'], mb_strrpos($GLOBALS['TempFile'],'/') + 1 );
...
$DelFile = $NomFile;
$NomFile .= '.'.mb_strtolower($cd_gs_destino);
$tReg = 0;
$gsTipo = 'L';
if( $cd_gs_destino=='REC' ) $gsTipo = 'R';
if( $GLOBALS['_TronDownLoad'] ) error_log( "Antes de SI Graba exportación 1\n", 3, '___download__.info' );
if( eSubstrCount($def__online,'H')>0 || eSubstrCount($def__online,'2')>0 ){
if( $GLOBALS['_TronDownLoad'] ){
...
if( $cd_gs_destino!='REC' ){
if( $_POST['_SqlToTron_']==$_ENV['ON'] ) eTron("select {$ListaCampos} from {$ListaTablas} where {$Condicion} order by {$ListaOrdenacion}");
...
$tReg = SS::count($ListaTablas, $Condicion);
if( $tReg>0 ) $GLOBALS['_InformarCada'] = $tReg/100;
SS::query("select {$ListaCampos} from {$ListaTablas} where {$Condicion} order by {$ListaOrdenacion}");
}
}else{
if( $_POST['_SqlToTron_']==$_ENV['ON'] ) eTron("select count(*) from {$ListaTablas} where {$Condicion}");
...
if( $Estructura[$n][0][0]==',' ) $Estructura[$n][0] = trim(mb_substr($Estructura[$n][0],1));
if( mb_substr($Estructura[$n][0],-1)==')' ){
$Estructura[$n][0] = trim(mb_substr($Estructura[$n][0],mb_strpos($Estructura[$n][0],'(')+1,-1));
if( eSubstrCount($Estructura[$n][0],',') ){
...
if( eSubstrCount($Estructura[$n][0],' as ') ){
...
case 'HTM':
if( $def__online=='V' ){
$NomFile = mb_substr($NomFile,0,-4);
if( eSubstrCount($NomFile,'.')==0 ) $NomFile .= ".tmp";
...
if( !function_exists("_eZipFile") ) include(DIREDES."itm/zip.php");
...
SS::query("insert into {$_ENV['SYSDB']}gs_acceso
...
$SerialDOC = SS::id();
$Dir = eGetCWD();
eZipFile(SETUP::$LogDownload['LogFileDownload'].$SerialDOC, $NomFile);
}
}
if( eSubstrCount($def__online,'D')>0 && eSubstrCount(',REC,HTM,TXT,',",{$cd_gs_destino},")>0 ){
$NmFileOri = $NomFile;
if( WINDOW_OS ) $NmFileOri = str_replace('\\','/',$NmFileOri);
if( $_DEBUG=='' ) eInit();
...
if( eSubstrCount($def__online,'V')>0 && eSubstrCount(',REC,HTM,',",{$cd_gs_destino},")>0 ){
$NmFileOri = $NomFile;
if( WINDOW_OS ) $NmFileOri = str_replace('\\','/',$NmFileOri);
if( $_DEBUG=='' ) eInit();
eExplodeLast(str_replace('\\','/',$NomFile), "/", $iz, $dch);
...
$gsFileOri = $NomFile;
if( WINDOW_OS ) $gsFileOri = str_replace('\\','/',$gsFileOri);
$xNomFile = mb_substr($gsFileOri, mb_strrpos($gsFileOri,'/')+1);
$NomExt = mb_substr($xNomFile,-3);
if( $NomExt=='mdb' ){
}else if( $gsTipo!='R' ){
$xNomFile = mb_substr($xNomFile, 0, mb_strrpos($xNomFile,'.')+1).'zip';
}else{
$xNomFile = mb_substr($xNomFile, 0, mb_strrpos($xNomFile,'.')+1).'htm';
}
...
if( $def__online!='2' ){
if( eSubstrCount(',TXT,DBF,PDF,XLS,HTM,REC,', ",{$cd_gs_destino},")>0 ){
if( eSubstrCount($def__online, 'H')>0 && $cd_gs_destino<>"REC" ){
SS::query("update {$_ENV['SYSDB']}gs_exp_file set download='".date('Y-m-d H:i:s')."' where {$FicheroAModificar}");
SS::close();
eMessage('Descargando '.$cd_gs_destino,'HS','','top.eCallSrv(window,"'.$DatoDownload.'");');
}else if( eSubstrCount($def__online,'V')>0 && $cd_gs_destino<>"REC" ){
...
}else if( eSubstrCount($def__online,'D')>0 ){
$NomFile = str_replace('\\','/',$NomFile);
$Dim = explode( '/',$NomFile);
$NomFile = $Dim[count($Dim)-1];
$NomFile = $GLOBALS['_sDownloadPath'].'/'.$NomFile;
...
if( eSubstrCount($def__online, 'H')>0 ){
...
}else if( eSubstrCount($def__online, 'V')>0 ){
$NomFile = str_replace('\\','/',$NomFile);
$Dim = explode( '/',$NomFile);
$NomFile = $Dim[count($Dim)-1];
$NomFile = $GLOBALS['_sDownloadPath'].'/'.$NomFile;
...
if( mb_strlen($q)>255 ){
$n = 1;
while( mb_strlen($q)>255 ){
$sql[$n] = mb_substr( $q, 0, 255 );
$q = mb_substr( $q, 255 );
$n++;
}
$sql[$n] = mb_substr( $q, 0, 255 );
...
if( WINDOW_OS ) $gsFileOri = str_replace('\\','/',$gsFileOri);
$NomFile = mb_substr( $gsFileOri, mb_strrpos($gsFileOri,'/') + 1 );
$NomExt = mb_substr($NomFile,-3);
if( $NomExt=='mdb' ){
}else if( $gsTipo!='R' ){
$NomFile = mb_substr( $NomFile, 0, mb_strrpos($NomFile,'.')+1 ) . 'zip';
}else{
$NomFile = mb_substr( $NomFile, 0, mb_strrpos($NomFile,'.')+1 ) . 'htm';
}
$Dir = mb_substr( $gsFileOri, 0, mb_strrpos($gsFileOri,'/') + 1 );
if( mb_strtoupper($gsFormato) == 'REC' ) $gsFormato = 'HTM';
$cdi = date('Y-m-d H:i:s');
if( $GLOBALS['_TronDownLoad'] ) error_log( "Estado: {$gsEstado}\n", 3, '___download__.info' );
if( $gsEstado=='P' ){
if( $GLOBALS['_TronDownLoad'] ){
error_log( "Pendiente....\n", 3, '___download__.info' );
error_log( "insert into {$_ENV['SYSDB']}gs_exp_file
...
SS::query("insert into {$_ENV['SYSDB']}gs_exp_file
(     descripcion  ,   fichero  ,  t_reg  ,    tipo   ,    formato   ,    cd_gs_formato  ,    estado    , comprimido, cd_gs_user ,  sql_1  ,  sql_2  ,  sql_3  ,   cdi  , descargado, sg ) values
('{$gsDescripcion}','{$NomFile}',{$gsTReg},'{$gsTipo}','{$gsFormato}','{$_cd_gs_formato}', '{$gsEstado}',  {$gsZip} ,{$gsUsuario},'$sql[1]','$sql[2]','$sql[3]','{$cdi}',     0     , 0  )");
...
SS::query( "select cd_gs_exp_file from {$_ENV['SYSDB']}gs_exp_file {$_RegActual}" );
list( $_IdReg ) = SS::get("num");
...
SS::query( "update {$_ENV['SYSDB']}gs_exp_file set estado='{$gsEstado}', t_reg={$gsTReg}, sg={$tSegundos}{$UnaDescarga} where fichero='{$NomFile}' and tipo='{$gsTipo}' and cd_gs_user={$gsUsuario} and cdi='{$gsCdi}'" );
...
if( $gsTipo=='R' ){
$gsFileOri = mb_substr( $gsFileOri, 0, mb_strrpos($gsFileOri,'.')+1 ) . 'sql';
@unlink( $gsFileOri );
}
return $cdi;
}
...
fputs($File, '<TITLE>'.SETUP::$System['ApplicationName'].'</TITLE><STYLE>');
...
$pI = mb_strpos( $txt, '/'.'* ' );
$pF = mb_strpos( $txt, ' *'.'/' );
$txt = trim(mb_substr( $txt, 0, $pI )) . mb_substr( $txt, $pF+3 );
}
$Dim = explode("\n",$txt);
$txt = '';
...
$nCondi = 0;
$txtC = '';
foreach($ValCount as $clave=>$val){
if( mb_substr($clave,0,3)=='cd_' ){
SS::query( "select * from {$_ENV['SYSDB']}gs_campo where cd_gs_entidad='{$cd_gs_entidad}' and campo='nm_".mb_substr($clave,3)."'" );
$r = SS::get();
if( $r['campo']!='' ){
$EtiCount[$clave] = $r['etiqueta'];
SS::query( "select {$r['campo']} from {$r['tabla']} where {$clave}='{$val}'" );
$r = SS::get("num");
$val = $r[0];
}
}
if( (mb_strlen($val)==10 || mb_strlen($val)==19) && isZero($val) ) $val = '';
if( $val!='' ){
if( $val=='=' ) $val = '';
$txtC .= '<tr>';
$txtC .= '<td>'.$EtiCount[$clave].'</td>';
if( mb_substr($clave,0,3)=='tf_' ){
if( $val==$_ENV['ON'] ) $val = 'Si';
if( $val=='<>'.$_ENV['ON'] ) $val = 'No';
}
$txtC .= '<td>'.$val.'</td>';
$txtC .= '</tr>';
$nCondi++;
...
global $Formato, $Etiquetas, $ListaAlineacion, $Pijama, $_DirRoot, $_VirtualVALUE, $_VirtualFIELD, $_db_, $dimAling;
...
$TColumnas = 0;
$txt = '<tr id=T>';
foreach($Etiquetas as $clave=>$val){
...
if( (mb_strlen($_vF[$i])==10 || mb_strlen($_vF[$i])==19) && isZero($_vF[$i]) ) $_vF[$i] = '';
}
$txt .= '<td>'.$_vF[$i];
}
}else{
...
fputs($File, $txt);
$tReg++;
if( $Informar<=$tReg && (eSubstrCount($def__online,'H')>0 || eSubstrCount($def__online,'2')>0) ){
SS::query("update {$_ENV['SYSDB']}gs_exp_file set t_reg={$Informar} where cd_gs_exp_file={$_IdReg}");
...
if( (mb_strlen($value)==10 || mb_strlen($value)==19) && isZero($value) ) $value = '';
$txt .= mb_substr(str_pad($value, $ListaAnchos[$nc], ' '), 0, $ListaAnchos[$nc]);
$nc++;
}
fputs( $File, $txt.CHR13.CHR10 );
$tReg++;
if( $Informar<=$tReg && (eSubstrCount($def__online,'H')>0 || eSubstrCount($def__online,'2')>0) ){
SS::query("update {$_ENV['SYSDB']}gs_exp_file set t_reg={$Informar} where cd_gs_exp_file={$_IdReg}");
if( $GLOBALS['_SqlTransaction'] ){
...
$NewRow	 = '~';
$fp = fopen( $NomFile, 'w' );
fwrite( $fp, 'colDelimiter='.$NewCampo.$Enter );
fwrite( $fp, 'rowDelimiter='.$NewRow.$Enter );
fwrite( $fp, 'replace=|::{&#124;}'.$Enter );
fwrite( $fp, 'replace=~::{&#126;}'.$Enter );
fwrite( $fp, 'cmd=open'.$Enter );
fwrite( $fp, 'tableName='.$NomTabla.$Enter );
...
$clave = mb_strtoupper(trim( str_replace('·',' ',trim($Etiquetas[$nc])) ));
while( eSubstrCount( $clave, '  ' ) ) $clave = str_replace( '  ', ' ', $clave );
$sClave = $clave;
$clave = '';
for( $i=0; $i<mb_strlen( $sClave ); $i++ ){
$n = mb_ord( mb_substr( $sClave, $i, 1 ) );
if( ( $n>=48 && $n<=57 ) || ( $n>=65 && $n<=90 ) || ( $n>=97 && $n<=122 ) || ( $n==95 ) ){
$clave .= mb_substr( $sClave, $i, 1 );
}else{
$clave .= '_';
}
}
if( eSubstrCount( '0123456789', $clave[0] ) > 0 ) $clave = '_'.$clave;
$clave = str_replace( '__', '_', $clave );
if( mb_strlen( $clave ) > 10 ) $clave = mb_substr( $clave, 0, 10 );
$tc = mb_strlen($clave);
while( in_array( $clave, $DimCampo ) ){
$sClave = $clave = mb_substr($clave,0,$tc );
...
$nl++;
dbf_Write( $File, $txt );
if( $Informar <= $nl && (eSubstrCount($def__online,'H')>0 || eSubstrCount($def__online,'2')>0) ){
SS::query("update {$_ENV['SYSDB']}gs_exp_file set t_reg={$Informar} where cd_gs_exp_file={$_IdReg}");
if( $GLOBALS['_SqlTransaction'] ){
SS::commit();
SS::beginTransaction();
...
$aux = mb_strlen( $cadena );
$dest = 0;
for( $i=0; $i < $aux; $i++ ){
$dest += Ord( mb_substr($cadena, $aux-1-$i, 1) ) * pow(256, $aux-1-$i);
}
return $dest;
}
...
$version = 3;
$fecha = date('ymd');
$fecha = mb_chr(substr ($fecha, 0, 2)).mb_chr(substr ($fecha, 2, 2)).chr (mb_substr($fecha, 4, 2));
$File = fopen( $NomFile, "w+" );
fwrite( $File, mb_chr($version) );
fwrite( $File, $fecha );
fwrite( $File, str_repeat( mb_chr(0), 4 ) );
$anchoCab += (32 * count($DimCampo) );
$anchoCab = Dec4Bin( $anchoCab, 2 );
$longitud = mb_strlen( $anchoCab );
$anchoCab .= str_repeat( mb_chr(0), 2-$longitud );
...
$longitud = mb_strlen( $anchoReg );
$anchoReg .= str_repeat( mb_chr(0), 2-$longitud );
fwrite( $File, $anchoReg );
fwrite( $File, str_repeat( mb_chr(0), 20 ) );
for( $i=0; $i<count($DimCampo); $i++ ){
$nombre = str_pad( trim($DimCampo[$i][0]), 10, mb_chr(0) ).mb_chr(0);
fwrite( $File, $nombre );
fwrite( $File, trim($DimCampo[$i][1]) );
fwrite( $File, str_repeat( mb_chr(0), 4 ) );
fwrite( $File, mb_chr( $DimCampo[$i][2]) );
fwrite( $File, mb_chr( $DimCampo[$i][3]) );
fwrite( $File, str_repeat( mb_chr(0), 14 ) );
}
fwrite( $File, mb_chr( HexDec( '0D' ) ) );
return $File;
}
...
$numReg .= str_repeat( mb_chr(0), 4-mb_strlen( $numReg ) );
fwrite( $File, $numReg );
fclose( $File );
}
...
$txt = '';
$campo = '';
for($i=0; $i<mb_strlen($DimCampo['campo']); $i++){
$c = mb_substr($DimCampo['campo'],$i,1);
switch( $c ){
case ",":
if( eSubstrCount($DimCampo['campo'],'(')==0 ){
...
case "'":
$campo .= $c;
for( $p=$i+1; $p<mb_strlen($DimCampo['campo']); $p++ ){
$c2 = mb_substr($DimCampo['campo'],$p,1);
$campo .= $c2;
if( $c==$c2 ) break;
}
for( $p=$p+1; $p<mb_strlen($DimCampo['campo']); $p++ ){
$c2 = mb_substr($DimCampo['campo'],$p,1);
if( $c2==',' ){
if( SS::isDriver('mysql,mysqli') ){
$campo .= $c2;
...
if( SS::isDriver('mysql,mysqli') ) $txt = 'concat('.$txt.')';
return $txt;
}
