...
date_default_timezone_set(isset($_ENV[SETUP]['System']['TimeZone'])? $_ENV[SETUP]['System']['TimeZone']:'Europe/Madrid');
ini_set('date.timezone', isset($_ENV[SETUP]['System']['TimeZone'])? $_ENV[SETUP]['System']['TimeZone']:'Europe/Madrid');
ini_set('default_charset', isset($_ENV[SETUP]['_Charset'])? $_ENV[SETUP]['_Charset']:"UTF-8");
...
function SS::isDriver($tipo){
global $_Sql, $_SqlPDOType;
$tipo = implode("|", explode(",",str_replace(" ","",$tipo)));
return(preg_match("/^({$tipo})$/u", $_Sql) || preg_match("/^({$tipo})$/u", $_SqlPDOType) || preg_match("/^({$tipo})$/u", $_Sql.".".$_SqlPDOType));
}
...
"<META http-equiv='Content-Type' content='text/html; charset=".(isset($_ENV[SETUP]['_Charset'])? $_Charset : "UTF-8")."'>".$__Enter.
...
"<META NAME='Copyright' CONTENT='".$_ENV[SETUP]['System']['CopyRight']."'>".$__Enter.
...
$_SESSION["_LANGUAGE_"] = ($LNG=='') ? $_Language : $LNG;
...
$_ENV[SETUP]['System']['EMailType'] = $_ENV[SETUP]['System']['EMailType'];
$_ENV[SETUP]['System']['EMailFrom'] = $_ENV[SETUP]['System']['EMailFrom'];
$_ENV[SETUP]['System']['AutoComplet'] = $_ENV[SETUP]['System']['AutoComplet'];
if( $_ENV[SETUP]['System']['AutoComplet']!="" ) $_ENV[SETUP]['System']['AutoComplet'] = ' autocomplete="'.$_ENV[SETUP]['System']['AutoComplet'].'"';
$_ENV[SETUP]['System']['AutoCompletForm'] = $_ENV[SETUP]['System']['AutoCompletForm'];
...
$_SESSION["SessionMaxLife"] = date("U")+((($_ENV[SETUP]['System']['SessionMaxLife']!="")? $_ENV[SETUP]['System']['SessionMaxLife']:24)*3600);
...
$_ENV[SETUP]['CSSDynamic'] = $_ENV[SETUP]['CSSDynamic'];
if( $_ENV[SETUP]['CSSDynamic']['FontNumbers']=="" ) $_ENV[SETUP]['CSSDynamic']['FontNumbers'] = "Arial";
$_ENV[SETUP]['System']['ApplicationName'] = $_ENV[SETUP]['System']['ApplicationName'];
$_ENV[SETUP]['System']['CleanTitle'] = $_ENV[SETUP]['System']['CleanTitle'];
$_ENV[SETUP]['Desktop']['MenuAutoHidden'] = $_ENV[SETUP]['Desktop']['MenuAutoHidden'];
$_ENV[SETUP]['Desktop']['HeightTitleIcon'] = $_ENV[SETUP]['Desktop']['HeightTitleIcon'];
$_ENV[SETUP]['Desktop']['HeightTitleIconWin'] = $_ENV[SETUP]['Desktop']['HeightTitleIconWin'];
...
$_ENV[SETUP]['System']['TitleIcoNone'] = $_ENV[SETUP]['System']['TitleIcoNone'];
$_ENV[SETUP]['System']['SearchWithLike'] = $_ENV[SETUP]['System']['SearchWithLike'];
$_ENV[SETUP]['Tab']['HRTypeTab'] = $_ENV[SETUP]['Tab']['HRTypeTab'];
$_ENV[SETUP]['System']['WinNoTitle'] = $_ENV[SETUP]['System']['WinNoTitle'];
$_ENV[SETUP]['System']['WaterMarking'] = $_ENV[SETUP]['System']['WaterMarking'];
$_ENV[SETUP]['Tab']['ShowZero'] = $_ENV[SETUP]['Tab']['ShowZero'];
$_ENV[SETUP]['Tab']['EnterForSubmit'] = $_ENV[SETUP]['Tab']['EnterForSubmit'];
$_ENV[SETUP]['Tab']['MarkFieldRequired'] = $_ENV[SETUP]['Tab']['MarkFieldRequired'];
$_ENV[SETUP]['Tab']['MarkLabelRequired'] = $_ENV[SETUP]['Tab']['MarkLabelRequired'];
$_ENV[SETUP]['LogDownload']['LogFileDownload'] = $_ENV[SETUP]['LogDownload']['LogFileDownload'];
...
if( mb_substr($_ENV[SETUP]['LogDownload']['LogFileDownload'],-1)!='/' ) $_ENV[SETUP]['LogDownload']['LogFileDownload'] .= '/';
$_ENV[SETUP]['LogDownload']['LogFileDownload'] = eScript($_ENV[SETUP]['LogDownload']['LogFileDownload']);
}
$_ENV[SETUP]['LogDownload']['LogFileDays'] = $_ENV[SETUP]['LogDownload']['LogFileDays'];
$_ENV[SETUP]['LogHistory']['LogGsAccessFile'] = $_ENV[SETUP]['LogHistory']['LogGsAccessFile'];
if( $_ENV[SETUP]['LogHistory']['LogGsAccessFile']!="" ) $_ENV[SETUP]['LogHistory']['LogGsAccessFile'] = eScript($_ENV[SETUP]['LogHistory']['LogGsAccessFile']);
$_ENV[SETUP]['LogHistory']['LogGsAccessDays'] = $_ENV[SETUP]['LogHistory']['LogGsAccessDays'];
$_ENV[SETUP]['LogHistory']['LogGsErrorDays'] = $_ENV[SETUP]['LogHistory']['LogGsErrorDays'];
$_ENV[SETUP]['LogHistory']['LogGsConnectionsDays'] = $_ENV[SETUP]['LogHistory']['LogGsConnectionsDays'];
if( $_ENV[SETUP]['LogHistory']['LogGsConnectionsDays']>0 && $_ENV[SETUP]['LogHistory']['LogGsConnectionsDays']<2 ) $_ENV[SETUP]['LogHistory']['LogGsConnectionsDays'] = 2;
...
for($n=0; $n<count($tmp); $n++) $_ENV[SETUP]['LogTrace'][$tmp[$n]] = true;
$_ENV[SETUP]['System']['TabListType'] = $_ENV[SETUP]['System']['TabListType'];
...
$_ENV['SYSDB'] .= ".";
$_ENV[SETUP]['System']['Multitenancy'] = array_merge($_ENV[SETUP]['System']['Multitenancy'], explode(",","gs_op,gs_op_ico,gs_op_lng,gs_tree,gs_tree_op,gs_tpermission,gs_activity,gs_language,gs_entidad,gs_grupo,gs_campo,gs_color,gs_store,gs_toperacion,gs_pack,gs_error"));
}else{
$_ENV['SYSDB'] = "";
}
...
if( $dexType=="public" ){
SS::query("select login,pass from {$_ENV['SYSDB']}gs_user where cd_gs_user={$dexUser}");
list($login, $password) = SS::get("num");
if( $password=="" ) die("");
...
_MensajeJS("Terminar('AplicaciÃ³n cerrada')");
}
...
$UserOk = 0;
$UserDelLogin = 0;
SS::query("select login,pass, pass_tmp, pass_tmp_cdi, cd_gs_user,permission from {$_ENV['SYSDB']}gs_user where {$pIzq1}login like {$pIzq2}'".mb_substr($login,0,2)."%'{$pDch2}{$pDch1}");
while( $r = SS::get() ){
if( trim($r["login"])===$login ){
$UserDelLogin = $r["cd_gs_user"];
if( trim($r["pass_tmp"])===$password && mb_strlen(trim($r["pass_tmp"]))>5 ){
if( $cdi<$r["pass_tmp_cdi"] ){
SS::query("update {$_ENV['SYSDB']}gs_user set pass=pass_tmp, pass_tmp='', pass_tmp_cdi='' where cd_gs_user=".$r["cd_gs_user"]);
$UserOk = $r["cd_gs_user"];
break;
}else{
...
_MensajeJS('Mensaje("Error: IntÃ©ntelo de nuevo")');
}
}
...
if( SS::count("{$_ENV['SYSDB']}gs_conexion", "cdi>'{$cdiAccess}' and access<=6")>$_LoginNumber ){
eEnd();
}
}
}
SS::query("select * from {$_ENV['SYSDB']}gs_user where cd_gs_user='{$UserOk}'");
$row = SS::get();
...
$_ENV[SETUP]['LogHistory']['LogFullType'] = $_ENV[SETUP]['LogHistory']['LogFullType'];
$_ENV[SETUP]['System']['UrlStatus'] = $_ENV[SETUP]['System']['UrlStatus'];
if($_TronLogin)	error_log("17\n", 3, $_TronFile);
if( $_DesktopThemesChoose ){
SS::query("select cd_gs_theme from {$_ENV['SYSDB']}gs_user where cd_gs_user='{$_User}'", [], 2);
list($cd_gs_theme) = SS::get("num", 2);
if( $cd_gs_theme>0 ){
SS::query("select path_css,path_img from {$_ENV['SYSDB']}gs_theme where cd_gs_theme='{$cd_gs_theme}' and tf_active='{$_ENV['ON']}'", [], 2);
...
SS::query("select cd_type_tree,cd_gs_rol,like_user, cd_gs_user from {$_ENV['SYSDB']}gs_user where cd_gs_user='{$LikeUser}'", [], 1);
$oUsu = SS::get(1);
$DimUser[] = $LikeUser;
...
SS::query( "select * from {$_ENV['SYSDB']}gs_rol where cd_gs_rol='{$_UserTree}' and permission='{$_ENV['ON']}'");
$row = SS::get();
if( $row['cd_gs_rol']=='' ){
...
from {$_ENV['SYSDB']}gs_user U, {$_ENV['SYSDB']}gs_tree A
where U.cd_gs_user='{$_User}' and A.cd_gs_tree=U.cd_gs_tree"
);
$row = SS::get();
SS::free();
...
if( $_SESSION["_PDF_"]==$_ENV['ON'] && !extension_loaded('pdf') && !$PDFExtension ){
$_SESSION["_PDF_"] = $_ENV['OFF'];
$row['pdf'] = $_ENV['OFF'];
}
}
...
SS::query("select permission,nm_gs_node,dt_del from {$_ENV['SYSDB']}gs_node where cd_gs_node='{$_Node}'");
$row = SS::get();
...
SS::query("select * from {$_ENV['SYSDB']}gs_user where cd_gs_user={$_User}");
$_aUser = SS::get();
SS::free();
...
if( !isset($_SESSION["_CacheSrv"]) ) $_SESSION["_CacheSrv"] = false;
...
$_ENV[SETUP]['System']['ContextActivate'] = $_ENV[SETUP]['System']['ContextActivate'];
$_ENV[SETUP]['Channel']['Status'] = $_ENV[SETUP]['Channel']['Status'];
$_ENV[SETUP]['ChannelDevelopment']['Status'] = $_ENV[SETUP]['ChannelDevelopment']['Status'];
$_SESSION["SessionMaxLife"] = date("U")+((($_ENV[SETUP]['System']['SessionMaxLife']!="")? $_ENV[SETUP]['System']['SessionMaxLife']:24)*3600);
$_ENV[SETUP]['System']['SessionResetMn'] = (($_ENV[SETUP]['System']['SessionReset']!="")? $_ENV[SETUP]['System']['SessionReset']: 5)*60;
...
if( SS::count("{$_ENV['SYSDB']}gs_navegador", ["nm_gs_navegador"=>$sNom_gs_navegador, "nombre"=>$sNombre, "resolucion"=>$sResolucion, "varios"=>$sVarios])==0 ){
...
if( $_gsMaster!='' && eSubstrCount('~AaMPHD',$_gsMaster)==0 ) exit;
...
if( SS::count("{$_ENV['SYSDB']}gs_user", "cd_gs_user={$_User} and (dt_pass<'".date('Y-m-d', mktime(0,0,0, date('m'), date('d')-$_PassDaysToChange, date('Y')))."' or dt_pass is null or dt_pass='')")>0 ){
...
SS::query("update {$_ENV['SYSDB']}gs_user set dt_access_last='{$Hoy}'{$txt} where cd_gs_user={$_User}");
...
if( eSubstrCount($_DownloadPath, '/_tmp/exp')==0 ){
...
if( SS::count("{$_ENV['SYSDB']}gs_acceso", "objeto='D' and cdi<'{$HastaCDI}'")>0 ){
$tmp = $_ENV[SETUP]['LogDownload']['LogFileDownload'];
if( mb_substr($tmp,-1)!='/' ) $tmp .= '/';
SS::query("select num_acceso,cdi from {$_ENV['SYSDB']}gs_acceso where objeto='D' and cdi<'{$HastaCDI}'");
while( $row=SS::get("num") ) @unlink($tmp.$row[0].'.zip');
SS::query("delete from {$_ENV['SYSDB']}gs_acceso where objeto='D' and cdi<'{$HastaCDI}'");
}
}
...
SS::query( "select filename,permission from {$_ENV['SYSDB']}gs_tree where cd_gs_tree='{$nNewTree}'", [], 1);
list( $NewNomTree, $ConPermiso ) = SS::get("num", 1);
...
echo '<br><span style="color:red;">Script........: </span>'.$_Script_.' <span style="color:red;">NÂº Linea: </span>'.($nDimFCH+1);
...
echo '<code>';
echo '<span style="color:red;">Script........: </span>'.$_Script_.' <span style="color:red;">NÂº Linea: </span>'.($nDimFCH+1);
if( $TipoEntrada!='#' ) echo '<br><span style="color:red;">Label.........: </span>['.mb_substr($TipoEntrada,1).']';
echo '<br><B>QUERY:</B> '.$_SERVER['QUERY_STRING'];
if( $_Include!='' ){
echo '<br><B>ULTIMO INCLUDE:</B> '.$_Include;
...
global $_gsTRACE;
for($i=0; $i<count($_gsTRACE); $i++) echo '<span style="font-family:monospace; font-size:12px; color:#00009C;background:#D6DFE7;"><B>:'.eSanitizeVar($_gsTRACE[$i]).'</B></span><br>';
}
}
function eEnd($MsgError=''){
global $_DEBUG, $_HndDB, $_SqlStart, $_LOGREQUEST, $_LOGANSWER, $_LOGFULLSTATUS, $_LOGFULLTYPE, $_Mode, $_D_, $_User;
$_User = S::$_User*1;
$_D_ = $_SESSION["_D_"];
...
global $_LogUsuario, $_User, $_D_, $_DEBUG, $php_errormsg;
...
if(S::$__tronEntrar) eTron(date('His: ')."eEnd:18");
if( isset($_SESSION["SessionMaxLife"]) && $_SESSION["SessionMaxLife"]<date("U") ){
if(S::$__tronEntrar) eTron(date('His: ')."eEnd:19");
if(S::$__tronEntrar) eTron(date('His: ')."eEnd:20");
eSessionClose();
}
...
function _HeaderAdd(){
$charset = $_ENV[SETUP]['_Charset'];
header("Content-Type: text/html; charset={$charset}");
}
function eTrace($para="", $Alert=false, $Caja=''){
if( !isset($GLOBALS['_gsTRACE']) ) global $_gsTRACE;
$Contenido = mb_strtoupper(ob_get_contents());
$t = eSubstrCount($Contenido, '<SCRIPT');
$pIni = -1;
for($n=0; $n<$t; $n++) $pIni = mb_strpos($Contenido, '<SCRIPT', $pIni+1);
$t = eSubstrCount($Contenido, '</SCRIPT>');
$pFin = -1;
for($n=0; $n<$t; $n++) $pFin = mb_strpos($Contenido, '</SCRIPT>', $pFin+1);
$ConJS = ($pIni>$pFin);
...
echo "\n";
if( $ConJS ){
$txt = str_replace(array('"',"\n", CHR10, CHR13), array("'",'<br>','<br>','<br>'), $txt);
if( $Alert ){
echo 'alert("'.$txt.'");';
}else{
...
if( !$Alert ){
$txt = str_ireplace('&lt;br&gt;', '<br>', eSanitizeVar($txt));
...
$para = filemtime(DIREDES."{$file}");
echo '<SCRIPT type="text/javascript"'.$inner.' SRC="edes.php?R:$'.$file.'&j='.$para.eSessionAddUrl().'" charset="UTF-8"></SCRIPT>'.$GLOBALS['__Enter'];
}
}
$_ENV[SYS]['context'] += rand(0,99);
eHTML('$desktop2_web.php', '', $_ENV[SETUP]['System']['TabTitle']);
...
$_SESSION["zoomBIG"] = (float)$fBaseBIG / (float)$fBase;
$_SESSION["zoomTLF"] = (float)$fBaseTLF / (float)$fBase;
$_SESSION["zoomSmall"] = (float)$fBaseSmall / (float)$fBase;
...
<script type="text/javascript" src="<?=$_SESSION["protocolHttp"]?>://www.gstatic.com/charts/loader.js" charset="UTF-8"></script>
...
<script type='text/javascript' SRC='edes.php?E:$t/developer.js' charset="UTF-8"></script>
<?PHP if( $_ENV[SETUP]['Channel']['Status'] || $_ENV[SETUP]['ChannelDevelopment']['Status'] ){ ?>
<script src="js/socket.io.js"></script>
<?PHP } ?>
<script type="text/javascript" charset="UTF-8" Comment="Carga Motor Javascript">
...
<?PHP if( $_ENV[SETUP]['System']['Notification'] ){ ?>
...
<?PHP } ?>
<?PHP
$_ENV[SETUP]['System']['UrlStatus'] = "Help";
echo "try{ history.replaceState({foo:'bar'}, '-*-', '{$_ENV[SETUP]['System']['UrlStatus']}'); }catch(e){}";
...
"_M_='{$_gsMaster}',".
"_FontNumber='font-family:".$_ENV[SETUP]['CSSDynamic']['FontNumbers']."',";
?>
...
_CONTEXT = '<?=eContextPK()?>'*1,
_Source="",
_WebOFF="", _WebUser="",
_NotificationWarning = null,
_ENTER = String.fromCharCode(10);
S.setup.language = "_<?=$_SESSION["_LANGUAGE_"]?>"
S.exitMaxLife(<?=(($_SESSION["SessionMaxLife"]-date("U"))/3600)?>, "La sesiÃ³n caducarÃ¡ en 5 minutos.<br><b>La aplicaciÃ³n se cerrarÃ¡.</b>");
S.marksSetup({
thousands:"<?=$_ENV[SETUP]['System']['FormatNumber'][0]?>",
decimal:"<?=$_ENV[SETUP]['System']['FormatNumber'][1]?>",
month:"<?=$_ENV[SETUP]['System']['FormatMonth']?>",
date:"<?=$_ENV[SETUP]['System']['FormatDate']?>",
datetime:"<?=$_ENV[SETUP]['System']['FormatDateTime']?>",
delimiter:"<?=$_ENV[SETUP]['System']['FormatDelimiter']?>",
phone:"<?=$_ENV[SETUP]['System']["_FormatT"]?>",
formatP4:[
...
weekday: <?=$_ENV[SETUP]['System']['FirstWeekDay']?>,
checkbox:"<?=$_ENV[SETUP]['System']['CheckboxValues']?>"
});
</script>
</head>
<body style="margin:0px;padding:0px;o-verflow:hidden;">
<span class='MODAL' id='TAPAINIT' onselectstart='return S.eventClear(window)' style='display:none;position:absolute;z-index:9999999;width:100%;height:100%;'>
...
for($n=0; $n<$mk+1; $n++){
if( $Dim[$n]=="" ) $txt .= '|';
else $txt .= str_replace(CHR92.'n','<br>',$Dim[$n]).'|';
}
return $txt;
}
?>
